 ...
 *		GC for expose time)
 */
ControlLine* CreateControlLine( ControlField* field, int load_limit,
                                Pixel color )
{
  ControlLine* line;
  ...
  if ( line->field->num_lines == 3 )
  {
    if ( line->field->vertical )
    {
      ...
      for ( i = 0; i < line->last_level_index; i++ )
      {
        ...
        val_coord = (int)( ( line->values[i] * value_range ) + 0.5 ) + margin;
    ...
    if ( line->field->num_maps > 0 )
    {
      for ( i = 0; i < line->field->map[0]->num_points; i++ )
      {
        if ( i == 0 )
        {
          ...
          line_point->y = line->field->height + line->field->top_margin - 1;
          line_point++;
        }
        line_point->x = line->field->map[0]->points[i].x;
...
void DrawLine( ControlLine* line, GC gc )
{
  ...
  if ( !line->field->w )
    return;
  if ( !XtIsRealized( ( Widget )( line->field->w ) ) )
    return;
  XDrawLines( XtDisplay( line->field->w ), XtWindow( line->field->w ), gc,
              line->points, line->num_points, CoordModeOrigin );
