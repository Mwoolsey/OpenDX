...
static XtResource resources[] = {
    ...
    {XmNstart,                                                XmCStart,
     ...
     XtOffset( XmVCRControlWidget, vcr_control.start_value ), XmRImmediate,
     (XtPointer)0},
    {XmNnext,                                                XmCNext,
     XmRShort,                                               sizeof( short ),
     XtOffset( XmVCRControlWidget, vcr_control.next_value ), XmRImmediate,
     (XtPointer)0},
    {XmNcurrent,                                                XmCCurrent,
     XmRShort,                                                  sizeof( short ),
     XtOffset( XmVCRControlWidget, vcr_control.current_value ), XmRImmediate,
     (XtPointer)0},
    {XmNcurrentVisible,
     XmCCurrentVisible,
     XmRBoolean,
    ...
    {XmNstop,                                                XmCStop,
     XmRShort,                                               sizeof( short ),
     XtOffset( XmVCRControlWidget, vcr_control.stop_value ), XmRImmediate,
     (XtPointer)0},
    {XmNminimum,                                            XmCMinimum,
     XmRShort,                                              sizeof( short ),
     XtOffset( XmVCRControlWidget, vcr_control.min_value ), XmRImmediate,
     (XtPointer)1},
    {XmNmaximum,                                            XmCMaximum,
     XmRShort,                                              sizeof( short ),
     XtOffset( XmVCRControlWidget, vcr_control.max_value ), XmRImmediate,
     (XtPointer)10},
    {XmNincrement,
     XmCIncrement,
     XmRShort,
     ...
     XmRImmediate,
     (XtPointer)1},
    {XmNforwardButtonState,
     XmCForwardButtonState,
     XmRBoolean,
     sizeof( Boolean ),
     XtOffset( XmVCRControlWidget, vcr_control.button_is_in[VCR_FORWARD] ),
     XmRImmediate,
     (XtPointer)0},
    {XmNbackwardButtonState,
     XmCBackwardButtonState,
     XmRBoolean,
     sizeof( Boolean ),
    ...
    {XmNlimitColor,                                           XmCLimitColor,
     ...
     XtOffset( XmVCRControlWidget, vcr_control.limit_color ), XmRCallProc,
     (XtPointer)_XmForegroundColorDefault},
    {XmNnextColor,                                           XmCNextColor,
     XmRPixel,                                               sizeof( Pixel ),
     XtOffset( XmVCRControlWidget, vcr_control.next_color ), XmRCallProc,
     (XtPointer)_XmForegroundColorDefault},
    {XmNcurrentColor,                                           XmCCurrentColor,
     XmRPixel,                                                  sizeof( Pixel ),
     XtOffset( XmVCRControlWidget, vcr_control.current_color ), XmRCallProc,
     (XtPointer)_XmForegroundColorDefault},
    {XmNframeSensitive,
     XmCFrameSensitive,
     XmRBoolean,
...
static Boolean SetValues( XmVCRControlWidget current,
                          XmVCRControlWidget request, XmVCRControlWidget new );
...
static void Initialize( XmVCRControlWidget request, XmVCRControlWidget new )
{
  ...
  new->vcr_control.shell =
      XmCreateDialogShell( ( Widget ) new, "Frame Control", wargs, 6 );
                          ...
                          XmVCRControlWidget request, XmVCRControlWidget new )
{
  ...
  if ( new->vcr_control.frame_increment >
       ( new->vcr_control.stop_value - new->vcr_control.start_value ) )
  {
    new->vcr_control.frame_increment =
        new->vcr_control.stop_value - new->vcr_control.start_value;
  };
  ...
  for ( i = 0; i < VCR_STOP; i++ ) /* To first non-resource button */
  {
    ...
    {
      new->vcr_control.button_is_in[i] = current->vcr_control.button_is_in[i];
      SimulateButtonPress( new, i );
      XFlush( XtDisplay( new ) );
    }
  }
  if ( current->vcr_control.min_value != new->vcr_control.min_value )
  {
    ...
    if ( new->vcr_control.min_value > new->vcr_control.next_value )
      ChangeVCRNextValue( new, new->vcr_control.min_value, TRUE, FALSE, TRUE );
  }
  if ( current->vcr_control.max_value != new->vcr_control.max_value )
  {
    XtSetArg( wargs[0], XmNmaximum, new->vcr_control.max_value );
    XtSetValues( ( Widget ) new->vcr_control.frame_control, wargs, 1 );
    if ( new->vcr_control.max_value < new->vcr_control.next_value )
      ChangeVCRNextValue( new, new->vcr_control.max_value, TRUE, FALSE, TRUE );
  }
  if ( current->vcr_control.start_value != request->vcr_control.start_value )
  {
    ChangeVCRStartValue( new, request->vcr_control.start_value, TRUE, TRUE,
                         FALSE );
  }
  if ( current->vcr_control.stop_value != request->vcr_control.stop_value )
  {
    ChangeVCRStopValue( new, request->vcr_control.stop_value, TRUE, TRUE,
                        FALSE );
  }
  if ( current->vcr_control.next_value != request->vcr_control.next_value )
  {
    ChangeVCRNextValue( new, new->vcr_control.next_value, TRUE, TRUE, FALSE );
  }
  if ( current->vcr_control.current_value !=
       request->vcr_control.current_value )
  {
    ChangeVCRCurrentValue( new, new->vcr_control.current_value, TRUE, TRUE,
                           FALSE );
  }
  if ( current->vcr_control.frame_increment !=
       request->vcr_control.frame_increment )
  {
    ChangeVCRIncrementValue( new, new->vcr_control.frame_increment, TRUE, TRUE,
                             FALSE );
  }
  if ( current->vcr_control.next_color != new->vcr_control.next_color )
  {
    XtSetArg( wargs[0], XmNnextColor, new->vcr_control.next_color );
    XtSetValues( ( Widget ) new->vcr_control.frame_control, wargs, 1 );
  }
  if ( current->vcr_control.current_color != new->vcr_control.current_color )
  {
    XtSetArg( wargs[0], XmNcurrentColor, new->vcr_control.current_color );
    XtSetValues( ( Widget ) new->vcr_control.frame_control, wargs, 1 );
  }
  if ( current->vcr_control.limit_color != new->vcr_control.limit_color )
  {
  ...
  if ( current->vcr_control.min_sensitive != new->vcr_control.min_sensitive )
  {
    XtVaSetValues( ( Widget ) new->vcr_control.frame_control, XmNminSensitive,
                   new->vcr_control.min_sensitive, NULL );
  }
  if ( current->vcr_control.max_sensitive != new->vcr_control.max_sensitive )
  {
    XtVaSetValues( ( Widget ) new->vcr_control.frame_control, XmNmaxSensitive,
                   new->vcr_control.max_sensitive, NULL );
  }
  if ( current->vcr_control.inc_sensitive != new->vcr_control.inc_sensitive )
  {
    XtVaSetValues( ( Widget ) new->vcr_control.frame_control, XmNincSensitive,
                   new->vcr_control.inc_sensitive, NULL );
  }
...
static void SetLabel( XmVCRControlWidget vcr, int i, char *bits, int width,
                      int height )
{
  Arg wargs[2];
  unsigned long fg, bg;
  ...
  vcr->vcr_control.button[i] = (XmDrawingAreaWidget)XmCreateDrawingArea(
      (Widget)vcr->vcr_control.frame[i], "label", wargs, 0 );
  ...
  vcr->vcr_control.button_image[i].pixid = XCreatePixmapFromBitmapData(
      display, XRootWindowOfScreen( XtScreen( vcr ) ), bits, width, height, fg,
      bg, DefaultDepth( display, XScreenNumberOfScreen( XtScreen( vcr ) ) ) );
...
static void CreateVCRCounter( XmVCRControlWidget vcr )
{
  ...
  vcr->vcr_control.button[VCR_COUNT] = (XmDrawingAreaWidget)XmCreateDrawingArea(
      (Widget)vcr->vcr_control.frame[VCR_COUNT], "label", wargs, 0 );
  ...
  vcr->vcr_control.label = (XmLabelWidget)XmCreateLabel(
      (Widget)vcr->vcr_control.button[VCR_COUNT], "label", wargs, 4 );
  ...
  XtSetArg( wargs[1], XmNy, 15 );
  XtSetArg( wargs[2], XmNlabelString, text );
  vcr->vcr_control.dot_dot_dot = (XmLabelWidget)XmCreateLabel(
      (Widget)vcr->vcr_control.button[VCR_COUNT], "label", wargs, 3 );
  XmStringFree( text );
  XtManageChild( (Widget)vcr->vcr_control.dot_dot_dot );

  XtAddEventHandler( (Widget)vcr->vcr_control.label, ButtonPressMask, False,
                     _VCRButtonEvent, vcr );
                              ...
                              XmDrawingAreaCallbackStruct *cb )
{
  ...
  y = ( vcr->vcr_control.button[VCR_COUNT]->core.height -
        vcr->vcr_control.label->core.height ) /
      2;
  ...
  x2 = x1 + ( ( vcr->vcr_control.button[VCR_COUNT]->core.width -
                ( x1 + vcr->vcr_control.dot_dot_dot->core.width ) ) /
              2 );
  y = ( vcr->vcr_control.button[VCR_COUNT]->core.height -
        vcr->vcr_control.dot_dot_dot->core.height ) /
      2;
 ...
 */
static void _VCRButtonEvent( Widget w, XmVCRControlWidget vcr, XEvent *event,
       ...
       call_data->event->xbutton.button == Button1 )
  {
    ...
    else if ( w == vcr->vcr_control.button[VCR_COUNT] )
    {
      ...
      else
      {
        ...
        ChangeVCRCurrentValue( vcr, vcr->vcr_control.current_value, TRUE, TRUE,
                               FALSE );
        ChangeVCRNextValue( vcr, vcr->vcr_control.next_value, TRUE, TRUE,
                            FALSE );
      }
    }
  }
...
static void ReplaceFrame( XmFrameWidget frame )
{
  ...
  if ( XtIsRealized( (Widget)frame ) )
    ...
    _XmDrawShadowType( (Widget)frame, frame->frame.shadow_type,
                       ...
                       frame->manager.shadow_thickness, highlight_thickness,
    ...
    XmeDrawShadows( XtDisplay( (Widget)frame ), XtWindow( (Widget)frame ),
                    frame->manager.top_shadow_GC,
                    frame->manager.bottom_shadow_GC, highlight_thickness,
                    highlight_thickness, frame->core.width, frame->core.height,
                    frame->manager.shadow_thickness, frame->frame.shadow_type );
#endif /* XmVersion */
#endif /* OLD_LESSTIF */
}
                        ...
                        XmAnyCallbackStruct *call_data )
{
  ...
  XTranslateCoordinates( display, vcrW, XRootWindowOfScreen( XtScreen( vcr ) ),
  ...
  */
  ...
  XQueryTree( display, vcrW, &rootW, &parentW, &childrenW, &numChildren );
  if ( childrenW )
    XFree( (char *)childrenW );

  XQueryTree( display, parentW, &rootW, &parentW, &childrenW, &numChildren );
  if ( childrenW )
    XFree( (char *)childrenW );
  ...
  XtVaSetValues( dialog, XmNx, dest_x, XmNy, dest_y, NULL );
}
                              ...
                              Boolean to_outside )
{
  ...
  if ( to_outside )
  {/*&& vcr->vcr_control.value_callback )*/
...
{
  return (
      XtCreateWidget( name, xmVCRControlWidgetClass, parent, args, num_args ) );
}
