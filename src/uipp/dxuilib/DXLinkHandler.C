                                    ...
                                    boolean includeSubclasses )
{
  ...
  {
    result = theDXApplication->network->makeClassifiedNodeList(
        classname, includeSubclasses );
  }
  else if ( qualifierType == QUALIFIER_LABEL )
  {
    Node *node;
#if 00
    List *list = theDXApplication->network->makeLabelledNodeList( qualifier );
...
#else
    List *list = theDXApplication->network->makeClassifiedNodeList(
        classname, includeSubclasses );
...
boolean DXLinkHandler::setRenderMode( const char *msg, int id, boolean swmode )
{
  ...
  if ( !list )
  {
    ...
    sprintf( buffer, "SetRenderMode: "
                     "unable to set Image render mode (%s)",
             msg );
...
boolean DXLinkHandler::OpenVPE( const char *c, int id, void *va )
{
  ...
  {
    DXLinkHandler *a = (DXLinkHandler *)va;
    a->sendPacket( DXPacketIF::PKTERROR, id, "cannot create editor" );
    return FALSE;
  }
...
boolean DXLinkHandler::SetTabValue( const char *c, int id, void *va )
{
  ...
  sscanf( c, "%[^_]_%[^_]_%d_%[^_]_%d = %n", macro, module, &inst, inout,
          &paramInd, &length );
  const char *value = c + length;
  ...
  {
    sprintf( buffer, "Value \"%s\" not valid for %sput %d of %s", value, inout,
             paramInd, module );
    ErrorMessage( buffer );
    a->sendPacket( DXPacketIF::PKTERROR, id, buffer );
    return FALSE;
...
boolean DXLinkHandler::QueryValue( const char *c, int id, void *va )
{
  ...
  sscanf( c, "%[^_]_%[^_]_%d_%[^_]_%d", macro, module, &inst, inout,
          &paramInd );
...
boolean DXLinkHandler::SyncExec( const char *, int id, void *va )
{
  ...
  if ( pif )
  {
    ...
    sd->id = id;
    sd->dxlh = a;
    pif->send( DXPacketIF::FOREGROUND, "Executive(\"nop\");", SyncCB,
               (void *)sd );
  }
  else
...
boolean DXLinkHandler::Version( const char *c, int id, void *va )
{
  DXLinkHandler *a = (DXLinkHandler *)va;
  char buffer[1024];
  sprintf( buffer, "UI version: %d %d %d\n", DX_MAJOR_VERSION, DX_MINOR_VERSION,
           DX_MICRO_VERSION );
...
boolean DXLinkHandler::SetProbePoint( const char *c, int id, void *va )
{
  ...
  l = theDXApplication->network->makeClassifiedNodeList( ClassProbeNode,
                                                         FALSE );
  if ( !l )
  {
...
boolean DXLinkHandler::SelectProbe( const char *c, int id, void *va )
{
  ...
  if ( n == 0 )
  {
    a->sendPacket( DXPacketIF::PKTERROR, id, "missing probe name" );
    return FALSE;
  }
  ...
  if ( !images || 0 == images->getSize() )
  {
    ...
    {
      sprintf( buffer, "SelectProbe: no images matching %s in network", arg1 );
      a->sendPacket( DXPacketIF::PKTERROR, id, buffer );
    }
    if ( images )
      delete images;
...
boolean DXLinkHandler::SetInteractionMode( const char *c, int id, void *va )
{
  ...
  if ( n >= 2 )
  {
    ...
    {
      char buffer[1024];
      sprintf( buffer, "SetInteractionMode: no images matching %sin network",
               qualifier0 );
      a->sendPacket( DXPacketIF::PKTERROR, id, buffer );
      if ( l )
        delete l;
  ...
  {
    l = theDXApplication->network->makeClassifiedNodeList( ClassImageNode );
  ...
  if ( EqualString( mode, "camera" ) )
    dim = CAMERA;
  else if ( EqualString( mode, "cursors" ) )
    dim = CURSORS;
  else if ( EqualString( mode, "pick" ) )
    dim = PICK;
  else if ( EqualString( mode, "navigate" ) )
    dim = NAVIGATE;
  else if ( EqualString( mode, "panzoom" ) )
    dim = PANZOOM;
  else if ( EqualString( mode, "roam" ) )
    dim = ROAM;
  else if ( EqualString( mode, "rotate" ) )
    dim = ROTATE;
  else if ( EqualString( mode, "zoom" ) )
    dim = ZOOM;
  else
    dim = NONE;
  ...
  if ( ( dim == CURSORS || dim == PICK ) && n >= 3 )
  {
    ...
    {
      char buffer[1024];
      sprintf( buffer, "SetInteractionMode: no %s named %s in network",
    ...
    while ( NULL != ( in = (ImageNode *)it.getNext() ) )
    {
      ...
      {
        char buffer[1024];
        sprintf( buffer, "SetInteractionMode: could not current %s to '%s'",
...
DXLinkHandler::DXLinkHandler( PacketIF *pif ) : LinkHandler( pif )
{
  ...
  this->addSubCommand( "open", "network", DXLinkHandler::OpenNetwork );
  this->addSubCommand( "open", "config", DXLinkHandler::OpenConfig );
  this->addSubCommand( "open", "controlpanel",
                       DXLinkHandler::OpenControlPanel );
  this->addSubCommand( "open", "VPE", DXLinkHandler::OpenVPE );
  this->addSubCommand( "open", "sequencer", DXLinkHandler::OpenSequencer );
  this->addSubCommand( "open", "image", DXLinkHandler::OpenImage );
  ...
  this->addSubCommand( "close", "VPE", DXLinkHandler::CloseVPE );
  this->addSubCommand( "close", "colormapEditor",
                       DXLinkHandler::CloseColormapEditor );
  this->addSubCommand( "close", "image", DXLinkHandler::CloseImage );
  this->addSubCommand( "close", "sequencer", DXLinkHandler::CloseSequencer );
  ...
  this->addSubCommand( "load", "macroFile", DXLinkHandler::LoadMacroFile );
