...
extern "C" void PostScriptImageFormat_ParseSizeCB( Widget, XtPointer,
                                                   XtPointer );
extern "C" void PostScriptImageFormat_ParseSizeEH( Widget, XtPointer, XEvent*,
                                                   Boolean* );
extern "C" void PostScriptImageFormat_PageSizeTO( XtPointer, XtIntervalId* );
extern "C" void PostScriptImageFormat_PageModifyCB( Widget, XtPointer,
                                                    XtPointer );
extern "C" void PostScriptImageFormat_ParsePageSizeCB( Widget, XtPointer,
                                                       XtPointer );
...
class DirtyQueue
{
 ...
 protected:
  ...
  void push( int dirty, int* all_flags )
  {
    if ( dirty <= 0 )
      return;
    if ( this->data == NUL(List*))
      this->data = new List;

    int size = this->data->getSize();
    int top = 0;
    if ( size > 0 )
      top = (int)(long)this->data->getElement( 1 );
    if ( dirty == top )
      return;
...
class PostScriptImageFormat : public ImageFormat
{
 ...
 protected:
  ...
  friend void PostScriptImageFormat_ParseSizeCB( Widget, XtPointer, XtPointer );
  friend void PostScriptImageFormat_ParseSizeEH( Widget, XtPointer, XEvent*,
                                                 Boolean* );
  friend void PostScriptImageFormat_PageSizeTO( XtPointer, XtIntervalId* );
  friend void PostScriptImageFormat_PageModifyCB( Widget, XtPointer,
                                                  XtPointer );
  friend void PostScriptImageFormat_ParsePageSizeCB( Widget, XtPointer,
                                                     XtPointer );
  friend void PostScriptImageFormat_DpiCB( Widget, XtPointer, XtPointer );
 ...
 public:
  ...
  {
    return ClassPostScriptImageFormat;
  }
...
// width of the sheet of paper will limit it.  So you must calculate both dpi
// values
// and use the smaller of the 2.  As long as you're going to so much trouble,
// just
...
class Orientation
{
 ...
 protected:
  ...
  //
  // By default we'll increment dpi if there is any fractional part to it.
  // That's
  ...
  //
  Orientation( int w, int h, double page_w, double page_h, double margin,
               boolean rnd_up = TRUE )
  {
    valid = TRUE;
    ...
    if ( ( divisorw > 0 ) && ( divisorh > 0 ) )
    {
      ...
      if ( ( diff - horizontal_dpi ) >= round_threshold )
        horizontal_dpi++;
      //
      ...
      if ( ( diff - vertical_dpi ) >= round_threshold )
        vertical_dpi++;
      //
...
class RestrictedOrientation : public Orientation
{
 ...
 protected:
  RestrictedOrientation( int w, int h, double page_w, double page_h,
                         double margin, double image_w, double image_h,
                         boolean rnd_up = TRUE )
      : Orientation( w, h, image_w, image_h, 0.0, rnd_up )
  {
    if ( ( page_w - margin ) < image_w )
      valid = FALSE;
    if ( ( page_h - margin ) < image_h )
      valid = FALSE;
    if ( !valid )
      misfit = 9999999;
  }
