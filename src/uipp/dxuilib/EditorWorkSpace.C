...
void EditorWorkSpace::doBackgroundAction( Widget, XtPointer callData )
{
  ...
  this->editor->addCurrentNode( call->event->xbutton.x, call->event->xbutton.y,
                                this );
}

EditorWorkSpace::EditorWorkSpace( const char *name, Widget parent,
                                  WorkSpaceInfo *info, EditorWindow *editor )
    : WorkSpace( name, parent, info ), DXDropSite()
    ...
    boolean fromFirstIntanceOfADerivedClass )
{
  ...
  n = 0;
  XtSetArg( arg[n], XmNbackground, &background );
  n++;
  ...
  this->gc_xor = XtGetGC(
      this->getRootWidget(),
      GCForeground | GCBackground | GCFunction | GCSubwindowMode, &values );

  values.background = background;
  values.foreground = white;
  this->gc =
      XtGetGC( this->getRootWidget(), GCForeground | GCBackground, &values );
  ...
  this->cursor[UP_CURSOR] = XCreateFontCursor( this->display, XC_sb_up_arrow );

  this->cursor[DOWN_CURSOR] =
      XCreateFontCursor( this->display, XC_sb_down_arrow );
}
...
boolean EditorWorkSpace::mergeNetElements( Network *tmpnet, List *tmppanels,
                                           int x, int y )
{
  boolean retval;
  int x2use = x;
  ...
  if ( XmWorkspaceLocationEmpty( this->getRootWidget(), x, y ) )
  {
    ...
    retval =
        this->editor->getNetwork()->mergeNetworks( tmpnet, tmppanels, TRUE );
  }
  else
    retval = False;
...
boolean EditorWorkSpace::decodeDropType( int tag, char *type, XtPointer value,
                                         unsigned long len, int x, int y )
{
  ...
  switch ( tag )
  {
    ...
    // the two names, then do a lookup operation.  This dx must know about both
    // the
...
boolean EditorWorkSpace::compoundTextTransfer( char *, XtPointer value,
                                               unsigned long len, int x, int y )
{
#if COMPOUND_TEXT_TRANSFER
  ...
  {
    if ( EqualString( ds->getNameString(), "Marker" ) )
      break;
  }
  if ( !ds )
  {
  ...
  DecoratorInfo *dnd =
      new DecoratorInfo( this->editor->getNetwork(), (void *)this->editor,
...
void EditorWorkSpace::getSelectedBoundingBox( int *minx, int *miny, int *maxx,
                                              int *maxy )
{
...
// sites don't seem to work properly.  I have tried
// XmDropSiteConfigureStackingOrder,
// and changing XmNdropSiteActivity to XmDROP_SITE_{IN}ACTIVE, but so far the
// only
