...
class ComponentAttributes : public Base
{
 ...
 protected:
  ...
  void setIntegerType()
  {
    this->integerTyped = TRUE;
  }
  void setRealType()
  {
    this->integerTyped = FALSE;
  }
...
#define ROUND( x ) \
  ...
  double getValue()
  {
    return ( this->integerTyped ? ROUND( this->currentValue )
                                : this->currentValue );
  }
  double getDelta()
  {
    return ( this->integerTyped ? ROUND( this->delta ) : this->delta );
  }
  double getMinimum()
  {
    return ( this->integerTyped ? ROUND( this->minimum ) : this->minimum );
  }
  double getMaximum()
  {
    return ( this->integerTyped ? ROUND( this->maximum ) : this->maximum );
  }
