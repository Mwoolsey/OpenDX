...
//
boolean DXDragSource::convert( Network *netw, char *, XtPointer *value,
                               unsigned long *length, long )
{
  FILE *netf = 0;
  FILE *cfgf = 0;
  ...
  if ( !this->createNetFiles( netw, netf, cfgfilename ) )
...
// can't use "r+" on a file that doesn't exist yet.
  ...
  *length = header_len + net_len + cfg_len;
  ...
  //
  strcpy( buf, header );
  if ( fread( &( buf[header_len] ), sizeof( char ), (unsigned int)net_len,
              netf ) != net_len )
  {
