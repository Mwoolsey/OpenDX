...
String SetDecoratorTextDialog::DefaultResources[] = {
    ...
    "*arrowLabel.marginWidth:	0", "*arrowLabel.marginHeight:	0",
    "*arrowLabel.marginTop:		0", "*arrowLabel.marginLeft:	0",
    "*arrowLabel.marginRight:	0", "*arrowLabel.marginBottom:	0",
    "*okButton.labelString:		OK", "*applyButton.labelString:	Apply",
    "*restoreButton.labelString:	Restore",
    "*reformatButton.labelString:	Reformat",
    "*cancelButton.labelString:	Cancel", "*okButton.width:		80",
    ...
    // virtual bindings (usually in ~/.motifbind), then the text widget doesn't
    // handle
    ...
    // - Martin
    "*editorText.translations: #override None<Key>osfActivate: "
    "insert-string(\"\\n\")",
    NULL};
...
//
void SetDecoratorTextDialog::post()
{
  this->Dialog::post();
...
//
//
boolean SetDecoratorTextDialog::restoreCallback( Dialog * )
{
  //
  // text
  //
  if ( this->dirty & DIRTY_TEXT )
  {
    const char *cp = "";
    if ( this->decorator )
      cp = this->decorator->getLabelValue();
    // XtRemoveCallback (this->editorText, XmNmodifyVerifyCallback,
    // (XtCallbackProc)
    //    SetDecoratorTextDialog_DirtyTextCB, (XtPointer)this);
    XmTextSetString( this->editorText, (char *)cp );
    // XtAddCallback (this->editorText, XmNmodifyVerifyCallback,
    // (XtCallbackProc)
    //    SetDecoratorTextDialog_DirtyTextCB, (XtPointer)this);
  ...
  if ( this->dirty & DIRTY_JUSTIFY )
  {
    ...
    boolean isset = FALSE;
    if ( this->decorator )
      isset = this->decorator->isResourceSet( XmNalignment );
    if ( isset )
    {
    ...
    XtVaGetValues( this->justifyPulldown, XmNnumChildren, &nkids, XmNchildren,
                   &kids, NULL );
    ASSERT( nkids > 0 );
    if ( ( align ) && ( align[0] ) )
    {
  ...
  if ( this->dirty & DIRTY_COLORS )
  {
    ...
    boolean isset = FALSE;
    if ( this->decorator )
      isset = this->decorator->isResourceSet( XmNforeground );
    if ( isset )
    {
    ...
    XtVaGetValues( this->colorPulldown, XmNnumChildren, &nkids, XmNchildren,
                   &kids, NULL );
    ASSERT( nkids > 0 );
    if ( ( color ) && ( color[0] ) )
    {
  ...
  if ( this->dirty & DIRTY_FONT )
  {
    ...
    if ( this->decorator )
      font_name = this->decorator->getFont();
    XtVaGetValues( this->fontPulldown, XmNnumChildren, &nkids, XmNchildren,
                   &kids, NULL );
    ASSERT( nkids > 0 );
...
boolean SetDecoratorTextDialog::okCallback( Dialog * )
{
...
Widget SetDecoratorTextDialog::createDialog( Widget parent )
{
  int n = 0;
  Widget form;
  Arg args[20];
  ...
  n++;
  XtSetArg( args[n], XmNwidth, ORIG_MINWIDTH );
  n++;
  XtSetArg( args[n], XmNminWidth, /*290*/ ORIG_MINWIDTH );
  n++;
  ...
  n++;
  XtSetArg( args[n], XmNbottomOffset, 130 );
  n++;
  XtSetArg( args[n], XmNtopOffset, 10 );
  n++;
  XtSetArg( args[n], XmNleftOffset, 6 );
  n++;
  XtSetArg( args[n], XmNrightOffset, ARROW_RIGHTMOST_POS );
  n++;
  ...
  XtAddCallback( this->editorText, XmNmodifyVerifyCallback,
                 (XtCallbackProc)SetDecoratorTextDialog_DirtyTextCB,
                 ( XtPointer ) this );

  n = 0;
  XtSetArg( args[n], XmNtopAttachment, XmATTACH_WIDGET );
  n++;
  XtSetArg( args[n], XmNtopWidget, XtParent( this->editorText ) );
  n++;
  ...
  XtAddEventHandler( this->resize_arrow, Button1MotionMask, False,
                     (XtEventHandler)SetDecoratorTextDialog_ArrowMotionEH,
                     ( XtPointer ) this );
  XtAddCallback( this->resize_arrow, XmNarmCallback,
                 (XtCallbackProc)SetDecoratorTextDialog_StartPosCB,
                 ( XtPointer ) this );
  // XtAddCallback (this->resize_arrow, XmNactivateCallback, (XtCallbackProc)
  //	SetDecoratorTextDialog_EndPosCB, (XtPointer)this);
  XtAddCallback( this->resize_arrow, XmNdisarmCallback,
                 (XtCallbackProc)SetDecoratorTextDialog_EndPosCB,
                 ( XtPointer ) this );
  ...
  XtSetArg( args[n], XmNleftOffset, 6 );
  n++;
  XtSetArg( args[n], XmNsubMenuId, this->createColorMenu( form ) );
  n++;
  ...
  n++;
  XtSetArg( args[n], XmNtopWidget, this->colorOM );
  n++;
  XtSetArg( args[n], XmNtopOffset, 0 );
  n++;
  XtSetArg( args[n], XmNleftAttachment, XmATTACH_WIDGET );
  n++;
  XtSetArg( args[n], XmNleftWidget, this->colorOM );
  n++;
  ...
  XtSetArg( args[n], XmNsubMenuId, this->createFontMenu( form ) );
  n++;
  ...
  XtSetArg( args[n], XmNrightOffset, 6 );
  n++;
  XtSetArg( args[n], XmNsubMenuId, this->createJustifyMenu( form ) );
  n++;
  ...
  XtSetArg( args[n], XmNtopAttachment, XmATTACH_WIDGET );
  n++;
  XtSetArg( args[n], XmNtopWidget, colorOM );
  n++;
  XtSetArg( args[n], XmNtopOffset, 10 );
  n++;
  XtSetArg( args[n], XmNleftAttachment, XmATTACH_FORM );
  ...
  XtSetArg( args[n], XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET );
  n++;
  XtSetArg( args[n], XmNtopWidget, this->ok );
  n++;
  XtSetArg( args[n], XmNtopOffset, 0 );
  n++;
  XtSetArg( args[n], XmNleftAttachment, XmATTACH_WIDGET );
  n++;
  XtSetArg( args[n], XmNleftWidget, this->ok );
  n++;
  XtSetArg( args[n], XmNleftOffset, 10 );
  n++;
  ...
  XtAddCallback( this->apply, XmNactivateCallback,
                 (XtCallbackProc)SetDecoratorTextDialog_ApplyCB,
                 ( XtPointer ) this );
  ...
  XtSetArg( args[n], XmNrightAttachment, XmATTACH_FORM );
  n++;
  XtSetArg( args[n], XmNrightOffset, 12 );
  n++;
  ...
  XtSetArg( args[n], XmNtopAttachment, XmATTACH_OPPOSITE_WIDGET );
  n++;
  XtSetArg( args[n], XmNtopWidget, this->cancel );
  n++;
  XtSetArg( args[n], XmNtopOffset, 0 );
  n++;
  XtSetArg( args[n], XmNrightAttachment, XmATTACH_WIDGET );
  n++;
  XtSetArg( args[n], XmNrightWidget, this->cancel );
  n++;
  ...
  XtAddCallback( this->restore, XmNactivateCallback,
                 (XtCallbackProc)SetDecoratorTextDialog_RestoreCB,
                 ( XtPointer ) this );
  ...
  XtSetArg( args[n], XmNrightAttachment, XmATTACH_WIDGET );
  n++;
  XtSetArg( args[n], XmNrightWidget, this->restore );
  n++;
  XtSetArg( args[n], XmNrightOffset, 10 );
  n++;
  this->reformat = XmCreatePushButton( button_form, "reformatButton", args, n );
  XtManageChild( this->reformat );
  XtAddCallback( this->reformat, XmNactivateCallback,
                 (XtCallbackProc)SetDecoratorTextDialog_ReformatCB,
                 ( XtPointer ) this );
...
//
Widget SetDecoratorTextDialog::createColorMenu( Widget parent )
{
  Widget button;
  ...
  while ( btn_names[i] )
  {
    ...
    n++;
#endif
    XtSetArg( args[n], XmNuserData, color_values[i] );
    n++;
    XtSetArg( args[n], XmNlabelString, xmstr );
    n++;
#if SHOW_IT_NOW
    ...
    XtAddCallback( button, XmNactivateCallback,
                   (XtCallbackProc)SetDecoratorTextDialog_DirtyColorsCB,
                   ( XtPointer ) this );
...
Widget SetDecoratorTextDialog::createFontMenu( Widget parent )
{
  Widget button;
  Arg args[9];
  int i, n;
...
#endif
      char *btn_names[] = {
          "bold 12 point",   "bold 14 point",   "bold 18 point",
          "bold 24 point",   "italic 12 point", "italic 14 point",
          "italic 18 point", "italic 24 point", "medium 12 point",
          "medium 14 point", "medium 18 point", "medium 24 point",
          DEFAULT_SETTING};
...
// These names must match those used for fontList in IMBApplication.C
  ...
  for ( i = 0; i < XtNumber( btn_names ); i++ )
  {
    ...
    n = 0;
    XtSetArg( args[n], XmNuserData, font_names[i] );
    n++;
#if SHOW_IT_NOW
    XtSetArg( args[n], XmNlabelString, xmstr );
    n++;
    ...
    XtManageChild( button );
    XtAddCallback( button, XmNactivateCallback,
                   (XtCallbackProc)SetDecoratorTextDialog_DirtyFontCB,
                   ( XtPointer ) this );
...
Widget SetDecoratorTextDialog::createJustifyMenu( Widget parent )
{
  Widget button;
  Arg args[9];
  int i, n;
  ...
  static
#endif
      char *btn_names[] = {"Left", "Center", "Right", DEFAULT_SETTING};
  ...
  n = 0;
  this->justifyPulldown =
      XmCreatePulldownMenu( parent, "pulldownMenu", args, n );

  for ( i = 0; i < XtNumber( btn_names ); i++ )
  {
    n = 0;
    XtSetArg( args[n], XmNuserData, align_names[i] );
    n++;
    button = XmCreatePushButtonGadget( this->justifyPulldown, btn_names[i],
                                       args, n );
    XtManageChild( button );
    XtAddCallback( button, XmNactivateCallback,
                   (XtCallbackProc)SetDecoratorTextDialog_DirtyJustifyCB,
                   ( XtPointer ) this );
  }
...
const char *SetDecoratorTextDialog::defaultJustifySetting()
{
...
void SetDecoratorTextDialog_DirtyColorsCB( Widget, XtPointer cdata, XtPointer )
{
...
void SetDecoratorTextDialog_DirtyJustifyCB( Widget, XtPointer cdata, XtPointer )
{
  SetDecoratorTextDialog *sdt = (SetDecoratorTextDialog *)cdata;
...
//
  ...
  XtVaSetValues( sdt->resize_arrow_label, XmNmappedWhenManaged, (Boolean)enab,
                 NULL );
}

void SetDecoratorTextDialog_DirtyTextCB( Widget w, XtPointer cdata,
                                         XtPointer cbs )
{
  SetDecoratorTextDialog *sdt = (SetDecoratorTextDialog *)cdata;
  ...
  else if ( disable_it )
  {
    int nkids;
    Widget *kids;
    XtVaGetValues( sdt->justifyPulldown, XmNnumChildren, &nkids, XmNchildren,
                   &kids, NULL );
    ASSERT( nkids > 0 );
...
void SetDecoratorTextDialog_DirtyFontCB( Widget, XtPointer cdata, XtPointer )
{
  SetDecoratorTextDialog *sdt = (SetDecoratorTextDialog *)cdata;
...
void SetDecoratorTextDialog_ApplyCB( Widget, XtPointer cdata, XtPointer )
{
  SetDecoratorTextDialog *sdt = (SetDecoratorTextDialog *)cdata;
...
void SetDecoratorTextDialog_RestoreCB( Widget, XtPointer cdata, XtPointer )
{
  SetDecoratorTextDialog *sdt = (SetDecoratorTextDialog *)cdata;
...
void SetDecoratorTextDialog_ReformatCB( Widget, XtPointer cdata, XtPointer )
{
  SetDecoratorTextDialog *sdt = (SetDecoratorTextDialog *)cdata;
...
void SetDecoratorTextDialog_ArrowMotionEH( Widget w, XtPointer cdata,
                                           XEvent *xev, Boolean *keep_going )
{
  SetDecoratorTextDialog *sdt = (SetDecoratorTextDialog *)cdata;
  ...
  *keep_going = True;
  ...
  if ( ( newofs > 500 ) && ( sdt->start_pos >= 500 ) )
    return;
...
void SetDecoratorTextDialog_StartPosCB( Widget, XtPointer cdata, XtPointer cbs )
{
  SetDecoratorTextDialog *sdt = (SetDecoratorTextDialog *)cdata;
...
void SetDecoratorTextDialog_EndPosCB( Widget, XtPointer cdata, XtPointer )
{
  SetDecoratorTextDialog *sdt = (SetDecoratorTextDialog *)cdata;

  ASSERT( sdt );
  XtVaSetValues( XtParent( sdt->editor_magic ), XmNrightOffset, sdt->start_pos,
                 NULL );
  sdt->reformatCallback( sdt );
}
...
void SetDecoratorTextDialog::installDefaultResources( Widget baseWidget )
{
  this->setDefaultResources( baseWidget,
                             SetDecoratorTextDialog::DefaultResources );
...
// - When backing up, look for only a space.  This is the normal case.  If there
// is
//   no space, then settle for ispunct()==1 while scanning ahead.
//
// This doesn't account for variable width fonts.  The value in XmNcolumns is
// just
// an estimate of the number of chars we can accommodate.  There is in fact no
// such
// number.  In order to do this correctly, we would have to count the pixels
// used
// by each char on the line.  Motif supplies a mechanism for that sort of
// behavior.
// It's what you get when you set XmNwordWrap==True.  Unfortunately, there isn't
// any way to query for the results of Motif's wordwrapping.  If there were,
// then
...
#define PREFER_BACKWARDS -( HOW_FAR_TO_LOOK + 4 )
      ...
      {
        int k = i;
        while ( ( spare[k] ) && ( to_next_space < HOW_FAR_TO_LOOK ) &&
      ...
      if ( ( to_next_space < HOW_FAR_TO_LOOK ) && ( most_recent_space > 0 ) &&
           ( regress <= PREFER_BACKWARDS ) )
      {
      }
...
#undef PREFER_BACKWARDS
      ...
      //
      while ( ( spare[i] ) && ( IsSpace( spare[i] ) == FALSE ) &&
...
char *SetDecoratorTextDialog::kern( char *src, char *font )
{
  ...
  for ( i = 0; i < this->line_count; i++ )
  {
    ...
    if ( pixel_diff == 0 )
      continue;
    if ( pixel_diff > threshhold )
      continue;
    if ( pixel_diff > ( this->kern_lengths[i] >> 1 ) )
      continue;
...
char *SetDecoratorTextDialog::Pad( char *src, int pad, int every_nth )
{
  ...
  if ( pads_added == 0 )
    ...
    return SetDecoratorTextDialog::Pad( newsrc, remaining,
                                        ( every_nth ? every_nth - 1 : 0 ) );
}
...
boolean SetDecoratorTextDialog::measureLines( XmString srcString, char *font )
{
  ...
  while ( 1 )
  {
    ...
    if ( retVal == XmSTRING_COMPONENT_END )
      break;
    switch ( retVal )
    {
      ...
      case XmSTRING_COMPONENT_LOCALE_TEXT:
        ...
        len = strlen( text );
        label_buf = this->kern_lines[next_kern_line++] = new char[len + 2];
      ...
      case XmSTRING_COMPONENT_SEPARATOR:
        ...
        if ( !label_buf )
        {
          label_buf = this->kern_lines[next_kern_line++] = new char[8];
        }
        label_buf[os++] = '\n';
        label_buf[os] = '\0';
        os = 0;
