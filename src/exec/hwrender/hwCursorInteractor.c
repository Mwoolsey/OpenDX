...
static void constrain( tdmInteractor I, double *s0, double *s1, double *s2,
                       double dx, double dy, double dz, double WItrans[4][4],
                       double Wtrans[4][4], int sdx, int sdy );

static void project( tdmInteractor I, double s0, double s1, double s2,
                     double Wtrans[4][4], double WItrans[4][4] );

static double line_of_sight( tdmInteractor I, double s0, double s1,
                             int *newbox );

static void CallCursorCallbacks( tdmInteractor I, int reason, int cursor_num,
                                 double screen_x, double screen_y,
                                 double screen_z, tdmInteractorReturn *R );
...
tdmInteractor _dxfCreateCursorInteractor( tdmInteractorWin W, tdmCursorType C )
{
  ...
  if ( W && ( I = _dxfAllocateInteractor( W, sizeof( CursorData ) ) ) )
  {
    ...
    if ( !( PDATA( coord_saveunder ) = _dxf_ALLOCATE_PIXEL_ARRAY(
                PORT_CTX, COORD_WIDTH, COORD_HEIGHT ) ) )
      goto error;

    /* allocate a buffer to store bounding box */
    if ( !( PDATA( basis ) =
                (XmBasis *)tdmAllocateLocal( sizeof( XmBasis ) ) ) )
      goto error;
...
static void DestroyCursorInteractor( tdmInteractor I )
{
  DEFDATA( I, CursorData );
  DEFPORT( I_PORT_HANDLE );
...
static void ResetRoamPoint( tdmInteractor I )
{
  double x, y, z, nx, ny;
  DEFDATA( I, CursorData );
...
void _dxfCursorInteractorChange( tdmInteractor I, int id, int reason, float x,
                                 float y, float z )
{
  /*
  ...
  ENTRY( ( "_dxfCursorInteractorChange(0x%x, %d, %d, %f, %f, %f)", I, id,
           reason, x, y, z ) );
  ...
  if ( I )
  {
    ...
    PRINT( ( "%s mode",
             ( ( PDATA( mode ) == XmCURSOR_MODE ) ? "cursor" : "roam" ) ) );
...
static void Redisplay( tdmInteractor I, tdmInteractorRedrawMode redrawMode )
{
  DEFDATA( I, CursorData );
  DEFPORT( I_PORT_HANDLE );
...
static void Config( tdmInteractor I, tdmInteractorRedrawMode redrawMode )
{
  DEFDATA( I, CursorData );
  DEFPORT( I_PORT_HANDLE );
  ...
  _dxf_BUFFER_CONFIG( PORT_CTX, CDATA( image ), 0, CDATA( h ) - CDATA( ih ),
                      CDATA( iw ) - 1, CDATA( h ) - 1, &PDATA( displaymode ),
                      &PDATA( buffermode ), redrawMode );
  ...
  /* update from changes */
  if ( PDATA( box_change ) || PDATA( box_state ) != CDATA( box_state ) ||
...
static void RestoreConfig( tdmInteractor I, tdmInteractorRedrawMode redrawMode )
{
  float M[4][4];
  ...
  _dxf_BUFFER_RESTORE_CONFIG( PORT_CTX, PDATA( displaymode ),
                              PDATA( buffermode ), redrawMode );
...
static void restore_image( tdmInteractor I )
{
  ...
  if ( CDATA( image ) )
  {
    ...
    _dxf_WRITE_BUFFER( PORT_CTX, 0, 0, CDATA( iw ) - 1, CDATA( ih ) - 1,
                       CDATA( image ) );
  }
...
static void PickStart( tdmInteractor I, int x, int y, int btn, int s )
{
  DEFDATA( I, CursorData );
  DEFPORT( I_PORT_HANDLE );
  ...
  if ( !CLIPPED( x, y ) )
  {
    Config( I, tdmBothBufferDraw );
    _dxf_WRITE_BUFFER(
        PORT_CTX, x - PDATA( cursor_size ) / 2, y - PDATA( cursor_size ) / 2,
        x + PDATA( cursor_size ) / 2, y + PDATA( cursor_size ) / 2,
...
static void PickEnd( tdmInteractor I, tdmInteractorReturnP R )
{
  DEFDATA( I, CursorData );
  DEFPORT( I_PORT_HANDLE );
...
static void Select( tdmInteractor I, int x, int y, int btn, int s )
{
  DEFDATA( I, CursorData );
  DEFPORT( I_PORT_HANDLE );
  ...
  draw_cursors( I );
  project( I, (double)x, (double)y, (double)PDATA( Z ), PDATA( Wtrans ),
           PDATA( WItrans ) );

  _dxf_DRAW_MARKER( I );
  CallCursorCallbacks( I, tdmCURSOR_SELECT, id, PDATA( X ), PDATA( Y ),
                       PDATA( Z ), &R );

  EXIT( ( "" ) );
}

static void BtnMotion( tdmInteractor I, int x, int y, int t, int s )
{
  /*
  ...
  if ( !PDATA( x_movement_allowed ) )
    dx = 0.0;
  if ( !PDATA( y_movement_allowed ) )
    dy = 0.0;
  if ( !PDATA( z_movement_allowed ) )
    dz = 0.0;
  ...
  constrain( I, &PDATA( X ), &PDATA( Y ), &PDATA( Z ), dx, dy, dz,
             PDATA( WItrans ), PDATA( Wtrans ), x - PDATA( px ),
             y - PDATA( py ) );
  ...
  /* digital feedback */
  CallCursorCallbacks( I, tdmCURSOR_DRAG, id, PDATA( X ), PDATA( Y ),
                       PDATA( Z ), &R );
  add2historybuffer( I, x, y );
...
static void Deselect( tdmInteractor I, tdmInteractorReturn *R )
{
  /*
  ...
  if ( PDATA( mode ) == XmCURSOR_MODE )
  {
    ...
    for ( i = 0; i < PDATA( n_cursors ); i++ )
    {
      if ( PDATA( selected )[i] == TRUE )
      {
        ...
        {
          /* warp pointer back */
          _dxf_WARP_POINTER( PORT_CTX, CDATA( ox ) + PDATA( xbuff[i] ),
    ...
    else
    {
      CallCursorCallbacks( I, tdmCURSOR_MOVE, PDATA( cursor_num ),
                           ...
                           PDATA( zbuff[PDATA( cursor_num )] ), R );
    }
  }
  else if ( PDATA( mode ) == XmROAM_MODE )
  {
    ...
    {
      /* warp pointer back */
      _dxf_WARP_POINTER( PORT_CTX, CDATA( ox ) + PDATA( roam_xbuff ),
    ...
    {
      CallCursorCallbacks( I, tdmCURSOR_MOVE, 0, (double)PDATA( roam_xbuff ),
                           (double)PDATA( roam_ybuff ), PDATA( roam_zbuff ),
                           R );
    }
  }
...
static void CreateDelete( tdmInteractor I, int x, int y,
                          tdmInteractorReturn *R )
{
  /*
  ...
  if ( PDATA( mode ) == XmCURSOR_MODE )
  {
    ...
    CallCursorCallbacks( I, tdmCURSOR_CREATE, id, PDATA( X ), PDATA( Y ),
                         PDATA( Z ), R );
  }
  else if ( PDATA( mode ) == XmROAM_MODE )
  {
    ...
    CallCursorCallbacks( I, tdmCURSOR_MOVE, id, PDATA( X ), PDATA( Y ),
                         PDATA( Z ), R );
  }
  ...
  {                                                                        \
    ...
    {                                                                      \
      ...
      x2 = x1 + t * dv[0];                                                 \
                                                                           ...
                                                                           \
    if ( x2 > ( CDATA( w ) - 1 ) || x2 < 0 || y2 > ( CDATA( h ) - 1 ) ||   \
         y2 < 0 )                                                          \
    {                                                                      \
...
static void draw_box_line( tdmInteractor I, struct pnt p0, struct pnt p1,
                           int moveOK, int constraint_axis )
{
  ...
  ENTRY( ( "draw_box_line(0x%x, 0x%x, 0x%x, %d, %d)", I, &p0, &p1, moveOK,
           constraint_axis ) );
  ...
  if ( p0.pwcoor > 0.0 || p1.pwcoor > 0.0 )
  {
    ...
    _dxf_DRAW_LINE( PORT_CTX, (int)p0.sxcoor, (int)p0.sycoor, (int)p1.sxcoor,
                    (int)p1.sycoor );
  }
                                 ...
                                 double screen_z, tdmInteractorReturn *R )
{
  ...
  ENTRY( ( "CallCursorCallbacks(0x%x, %d, %d, %f, %f, %f, 0x%x)", I, reason,
           cursor_num, screen_x, screen_y, screen_z, R ) );
  ...
  XFORM_COORDS( PDATA( PureWItrans ), screen_x, screen_y, screen_z, R->x, R->y,
                R->z );
  ...
  _dxf_WRITE_BUFFER( PORT_CTX, PDATA( coord_llx ), PDATA( coord_lly ),
                     PDATA( coord_urx ), PDATA( coord_ury ),
...
static int create_cursor( tdmInteractor I )
{
  ...
  dtmp = (double *)tdmAllocateLocal( ( PDATA( n_cursors ) + 1 ) *
                                     sizeof( double ) );
  for ( i = 0; i < PDATA( n_cursors ); i++ )
  {
...
static void save_cursors_in_canonical_form( tdmInteractor I, int i )
{
  ...
  if ( PDATA( mode ) == XmCURSOR_MODE )
  {
    XFORM_COORDS( PDATA( PureWItrans ), PDATA( xbuff[i] ), PDATA( ybuff[i] ),
                  PDATA( zbuff[i] ), PDATA( cxbuff[i] ), PDATA( cybuff[i] ),
                  PDATA( czbuff[i] ) );
  }
  else if ( PDATA( mode ) == XmROAM_MODE )
  {
    XFORM_COORDS( PDATA( PureWItrans ), PDATA( roam_xbuff ),
                  PDATA( roam_ybuff ), PDATA( roam_zbuff ),
                  PDATA( roam_cxbuff ), PDATA( roam_cybuff ),
                  PDATA( roam_czbuff ) );
  }
...
static void restore_cursors_from_canonical_form( tdmInteractor I )
{
  ...
  for ( i = 0; i < PDATA( n_cursors ); i++ )
  {
    XFORM_COORDS( PDATA( PureWtrans ), PDATA( cxbuff[i] ), PDATA( cybuff[i] ),
                  PDATA( czbuff[i] ), x, y, PDATA( zbuff[i] ) );

    PDATA( xbuff )[i] = (int)x;
    PDATA( ybuff )[i] = (int)y;
  }

  XFORM_COORDS( PDATA( PureWtrans ), PDATA( roam_cxbuff ), PDATA( roam_cybuff ),
                PDATA( roam_czbuff ), x, y, PDATA( roam_zbuff ) );
...
#include <gl.h>
  ...
  if ( lx != 0.0 )
  {
    ...
    if ( ( yt >= 0.0 ) && ( yt <= PDATA( FacetYmax ) ) && ( zt >= 0.0 ) &&
         ( zt <= PDATA( FacetZmax ) ) )
  ...
  if ( ly != 0.0 )
  {
    ...
    if ( ( xt >= 0.0 ) && ( xt <= PDATA( FacetXmax ) ) && ( zt >= 0.0 ) &&
         ( zt <= PDATA( FacetZmax ) ) )
  ...
  if ( lz != 0.0 )
  {
    ...
    if ( ( xt >= 0.0 ) && ( xt <= PDATA( FacetXmax ) ) && ( yt >= 0.0 ) &&
         ( yt <= PDATA( FacetYmax ) ) )
                     ...
                     double Wtrans[4][4], double WItrans[4][4] )
{
  ...
  ENTRY( ( "project(0x%x, %f, %f, %f, 0x%x, 0x%x)", I, s0, s1, s2, Wtrans,
           WItrans ) );
  ...
  {
    i++;
    XFORM_COORDS( PDATA( Wtrans ), x, 0, z, PDATA( Xmark[i] ),
                  PDATA( Ymark[i] ), PDATA( Zmark[i] ) );
  }
  ...
  {
    i++;
    XFORM_COORDS( PDATA( Wtrans ), 0, y, z, PDATA( Xmark[i] ),
                  PDATA( Ymark[i] ), PDATA( Zmark[i] ) );
  }
  ...
  {
    i++;
    XFORM_COORDS( PDATA( Wtrans ), x, y, 0, PDATA( Xmark[i] ),
                  PDATA( Ymark[i] ), PDATA( Zmark[i] ) );
  }
  PDATA( iMark ) = i + 1;
...
#define TRANSFORM4D( M, x, y, z, px, py, pz, pw, tx, ty, tz, sx, sy )       \
  {                                                                         \
    ...
    if ( tz < PDATA( Zmin ) )                                               \
      PDATA( Zmin ) = tz;                                                   \
                                                                            \
    PRINT( ( "%15f %15f %15f --> %15f %15f %15f %15f", x, y, z, tx, ty, tx, \
             pw ) );                                                        \
  }
...
static void setup_bounding_box( tdmInteractor I )
{
  ...
  for ( i = 0; i < 4; i++ )
  {
    TRANSFORM4D( PDATA( Wtrans ), PDATA( Face1[i].xcoor ),
                 PDATA( Face1[i].ycoor ), PDATA( Face1[i].zcoor ),
                 PDATA( Face1[i].pxcoor ), PDATA( Face1[i].pycoor ),
                 PDATA( Face1[i].pzcoor ), PDATA( Face1[i].pwcoor ),
                 PDATA( Face1[i].txcoor ), PDATA( Face1[i].tycoor ),
                 PDATA( Face1[i].tzcoor ), PDATA( Face1[i].sxcoor ),
  ...
  for ( i = 0; i < 4; i++ )
  {
    TRANSFORM4D( PDATA( Wtrans ), PDATA( Face2[i].xcoor ),
                 PDATA( Face2[i].ycoor ), PDATA( Face2[i].zcoor ),
                 PDATA( Face2[i].pxcoor ), PDATA( Face2[i].pycoor ),
                 PDATA( Face2[i].pzcoor ), PDATA( Face2[i].pwcoor ),
                 PDATA( Face2[i].txcoor ), PDATA( Face2[i].tycoor ),
                 PDATA( Face2[i].tzcoor ), PDATA( Face2[i].sxcoor ),
  ...
  for ( i = 0; i < 4; i++ )
  {
    ...
    XFORM_COORDS( PDATA( WItrans ), PDATA( Face1[i].txcoor ),
                  PDATA( Face1[i].tycoor ), nz, x, y, z );
    ...
    XFORM_COORDS( PDATA( WItrans ), PDATA( Face2[i].txcoor ),
                  PDATA( Face2[i].tycoor ), nz, x, y, z );
