...
int tdmMeshDraw( tdmPortHandleP portHandle, xfieldT *xf, int buttonUp )
{
  Point *points = 0;
  struct p2d
  {
  ...
  xgl_object_set(
      XGLCTX, XGL_3D_CTX_SURF_FACE_DISTINGUISH, FALSE,
      ...
      XGL_3D_CTX_LINE_COLOR_INTERP, TRUE, NULL );
  ...
  if ( xf->colorsDep == dep_field )
  {
    ...
    if ( approx == approx_lines )
      ...
      xgl_object_set( XGLCTX, XGL_CTX_LINE_COLOR_SELECTOR,
                      XGL_LINE_COLOR_CONTEXT, XGL_CTX_LINE_COLOR, &fcolors[0],
                      NULL );
    else if ( normals )
      /* get surface color from context, turn on lighting */
      xgl_object_set( XGLCTX, XGL_3D_CTX_SURF_FRONT_ILLUMINATION,
                      XGL_ILLUM_PER_VERTEX, XGL_CTX_SURF_FRONT_COLOR_SELECTOR,
                      XGL_SURF_COLOR_CONTEXT, XGL_CTX_SURF_FRONT_COLOR,
                      &fcolors[0], NULL );
    else
      /* get surface color from context, no lighting */
      xgl_object_set( XGLCTX, XGL_CTX_SURF_FRONT_COLOR_SELECTOR,
                      XGL_SURF_COLOR_CONTEXT, XGL_CTX_SURF_FRONT_COLOR,
                      &fcolors[0], NULL );
  }
  else
  ...
  if ( opacities && USE_SCREEN_DOOR )
    if ( ( opacity_map ? opacity_map[*(char *)opacities] : opacities[0] ) <
         0.75 )
  ...
  else
  {
    ...
    if ( facet_flags )
    {
      xgl_facet_list = stripsXGL->facet_lists =
          (Xgl_facet_list *)tdmAllocate( numStrips * sizeof( Xgl_facet_list ) );
    ...
    for ( ; stripsXGL->num_strips < numStrips; stripsXGL->num_strips++ )
    {
      ...
      if ( facet_flags )
      {
        xgl_facet_list->facets.color_normal_facets =
            (Xgl_color_normal_facet *)( flist = (float *)tdmAllocate(
                                            ( numPnts - 2 ) * fsize *
                                            sizeof( float ) ) );
      ...
      if ( facet_flags & TDM_FACET_COLOR )
        if ( color_map )
          for ( i = 0, dV = f_cOffs; i < numPnts - 2; i++, dV += fsize )
            CLAMP( ( flist + dV ), &color_map[( (char *)fcolors )[pntIdx[i]]] );
        else
          for ( i = 0, dV = f_cOffs; i < numPnts - 2; i++, dV += fsize )
