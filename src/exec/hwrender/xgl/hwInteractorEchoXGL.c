...
void _dxf_BUFFER_CONFIG( void *ctx, void *image, int llx, int lly, int urx,
                         int ury, int *CurrentDisplayMode,
                         ...
                         tdmInteractorRedrawMode redrawMode )
{
  ...
  *CurrentBufferMode = BUFFER_CONFIG_MODE;
  ...
  {
    DPRINT1( "\n%s", redrawMode == tdmBothBufferDraw ? "tdmBothBufferDraw"
                                                     : "tdmFrontBufferDraw" );
    /*
    ...
    if ( DISPLAY_BUFFER != DRAW_BUFFER )
      if ( DISPLAY_BUFFER == 1 )
      {
        xgl_object_set( XGLRAS, XGL_WIN_RAS_BUF_DRAW, 1,
                        XGL_WIN_RAS_BUF_DISPLAY, 1, NULL );
        DRAW_BUFFER = 1;
      }
      else
      {
        xgl_object_set( XGLRAS, XGL_WIN_RAS_BUF_DRAW, 0,
  ...
  else if ( redrawMode == tdmBackBufferDraw || redrawMode == tdmViewEchoMode )
  {
    DPRINT1( "\n%s", redrawMode == tdmViewEchoMode ? "tdmViewEchoMode"
                                                   : "tdmBackBufferDraw" );
    /*
...
void _dxf_BUFFER_RESTORE_CONFIG( void *ctx, int OrigDisplayMode,
                                 int OrigBufferMode,
                                 tdmInteractorRedrawMode redrawMode )
{
  /*
  ...
  if ( redrawMode == tdmBothBufferDraw || redrawMode == tdmFrontBufferDraw )
  {
    DPRINT1( "\ncurrent interactor redraw mode is %s",
             redrawMode == tdmFrontBufferDraw ? "tdmFrontBufferDraw"
                                              : "tdmBothBufferDraw" );
  ...
  {
    DPRINT1( "\ncurrent interactor redraw mode is %s",
             redrawMode == tdmViewEchoMode ? "tdmViewEchoMode"
                                           : "tdmBackBufferDraw" );
...
void _dxf_READ_BUFFER( void *ctx, int llx, int lly, int urx, int ury,
                       void *buff )
{
  /*
  ...
  rect.xmin = llx;
  rect.xmax = urx;
  rect.ymin = VP_YMAX - ury;
  rect.ymax = VP_YMAX - lly;
...
#endif
  else if ( DISPLAY_BUFFER == 1 )
  {
    /* set draw buffer to 1 so we can read it, but don't swap display */
    xgl_object_set( XGLRAS, XGL_WIN_RAS_BUF_DRAW, 1, XGL_WIN_RAS_BUF_DISPLAY, 1,
                    NULL );
...
/* read pixels and reset original buffer assignments */
    ...
    xgl_context_copy_raster( XGLTMPCTX, &rect, NULL, XGLRAS );
#endif
    xgl_object_set( XGLRAS, XGL_WIN_RAS_BUF_DRAW, 0, XGL_WIN_RAS_BUF_DISPLAY, 1,
                    NULL );
  }
  else
  {
...
void _dxf_WRITE_BUFFER( void *ctx, int llx, int lly, int urx, int ury,
                        void *buff )
{
  /*
...
void _dxf_DRAW_GLOBE( tdmInteractor I, void *udata, float rot[4][4], int draw )
{
  /*
...
#define FACEVISIBLE( u, v, z0, z1, z2 )                           \
  ( Globeface[u][v].norm[0] * z0 + Globeface[u][v].norm[1] * z1 + \
        Globeface[u][v].norm[2] * z2 >                            \
    0.0 )
...
#define DRAW( vertex )            \
  ...
  else
  {
    if ( PDATA( redrawmode ) != tdmViewEchoMode )
    {
      ...
      _dxf_BUFFER_CONFIG( PORT_CTX, &dummy, dummy, dummy, dummy, dummy, &dummy,
                          &dummy, tdmBackBufferDraw );
  ...
  if ( PDATA( redrawmode ) != tdmViewEchoMode )
  {
    ...
    position.x = PDATA( illx );
    position.y = VP_YMAX - PDATA( iury );
    rect.xmin = PDATA( illx );
    rect.ymin = VP_YMAX - PDATA( iury );
    rect.xmax = PDATA( iurx );
    rect.ymax = VP_YMAX - PDATA( illy );
...
#ifdef solaris
    xgl_object_set( XGLRAS, XGL_RAS_SOURCE_BUFFER, DRAW_BUFFER, NULL );

    /* Set XGLCTX output dest to XGL_RENDER_DISPLAY_BUFFER
       ( == DISPLAY_BUFFER) */
    xgl_object_set( XGLCTX, XGL_CTX_RENDER_BUFFER, XGL_RENDER_DISPLAY_BUFFER,
                    NULL );
    xgl_context_copy_buffer( XGLCTX, &rect, &position, XGLRAS );

    /* Reset XGLCTX output dest to default */
    xgl_object_set( XGLCTX, XGL_CTX_RENDER_BUFFER, XGL_RENDER_DRAW_BUFFER,
                    NULL );
#else
    xgl_context_copy_buffer( XGLCTX, &rect, &position, DRAW_BUFFER,
                             DISPLAY_BUFFER );
#endif

    _dxf_BUFFER_RESTORE_CONFIG( PORT_CTX, dummy, PDATA( buffermode ),
                                tdmBackBufferDraw );
  }
  DPRINT( ")" );
}

void _dxf_DRAW_ARROWHEAD( void *ctx, float ax, float ay )
{
  DEFCONTEXT( ctx );
  Xgl_pt_list line;
  Xgl_pt_f3d verts[3];
...
void _dxf_DRAW_GNOMON( tdmInteractor I, void *udata, float rot[4][4], int draw )
{
  /*
  ...
  if ( draw )
  {
    ...
    color.rgb.g = 1.0;
    color.rgb.b = 1.0;
    xgl_object_set( XGLCTX, XGL_CTX_LINE_COLOR, &color,
#ifdef solaris
  ...
  xlabel[0] = 0.8 * rot[0][0];
  xlabel[1] = 0.8 * rot[0][1];
  ylabel[0] = 0.8 * rot[1][0];
  ylabel[1] = 0.8 * rot[1][1];
  zlabel[0] = 0.8 * rot[2][0];
  zlabel[1] = 0.8 * rot[2][1];
...
static void _dxf_captureZoomBox( tdmInteractor I, void *udata, float rot[4][4] )
{
  ...
  /* left */
  rect.xmin = MAX( 0, x1 - 1 );
  rect.xmax = rect.xmin + 1;
  rect.ymin = MAX( 0, VP_YMAX - y2 );
  rect.ymax = MAX( 0, VP_YMAX - y1 );
...
static void _dxf_restoreZoomBox( tdmInteractor I, void *udata, float rot[4][4] )
{
  DEFDATA( I, tdmZoomData );
  DEFPORT( I_PORT_HANDLE );
  Xgl_bounds_i2d rect;
...
void _dxf_DRAW_ZOOMBOX( tdmInteractor I, void *udata, float rot[4][4],
                        int draw )
{
  /*
...
int _dxf_GET_ZBUFFER_STATUS( void *ctx )
{
  /*
...
void _dxf_SET_ZBUFFER_STATUS( void *ctx, int status )
{
  /*
...
void _dxf_SET_SOLID_FILL_PATTERN( void *ctx )
{
  /*
  ...
  xgl_object_set( XGLCTX, XGL_CTX_SURF_FRONT_FILL_STYLE, XGL_SURF_FILL_SOLID,
                  NULL );

  DPRINT( ")" );
}

void _dxf_GET_WINDOW_ORIGIN( void *ctx, int *x, int *y )
{
  /*
...
void _dxf_GET_MAXSCREEN( void *ctx, int *x, int *y )
{
  /*
...
void _dxf_SET_WORLD_SCREEN( void *ctx, int w, int h )
{
  /*
...
void _dxf_CREATE_CURSOR_PIXMAPS( tdmInteractor I )
{
  /*
  ...
  if ( !( PDATA( PassiveSquareCursor ) = _dxf_ALLOCATE_PIXEL_ARRAY(
              PORT_CTX, PDATA( cursor_size ), PDATA( cursor_size ) ) ) )
    goto error;

  /* set pixel array as rendering surface for temporary XGL context */
  xgl_object_set( XGLTMPCTX, XGL_CTX_DEVICE, PDATA( PassiveSquareCursor ),
                  NULL );
  ...
  if ( !( PDATA( ProjectionMark ) =
              _dxf_ALLOCATE_PIXEL_ARRAY( PORT_CTX, 3, 3 ) ) )
    goto error;

  /* set pixel array as rendering surface for temporary XGL context */
  xgl_object_set( XGLTMPCTX, XGL_CTX_DEVICE, PDATA( ProjectionMark ), NULL );
...
void _dxf_DRAW_MARKER( tdmInteractor I )
{
  /*
...
void _dxf_ERASE_PREVIOUS_MARKS( tdmInteractor I )
{
  DEFDATA( I, CursorData );
  DEFPORT( I_PORT_HANDLE );
  Xgl_pt_i2d position;
  ...
  for ( i = 0; i < PDATA( piMark ); i++ )
  {
    ...
    {
      rect.xmin = mx - 1;
      rect.ymin = VP_YMAX - ( my + 1 );
      rect.xmax = mx + 1;
      rect.ymax = VP_YMAX - ( my - 1 );
      position.x = rect.xmin;
      position.y = rect.ymin;
#ifdef solaris
      xgl_context_copy_buffer( XGLCTX, &rect, &position, CDATA( image ) );
...
void _dxf_ERASE_CURSOR( tdmInteractor I, int x, int y )
{
  /*
  ...
  if ( !PDATA( visible ) || !CDATA( image ) || CLIPPED( x, y ) )
    return;

  s2 = PDATA( cursor_size ) / 2;
  rect.xmin = x - s2;
  rect.ymin = VP_YMAX - ( y + s2 );
  rect.xmax = x + s2;
  rect.ymax = VP_YMAX - ( y - s2 );
  position.x = rect.xmin;
  position.y = rect.ymin;
#ifdef solaris
  xgl_context_copy_buffer( XGLCTX, &rect, &position, CDATA( image ) );
...
void _dxf_POINTER_INVISIBLE( void *ctx )
{
  /*
  ...
  p1 = XCreatePixmapFromBitmapData( XGLDISPLAY,
                                    RootWindow( XGLDISPLAY, XGLSCREEN ),
                                    tmp_bits, 4, 4, 0, 0, 1 );
  p2 = XCreatePixmapFromBitmapData( XGLDISPLAY,
                                    RootWindow( XGLDISPLAY, XGLSCREEN ),
                                    tmp_bits, 4, 4, 0, 0, 1 );
...
void _dxf_POINTER_VISIBLE( void *ctx )
{
  /*
...
void _dxf_WARP_POINTER( void *ctx, int x, int y )
{
  /*
...
static void _dxf_SET_LINE_ATTRIBUTES( void *ctx, int32 color, int style,
                                      float width )
{
  DEFCONTEXT( ctx );
  Xgl_color xgl_color;
...
void _dxf_SET_LINE_COLOR_WHITE( void *ctx )
{
...
void _dxf_SET_LINE_COLOR_GRAY( void *ctx )
{
  /* obsolete entry:  do not use in new code */
  DEFCONTEXT( ctx );
  Xgl_color color;
...
void _dxf_DRAW_LINE( void *ctx, int x1, int y1, int x2, int y2 )
{
  /*
   *  Used for cursor box.
   */
...
void _dxf_DRAW_CURSOR_COORDS( tdmInteractor I, char *text )
{
  /*
...
#ifdef solaris
  xgl_object_set( XGLCTX, XGL_CTX_ATEXT_CHAR_HEIGHT, 0.036,
                  XGL_CTX_STEXT_CHAR_EXPANSION_FACTOR, 0.7, XGL_CTX_STEXT_COLOR,
                  &black, NULL );
#else
  xgl_object_set( XGLCTX, XGL_CTX_ANNOT_CHAR_HEIGHT, 0.036,
                  XGL_CTX_SFONT_CHAR_EXPANSION_FACTOR, 0.7,
                  XGL_CTX_SFONT_TEXT_COLOR, &black, NULL );
#endif

  pos.x = PDATA( coord_llx ) + 5;
  pos.y = PDATA( coord_lly ) + 3;
  ...
  pos.x = PDATA( coord_llx ) + 4;
  pos.y = PDATA( coord_lly ) + 4;
  xgl_annotation_text( XGLCTX, text, &pos, &offs );
}
