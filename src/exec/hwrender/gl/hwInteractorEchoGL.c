...
#define FACEVISIBLE( u, v, z0, z1, z2 )                           \
  ...
  else
  {
    if ( PDATA( redrawmode ) != tdmViewEchoMode )
    {
      ...
      lrectwrite( PDATA( illx ), PDATA( illy ), PDATA( iurx ), PDATA( iury ),
                  PDATA( background ) );
    }
...
static void _dxf_captureZoomBox( tdmInteractor I, void *udata, float rot[4][4] )
{
  ...
  if ( x1 < iw - 1 )
  {
    ...
    for ( bp = buff, n = ( x1 + 1 ) + ( y1 * iw ), i = y1; i <= y2;
          i++, n += iw )
      image[n] = *bp++;
  }
  ...
  if ( x2 < iw - 1 )
  {
    ...
    for ( bp = buff, n = ( x2 + 1 ) + ( y1 * iw ), i = y1; i <= y2;
          i++, n += iw )
      image[n] = *bp++;
  }
}
...
static void _dxf_ERASE_CURSOR( tdmInteractor I, int x, int y )
{
  ...
  for ( i = 0; i < PDATA( cursor_size ); i++ )
    for ( j = 0; j < PDATA( cursor_size ); j++ )
      ( (long *)PDATA( cursor_saveunder ) )[k++] =
          IMAGE( long, llx + j, lly + i );

  /* blit it to the screen */
  lrectwrite( llx, lly, x + s2, y + s2,
              (const unsigned long *)PDATA( cursor_saveunder ) );
                                      ...
                                      float width )
{
   ...
   */
  ENTRY( ( "_dxf_SET_LINE_ATTRIBUTES(0x%x, 0x%x, 0x%x, %f)", ctx, color, style,
           width ) );
