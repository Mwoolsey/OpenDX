...
static void _dxf_READ_BUFFER( void *ctx, int llx, int lly, int urx, int ury,
                              void *buff )
{
  /*
  ...
  glReadPixels( llx, lly, ( urx - llx ) + 1, ( ury - lly ) + 1, GL_RGBA,
                GL_UNSIGNED_BYTE, buff );

  EXIT( ( "" ) );
}

static void _dxf_WRITE_BUFFER( void *ctx, int llx, int lly, int urx, int ury,
                               void *buff )
{
  DEFCONTEXT( ctx );
  ...
  glDrawPixels( ( urx - llx ) + 1, ( ury - lly ) + 1, GL_RGBA, GL_UNSIGNED_BYTE,
                buff );

  EXIT( ( "" ) );
}

static void _dxf_BUFFER_CONFIG( void *ctx, void *image, int llx, int lly,
                                int urx, int ury, int *CurrentDisplayMode,
...
static void _dxf_BUFFER_RESTORE_CONFIG( void *ctx, int OrigDisplayMode,
                                        int OrigBufferMode,
                                        tdmInteractorRedrawMode redrawMode )
{
  /*
  ...
  ENTRY( ( "_dxf_BUFFER_RESTORE_CONFIG (0x%x, %d, %d, %d)", ctx,
           OrigDisplayMode, OrigBufferMode, redrawMode ) );
  ...
  if ( redrawMode == tdmBothBufferDraw || redrawMode == tdmFrontBufferDraw )
  {
    ...
    if ( OrigBufferMode == GL_BACK || OrigBufferMode == GL_BACK_LEFT )
    {
  ...
  else if ( redrawMode == tdmBackBufferDraw || redrawMode == tdmViewEchoMode )
  {
    ...
    if ( OrigBufferMode == GL_FRONT || OrigBufferMode == GL_FRONT_LEFT )
    {
...
#define DRAW_ARROW( axis )                          \
  {                                                 \
    ...
    glVertex2f( axis[0] + ALENGTH *( axc - ays ),   \
                axis[1] + ALENGTH *( axs + ayc ) ); \
    glVertex2fv( axis );                            \
    glVertex2f( axis[0] + ALENGTH *( axc + ays ),   \
                axis[1] + ALENGTH *( ayc - axs ) ); \
    glEnd();                                        \
  }

static void _dxf_DRAW_GNOMON( tdmInteractor I, void *udata, float rot[4][4],
                              int draw )
{
  /*
  ...
  if ( xlabel[0] <= 0 )
    xlabel[0] -= PDATA( swidth );
  if ( xlabel[1] <= 0 )
    xlabel[1] -= PDATA( swidth );

  if ( ylabel[0] <= 0 )
    ylabel[0] -= PDATA( swidth );
  if ( ylabel[1] <= 0 )
    ylabel[1] -= PDATA( swidth );

  if ( zlabel[0] <= 0 )
    zlabel[0] -= PDATA( swidth );
  if ( zlabel[1] <= 0 )
    zlabel[1] -= PDATA( swidth );

  if ( !draw )
  {
    /* offset text slightly for shadow */
    xlabel[0] += PDATA( nudge );
    xlabel[1] -= PDATA( nudge );
    ylabel[0] += PDATA( nudge );
    ylabel[1] -= PDATA( nudge );
    zlabel[0] += PDATA( nudge );
    zlabel[1] -= PDATA( nudge );
  }
  ...
  if ( draw && PDATA( redrawmode ) != tdmViewEchoMode )
  {
    ...
    glCopyPixels( PDATA( illx ), PDATA( illy ),
                  ( PDATA( iurx ) - PDATA( illx ) ) + 1,
                  ( PDATA( iury ) - PDATA( illy ) ) + 1, GL_COLOR );
    /*
...
static void _dxf_DRAW_GLOBE( tdmInteractor I, void *udata, float rot[4][4],
                             int draw )
{
  /*
...
static void _dxf_GET_WINDOW_ORIGIN( void *ctx, int *x, int *y )
{
  ...
  *y = plc.ptMinPosition.y;
  ...
  XTranslateCoordinates( OGLDPY, OGLXWIN, RootWindow( OGLDPY, OGLSCREEN ), 0, 0,
                         x, y, &child );
                                      ...
                                      float width )
{
  ...
  ENTRY( ( "_dxf_SET_LINE_ATTRIBUTES (0x%x, 0x%x, %d, %d)", ctx, color, style,
           width ) );
...
static void _dxf_ERASE_CURSOR( tdmInteractor I, int x, int y )
{
  ...
  for ( i = 0; i < PDATA( cursor_size ); i++ )
    for ( j = 0; j < PDATA( cursor_size ); j++ )
      ( (int32 *)PDATA( cursor_saveunder ) )[k++] =
          IMAGE( int32, llx + j, lly + i );

  /* blit it to the screen */
  glRasterPos2i( llx, lly );
  CHECK_RASTER_POS();
  glDrawPixels( PDATA( cursor_size ), PDATA( cursor_size ), GL_RGBA,
                GL_UNSIGNED_BYTE, PDATA( cursor_saveunder ) );
...
static void _dxf_POINTER_INVISIBLE( void *ctx )
{
  ...
  p1 = XCreatePixmapFromBitmapData( OGLDPY, RootWindow( OGLDPY, OGLSCREEN ),
                                    tmp_bits, 4, 4, 0, 0, 1 );
  p2 = XCreatePixmapFromBitmapData( OGLDPY, RootWindow( OGLDPY, OGLSCREEN ),
                                    tmp_bits, 4, 4, 0, 0, 1 );
...
static void _dxf_POINTER_VISIBLE( void *ctx )
{
#if !defined( DX_NATIVE_WINDOWS )
  DEFCONTEXT( ctx );
...
static void _dxf_WARP_POINTER( void *ctx, int x, int y )
{
  ...
  XWarpPointer( OGLDPY, None, DefaultRootWindow( OGLDPY ), 0, 0, 0, 0, x,
                YMAXSCREEN - y );
...
static void _dxf_restoreZoomBox( tdmInteractor I, void *udata, float rot[4][4] )
{
  ...
  x1 = ( PDATA( x1 ) < 0 ? 0 : ( PDATA( x1 ) > ( iw - 1 ) ? iw - 1
                                                          : PDATA( x1 ) ) );
  x2 = ( PDATA( x2 ) < 0 ? 0 : ( PDATA( x2 ) > ( iw - 1 ) ? iw - 1
                                                          : PDATA( x2 ) ) );
  y1 = ( PDATA( y1 ) < 0 ? 0 : ( PDATA( y1 ) > ( ih - 1 ) ? ih - 1
                                                          : PDATA( y1 ) ) );
  y2 = ( PDATA( y2 ) < 0 ? 0 : ( PDATA( y2 ) > ( ih - 1 ) ? ih - 1
                                                          : PDATA( y2 ) ) );
...
static void _dxf_captureZoomBox( tdmInteractor I, void *udata, float rot[4][4] )
{
  ...
  /* Top Row */
  glReadPixels( x1, y1, ( x2 - x1 ) + 1, 1, GL_RGBA, GL_UNSIGNED_BYTE,
                image + ( x1 + y1 * iw ) );
  ...
  /* Bottom Row */
  glReadPixels( x1, y2, ( x2 - x1 ) + 1, 1, GL_RGBA, GL_UNSIGNED_BYTE,
                image + ( x1 + y2 * iw ) );
  if ( y2 < ih - 1 )
  ...
  if ( x1 < iw - 1 )
  {
    glReadPixels( x1 + 1, y1, 1, ( y2 - y1 ) + 1, GL_RGBA, GL_UNSIGNED_BYTE,
                  buff );
    for ( bp = buff, n = x1 + 1 + ( y1 * iw ), i = y1; i <= y2; i++, n += iw )
      image[n] = *bp++;
  }
  ...
  if ( x2 < iw - 1 )
  {
    glReadPixels( x2 + 1, y1, 1, ( y2 - y1 ) + 1, GL_RGBA, GL_UNSIGNED_BYTE,
                  buff );
    for ( bp = buff, n = x2 + 1 + ( y1 * iw ), i = y1; i <= y2; i++, n += iw )
      image[n] = *bp++;
  }
...
static void _dxf_DRAW_ZOOMBOX( tdmInteractor I, void *udata, float rot[4][4],
                               int draw )
{
...
static void _dxf_SET_LINE_COLOR_GRAY( void *ctx )
{
  /* obsolete entry:  do not use in new code */
