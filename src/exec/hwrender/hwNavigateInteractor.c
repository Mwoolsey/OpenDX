...
tdmInteractor _dxfCreateNavigator( tdmInteractorWin W, tdmViewEchoFunc EchoFunc,
                                   void *udata )
{
  /*
                            ...
                            float viewUpReturn[3] )
{
  ...
  ENTRY( ( "_dxfSetNavigateLookAt(0x%x, %d, %f, 0x%x, 0x%x, 0x%x)", I,
           direction, angle, current_view, viewDirReturn, viewUpReturn ) );
...
static void StartStroke( tdmInteractor I, int x, int y, int btn, int s )
{
  ...
  if ( !CDATA( projection ) )
  {
    ...
    _dxfGetNearFar( 1, CDATA( w ), CDATA( fov ), from, zaxis, CDATA( box ),
                    &Near, &Far );
...
static void StrokePoint( tdmInteractor I, int x, int y, int flag, int state )
{
  ...
  if ( PDATA( btn ) == 1 )
  {
    ...
    trn[3][0] = -PDATA( translate_speed_factor ) * 0.0005 * CDATA( vdist ) *
                PDATA( nav_dir[0] );
    trn[3][1] = -PDATA( translate_speed_factor ) * 0.0005 * CDATA( vdist ) *
                PDATA( nav_dir[1] );
    trn[3][2] = -PDATA( translate_speed_factor ) * 0.0005 * CDATA( vdist ) *
                PDATA( nav_dir[2] );
  ...
  GET_VC_ORIGIN( CDATA( viewXfm ), from );
  GET_VIEW_DIRECTION( CDATA( viewXfm ), zaxis );
  _dxfGetNearFar( 1, CDATA( w ), CDATA( fov ), from, zaxis, CDATA( box ), &Near,
                  &Far );

  /* setting new clip planes requires creating a new projection matrix */
  _dxfSetProjectionInfo( CDATA( stack ), 1, CDATA( fov ), CDATA( aspect ), Near,
                         Far );
  _dxf_SET_PERSPECTIVE_PROJECTION( PORT_CTX, CDATA( fov ), CDATA( aspect ),
                                   Near, Far );
