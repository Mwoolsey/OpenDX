...
static void _sendLinkCamera( tdmChildGlobalP globals, float *from, float *to,
                             float *up, float width, float aspect, float fov,
                             float dist, int projection, int pixwidth );

#if defined( DX_NATIVE_WINDOWS )
static int internAtoms()
#else
static int internAtoms( Display *dpy )
#endif
{
  /*
...
static char *lookupAtomName( Atom a )
{
  /*
...
#if defined( DX_NATIVE_WINDOWS )
void _dxfSendClientMessage( HWND win, int atomIndex, tdmMessageDataP data )
{
  SendMessage( win, WM_CLIENT_MESSAGE, atomIndex, (void *)data );
}
#else

void _dxfSendClientMessage( Display *dpy, Window win, int atomIndex,
                            tdmMessageDataP data )
{
  /*
  ...
  ENTRY( ( "_dxfSendClientMessage(0x%x, 0x%x, %d, 0x%x)", dpy, win, atomIndex,
           data ) );
...
static void _SendCamera( WinP win, float to[3], float up[3], float from[3],
                         ...
                         int proj, int refresh )
{
  ...
  ENTRY( ( "_SendCamera (0x%x, 0x%x, 0x%x, 0x%x, %f, %d, %f, %f, %d, %d)", win,
           to, up, from, width, pixwidth, aspect, fov, proj, refresh ) );
...
int _dxfInitCMProtocol( WinP win )
{
  /*
...
static void EchoFunction( tdmInteractor I, void *data, float rot[4][4],
                          int draw )
{
  DEFGLOBALDATA( data );
  DEFPORT( PORT_HANDLE );
...
void _dxfInitDXInteractors( tdmChildGlobalP globals )
{
  ...
  W = INTERACTOR_DATA = _dxfInitInteractors( PORT_HANDLE, MATRIX_STACK );
  ...
  globals->GnomonRotate =
      _dxfCreateRotationInteractor( W, tdmGnomonEcho, tdmXYPlaneRoll );

  /* the Globe is always passive, and is driven by the Gnomon */
  globals->Globe =
      _dxfCreateRotationInteractor( W, tdmGlobeEcho, tdmXYPlaneRoll );
  ...
  globals->GnomonTwirl =
      _dxfCreateRotationInteractor( W, tdmNullEcho, tdmZTwirl );
  ...
  globals->Navigator = _dxfCreateNavigator( W, EchoFunction, (void *)globals );
  ...
  else
  {
    ...
    globals->CrntInteractor = globals->RotateGroup;
    _dxfUpdateInteractorGroup( globals->RotateGroup, globals->ViewRotate,
                               globals->ViewRotate, globals->ViewTwirl );
  }
...
static int switchInteractors( tdmChildGlobalP globals )
{
  /*
  ...
  if ( globals->CrntInteractor == globals->RotateGroup )
  {
    ...
    _dxfDisassociateInteractorEcho( INTERACTOR_DATA, globals->GnomonTwirl );
    refresh = 1;
  }
...
void _dxfSetInteractionMode( tdmChildGlobalP globals, int mode, dxObject args )
{
  switch ( mode )
  {
    ...
    case INTERACTION_CURSORS:
      ...
      _dxfRotateInteractorVisible( globals->GnomonTwirl );
      _dxfRotateInteractorVisible( globals->GnomonRotate );
      _dxfAssociateInteractorEcho( globals->Cursor3D, globals->GnomonTwirl );
    ...
    case INTERACTION_ROAM:
      ...
      _dxfRotateInteractorVisible( globals->GnomonTwirl );
      _dxfRotateInteractorVisible( globals->GnomonRotate );
      _dxfAssociateInteractorEcho( globals->Roamer, globals->GnomonTwirl );
    ...
    case INTERACTION_NAVIGATE:
    {
      ...
      rot = DXGetCameraRotation( CAMERA );
      _dxfSetNavigateLookAt( globals->Navigator, tdmLOOK_ALIGN, 0.0, rot.A,
                             viewDirReturn, viewUpReturn );
...
#if defined( DX_NATIVE_WINDOWS )
Error _dxfReceiveClientMessage( tdmChildGlobalP globals, UINT msg, void *data,
                                int *NeedRefresh )
#else
Error _dxfReceiveClientMessage( tdmChildGlobalP globals,
...
#endif
{
  ...
  ENTRY( ( "_dxfReceiveClientMessage (0x%x, 0x%x, 0x%x)", globals, message,
           NeedRefresh ) );
  PRINT( ( "message_type = %d", message->message_type ) );

  /* get current view parameters */
  _dxfGetInteractorViewInfo( INTERACTOR_DATA, from, to, up, &dist, &fov, &width,
                             &aspect, &projection, &Near, &Far, &pixwidth );
...
#define COMPARE_MSGTYPE( t ) ( message->message_type == ATOM( ( t ) ) )
  ...
  else if ( COMPARE_MSGTYPE( ExecuteOnChange ) )
  {
    ...
    if ( dataP->l[0] && !globals->executeOnChange )
    {
      ...
      _dxfUpdateInteractorGroup( globals->RoamGroup, globals->Roamer,
                                 globals->ViewRotate, globals->ViewTwirl );
      _dxfUpdateInteractorGroup( globals->CursorGroup, globals->Cursor3D,
                                 globals->ViewRotate, globals->ViewTwirl );
      _dxfUpdateInteractorGroup( globals->RotateGroup, globals->ViewRotate,
                                 globals->ViewRotate, globals->ViewTwirl );
    }
    else if ( !dataP->l[0] && globals->executeOnChange )
    {
      ...
      _dxfUpdateInteractorGroup( globals->RoamGroup, globals->Roamer,
                                 globals->GnomonRotate, globals->GnomonTwirl );
      _dxfUpdateInteractorGroup( globals->CursorGroup, globals->Cursor3D,
                                 globals->GnomonRotate, globals->GnomonTwirl );
      _dxfUpdateInteractorGroup( globals->RotateGroup, globals->GnomonRotate,
                                 globals->GnomonRotate, globals->GnomonTwirl );
    }
  ...
  else if ( COMPARE_MSGTYPE( NavigateLookAt ) )
  {
    ...
    rot = DXGetCameraRotation( CAMERA );
    _dxfSetNavigateLookAt( globals->Navigator, dataP->l[0], dataP->f[1], rot.A,
                           viewDirReturn, viewUpReturn );
    ...
    f[0] = from.x;
    t[0] = f[0] + dist * viewDirReturn[0];
    f[1] = from.y;
    t[1] = f[1] + dist * viewDirReturn[1];
    f[2] = from.z;
    t[2] = f[2] + dist * viewDirReturn[2];
    ...
    _SendCamera( LWIN, t, viewUpReturn, f, 0, pixwidth, aspect, fov, 1, 1 );
  ...
  else if ( COMPARE_MSGTYPE( UndoCamera ) )
  {
    ...
    if ( ( r = _dxfPopInteractorCamera( INTERACTOR_DATA ) ) != 0 )
    {
      ...
      _dxfGetInteractorViewInfo( INTERACTOR_DATA, f, t, u, &dist, &fov, &width,
                                 &aspect, &projection, &Near, &Far, &pixwidth );

      _SendCamera( LWIN, t, u, f, width, pixwidth, aspect, fov, projection, 1 );

      if ( LINK )
        _sendLinkCamera( globals, f, t, u, width, aspect, fov, dist, projection,
                         pixwidth );
    }
    else
      data.l[0] = 0;
  ...
  else if ( COMPARE_MSGTYPE( Set_View ) )
  {
    ...
    from =
        DXAdd( to, DXMul( DXNormalize( dir ), DXLength( DXSub( from, to ) ) ) );

    data.f[0] = f[0] = from.x;
    data.f[1] = f[1] = from.y;
    data.f[2] = f[2] = from.z;
    PRINT( ( "new look-from point" ) );
    VPRINT( data.f );
#if defined( DX_NATIVE_WINDOWS )
                             ...
                             float dist, int projection, int pixwidth )
{
  ...
  DXUIMessage(
      "LINK", "VALUE camera %s [%f %f %f] [%f %f %f] [%f %f %f] %f %d %f %f %d",
      WHERE, to[0], to[1], to[2], up[0], up[1], up[2], from[0], from[1],
      from[2], width, pixwidth, aspect, fov, projection );
...
void _dxfSendInteractorData( tdmChildGlobalP globals, tdmInteractor I,
                             tdmInteractorReturn *returnP )
{
  /*
  ...
  ENTRY( ( "_dxfSendInteractorData(0x%x, 0x%x, 0x%x)", globals, I, returnP ) );
  ...
  else if ( I == globals->User )
  {
    ...
    _SendCamera( LWIN, R->to, R->up, R->from, width, pixwidth, aspect, fov,
                 proj, 0 );
  }
              ...
              R->reason == tdmROTATION_UPDATE ) )
  {
    if ( !PARENT_WINDOW )
    {
      ...
      _dxfSetInteractorViewInfo( INTERACTOR_DATA, R->from, R->to, R->up,
                                 R->dist, fov, width, aspect, projection, Near,
                                 Far );
    }
    else
    {
  ...
  else if ( I == globals->Navigator )
  {
    ...
    _SendCamera( LWIN, R->to, R->up, R->from, 0, pixwidth, aspect, fov, 1, 1 );
  }
