  ...
  switch ( approx )
  {
    case approx_dots:
      ...
      for ( k = 0; k < nshapes; k += mod )
      {
        ...
        {
          if ( color_map )
            for ( i = start, dV = numpts * vsize + cOffs; i < 2;
                  i++, dV += vsize )
              *(RGBColor *)( clist + dV ) =
            ...
            for ( i = start, dV = numpts * vsize + cOffs; i < 2;
                  i++, dV += vsize )
              *(RGBColor *)( clist + dV ) = fcolors[connections[2 * k + i]];
        }
        else if ( xf->colorsDep == dep_connections )
        {
          if ( color_map )
            for ( i = start, dV = numpts * vsize + cOffs; i < 2;
                  i++, dV += vsize )
              *(RGBColor *)( clist + dV ) = color_map[( (char *)fcolors )[k]];
          else
            for ( i = start, dV = numpts * vsize + cOffs; i < 2;
                  i++, dV += vsize )
      ...
      polymarker_with_data3d( FILDES, clist, numpts, num_coords, vertex_flags );
    ...
    default:
      ...
      polyline_with_data3d( FILDES, clist, numpts, num_coords, vertex_flags,
                            NULL );
