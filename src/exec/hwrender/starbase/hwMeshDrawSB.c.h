...
int tdmMeshDraw( tdmPortHandleP portHandle, xfieldT *xf, int buttonUp )
{
  ...
  if ( stripsSB = tdmGetTmeshCacheSB( cache_id, xf ) )
  {
    ...
    if ( approx == approx_lines )
      ...
      {
        polyline_with_data3d( FILDES, stripArray->clist, stripArray->numverts,
                              stripArray->numcoords, stripArray->vertex_flags,
                              NULL );
      }
    else
      for ( ; stripArray < end; stripArray++ )
  ...
  else
  {
    ...
    stripArray = stripsSB->stripArray =
        (tdmTmeshCacheSB *)tdmAllocate( numStrips * sizeof( tdmTmeshCacheSB ) );
    ...
    for ( ; stripsSB->num_strips < numStrips; stripsSB->num_strips++ )
    {
      ...
      if ( approx == approx_lines )
      {
        polyline_with_data3d( FILDES, clist, numPnts, vsize - 3, vertex_flags,
                              NULL );
      }
      else
      {
        triangular_strip_with_data( FILDES, clist, numPnts, gnormals, vsize - 3,
