...
void _dxf_DRAW_ARROWHEAD( void *, float, float );
                         ...
                         tdmInteractorRedrawMode redrawMode )
{
  ...
  ENTRY( ( "_dxf_BUFFER_CONFIG(0x%x, 0x%x, %d, %d, %d, %d, 0x%x, 0x%x, %d)",
           ctx, image, llx, lly, urx, ury, CurrentDisplayMode,
  ...
  *CurrentBufferMode = BUFFER_CONFIG_MODE;
  ...
  {
    PRINT( ( "%s", redrawMode == tdmBothBufferDraw ? "tdmBothBufferDraw"
                                                   : "tdmFrontBufferDraw" ) );
    /*
  ...
  {
    PRINT( ( "%s", redrawMode == tdmViewEchoMode ? "tdmViewEchoMode"
                                                 : "tdmBackBufferDraw" ) );
    /*
                                 ...
                                 tdmInteractorRedrawMode redrawMode )
{
  ...
  ENTRY( ( "_dxf_BUFFER_RESTORE_CONFIG(0x%x, %d, %d, %d)", ctx, OrigDisplayMode,
                       ...
                       void *buff )
{
  ...
  ENTRY( ( "_dxf_READ_BUFFER(0x%x, %d, %d, %d, %d, 0x%x)", ctx, llx, lly, urx,
           ury, buff ) );
                        ...
                        void *buff )
{
  ...
  ENTRY( ( "_dxf_WRITE_BUFFER(0x%x, %d, %d, %d, %d, 0x%x)", ctx, llx, lly, urx,
           ury, buff ) );
  ...
  if ( PLANES == 16 )
  {
    ...
    if ( BUFFER_CONFIG_MODE == BCKBUFFER )
      bank = BUFFER_STATE;
    if ( BUFFER_CONFIG_MODE == FRNTBUFFER )
      bank = !BUFFER_STATE; /* 8th */
    if ( BUFFER_CONFIG_MODE == BOTHBUFFERS )
      bank = BUFFER_STATE;
  ...
  for ( u = 0; u < LATS; u++ )
  {
    for ( v = 0, on = 0; v < LONGS - 1; v++ )
      if ( edges[u][v].latvis )
      {
        ...
        {
          on = 1;
          move3d( FILDES, Globe[u][v][0], Globe[u][v][1], Globe[u][v][2] );
        }
        draw3d( FILDES, Globe[u][v + 1][0], Globe[u][v + 1][1],
                Globe[u][v + 1][2] );
        edges[u][v].latvis = 0;
      }
      else if ( on )
    ...
    if ( edges[u][LONGS - 1].latvis )
    {
      if ( !on )
      {
        move3d( FILDES, Globe[u][LONGS - 1][0], Globe[u][LONGS - 1][1],
                Globe[u][LONGS - 1][2] );
      }
      draw3d( FILDES, Globe[u][0][0], Globe[u][0][1], Globe[u][0][2] );
  ...
  for ( v = 0; v < LONGS; v++ )
  {
    for ( u = 0, on = 0; u < LATS - 1; u++ )
      if ( edges[u][v].longvis )
      {
        ...
        draw3d( FILDES, Globe[u + 1][v][0], Globe[u + 1][v][1],
                Globe[u + 1][v][2] );
        edges[u][v].longvis = 0;
      }
      else if ( on )
...
/* Need to allocate gnomonBuffer once, and not each time */
    ...
    {
      /* allocate a buffer */
      if ( !( GNOBE_BUF = _dxf_ALLOCATE_PIXEL_ARRAY(
                  PORT_CTX, 2 * GNOMONRADIUS + 1, 2 * GNOMONRADIUS + 1 ) ) )
      {
                        ...
                        int draw )
{
  ...
  if ( draw )
  {
    ...
    zoombox[0][0] = x1;
    zoombox[0][1] = YFLIPSB( y1 ); /* llx and lly */
    zoombox[1][0] = x2;
    zoombox[1][1] = YFLIPSB( y1 );
    zoombox[2][0] = x2;
    zoombox[2][1] = YFLIPSB( y2 ); /* urx and ury */
    zoombox[3][0] = x1;
    zoombox[3][1] = YFLIPSB( y2 );
    zoombox[4][0] = x1;
    zoombox[4][1] = YFLIPSB( y1 );
    line_color( FILDES, 1.0, 1.0, 1.0 );
...
void _dxf_GET_WINDOW_ORIGIN( void *ctx, int *x, int *y )
{
  ...
  XTranslateCoordinates( DPY_SB, XWINDOW_ID, RootWindow( DPY_SB, SCREEN_SB ), 0,
                         0, x, y, &child );
...
void _dxf_CREATE_CURSOR_PIXMAPS( tdmInteractor I )
{
  ...
  PDATA( cursor_saveunder ) = _dxf_ALLOCATE_PIXEL_ARRAY(
  ...
  PDATA( ActiveSquareCursor ) = _dxf_ALLOCATE_PIXEL_ARRAY(
      PORT_CTX, PDATA( cursor_size ), PDATA( cursor_size ) );
  ...
  if ( ( PLANES == 24 ) || ( PLANES == 48 ) ) /* CRX48Z */
    for ( i = 0; i < PDATA( cursor_size ); i++ )
      for ( j = 0; j < PDATA( cursor_size ); j++ )
        if ( i == 0 || i == PDATA( cursor_size ) - 1 || j == 0 ||
             j == PDATA( cursor_size ) - 1 )
        {
  ...
  PDATA( PassiveSquareCursor ) = _dxf_ALLOCATE_PIXEL_ARRAY(
      PORT_CTX, PDATA( cursor_size ), PDATA( cursor_size ) );
...
void _dxf_ERASE_CURSOR( tdmInteractor I, int x, int y )
{
  ...
  /* write it to the screen */
  _dxf_WRITE_BUFFER( PORT_CTX, llx, lly, x + s2, y + s2,
                     (void *)PDATA( cursor_saveunder ) );
...
void _dxf_WARP_POINTER( void *ctx, int x, int y )
{
  ...
  XWarpPointer( DPY_SB, None, RootWindow( DPY_SB, SCREEN_SB ), 0, 0, 0, 0, x,
                YMAXSCREEN - y );
                                      ...
                                      float width )
{
   ...
   */
  ENTRY( ( "_dxf_SET_LINE_ATTRIBUTES(0x%x, %d, %d, %f)", ctx, color, style,
           width ) );
...
void _dxf_DRAW_MARKER( tdmInteractor I )
{
  ...
  for ( i = 0; i < PDATA( iMark ); i++ )
  {
    ...
    /* draw it */
    if ( !CLIPPED( mx, my ) )
      dcrectangle( FILDES, mx - 1, YFLIPSB( my - 1 ), mx + 1,
                   YFLIPSB( my + 1 ) );
  }
...
void _dxf_DRAW_CURSOR_COORDS( tdmInteractor I, char *text )
{
  ...
  dctext( FILDES, PDATA( coord_llx ) + 4, YFLIPSB( PDATA( coord_lly ) + 2 ),
          text );

  text_color( FILDES, 1.0, 1.0, 1.0 );
  dctext( FILDES, PDATA( coord_llx ) + 3, YFLIPSB( PDATA( coord_lly ) + 3 ),
          text );
