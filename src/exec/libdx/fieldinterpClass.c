...
Error _dxfno_PrimitiveInterpolate(FieldInterpolator a, int * b, float ** c, Pointer * d, int e) {
    char *name;
    name = (*(struct fieldinterpolator_class **)a)->name;
    DXSetError(ERROR_BAD_PARAMETER, "#12130", "DXPrimitiveInterpolate", name);
    return ERROR;
}

struct fieldinterpolator_class _dxdfieldinterpolator_class = {
    sizeof(struct fieldinterpolator),
    CLASS_FIELDINTERPOLATOR,
    "fieldinterpolator",
    CLASS_INTERPOLATOR,
    "interpolator",
    _dxfFieldInterpolator_Delete,
    _dxfObject_Shade,
    ...
    CLASS_FIELDINTERPOLATOR,
    "fieldinterpolator",
    _dxfno_PrimitiveInterpolate,
};

Error _dxfPrimitiveInterpolate(FieldInterpolator a, int * b, float ** c, Pointer * d, int e) {
    if (!a)
        return ERROR;
    return (*(struct fieldinterpolator_class **)a)->PrimitiveInterpolate((FieldInterpolator)a,b,c,d,e);
}

Class DXGetFieldInterpolatorClass(FieldInterpolator o) {
    return o? (*(struct fieldinterpolator_class **)o)->class: CLASS_MIN;
}
