...
static Error whatkindofmem( ulong limit )
{
  ...
  if ( !_dxd_exRunningSingleProcess )
  {
    ...
    {
      DXSetError( ERROR_BAD_PARAMETER, "DXSHMEM cannot be -1; multiprocessing "
                                       "requires using shared memory" );
      return ERROR;
    }
    m = MEM_SHARED;
    return OK;
  ...
  if ( limit >= DATASEG_LIMIT )
  {
    if ( force && ( m == MEM_DATASEG ) )
    {
      DXSetError( ERROR_BAD_PARAMETER, "DXSHMEM cannot be -1; more than %u "
                                       "Mbytes requires using shared memory",
...
Error _dxfsetmem( ulong limit )
{
  ...
  while ( totalsize < limit )
  {
     ...
     */
    size = ( ( limit - totalsize ) > maxsegsize ) ? maxsegsize
                                                  : ( limit - totalsize );
    ...
    if ( id < 0 )
    {
      ...
      DXAddMessage( "failed on segment %d of %ld needed segments", nextseg,
                    ( (long)limit + maxsegsize - 1 ) / maxsegsize );
    ...
    if ( nchunk >= MAX_CHUNKS - 1 )
    {
      DXSetError( ERROR_NO_MEMORY,
                  ...
                  MAX_CHUNKS );
      DXAddMessage( "start again with -memory set to a smaller value or with "
                    "$DXSHMEMSEGMAX to a larger value" );
      return ERROR;
    }
  ...
  got_one:
    ...
    if ( shmctl( id, IPC_RMID, 0 ) < 0 )
      DXWarning( "cannot mark shared memory segment to be deleted at process "
                 "exit.  id = %d",
                 id );
...
Pointer _dxfgetmem( Pointer base, ulong size )
{
  ...
  if ( base > alloc_addr_start[i] &&
       ADD_INT( base, size ) <= alloc_addr_end[i] )
    return base;

  /* we are either completely out of space, or we need to return the start of
   * the
  ...
  /* we've gone past the end of the current chunk - return the start of the next
   * one */
...
BOOL LoggedSetLockPagesPrivilege( HANDLE hProcess, BOOL bEnable )
{
  ...
  Result = AdjustTokenPrivileges( Token, FALSE, ( PTOKEN_PRIVILEGES ) & Info, 0,
                                  NULL, NULL );
  ...
  else
  {
    ...
    {
      DXSetError( ERROR_NO_MEMORY, "Cannot enable SE_LOCK_MEMORY privilege, "
...
*/
  ...
  ppPFN = (PULONG_PTR)HeapAlloc( GetProcessHeap(), 0,
                                 PFNArraySize ); /* Allocate PFN array */

  if ( !LoggedSetLockPagesPrivilege( GetCurrentProcess(), TRUE ) )
  {
    DXErrorReturn( ERROR_NO_MEMORY,
                   "Cannot get priveledges to lock physical memory." );
  }
  ...
  if ( AllocateUserPhysicalPages( hProcess, &pnPages, ppPFN ) )
  {
    ...
    sh_base =
        VirtualAlloc( NULL, limit, MEM_RESERVE | MEM_PHYSICAL, PAGE_READWRITE );
    if ( sh_base == 0 )
    {
      LPVOID lpMsgBuf;
      FormatMessage(
...
Error _dxfsetmem( ulong limit )
{
  ...
  MapFileHandle =
      CreateFile( DX_DAT_FILE, GENERIC_READ | GENERIC_WRITE,
                  FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_ALWAYS,
                  FILE_ATTRIBUTE_NORMAL |
                      FILE_FLAG_DELETE_ON_CLOSE /* | STANDARD_RIGHTS_REQUIRED |
                              FILE_MAP_WRITE | FILE_MAP_READ */,
                  NULL );

  MapHandle = CreateFileMapping( MapFileHandle, NULL, PAGE_READWRITE, 0,
                                 (DWORD)limit, DX_MAP_FILE );
  if ( MapHandle == NULL )
  {
  ...
  MappedPointer = MapViewOfFile( MapHandle, FILE_MAP_WRITE | FILE_MAP_READ, 0,
                                 0, (DWORD)limit );
  if ( MappedPointer )
  {
...
Pointer _dxfgetmem( Pointer base, ulong size )
{
  ...
  hAMap = OpenFileMapping( FILE_MAP_READ | FILE_MAP_WRITE, TRUE, DX_MAP_FILE );

  MappedPointer =
      MapViewOfFile( hAMap, FILE_MAP_WRITE | FILE_MAP_READ, 0, 0, DXLimit );
...
Error _dxfsetmem( ulong limit )
{
  ...
  if ( DosAllocSharedMem( &sh_base, "\\sharemem\\dx.dat", (int)limit,
                          PAG_READ | PAG_WRITE ) )
  {
...
Pointer _dxfgetbrk( Pointer base, ulong n )
{
  ...
  if ( x == ERR_PTR )
  {
    ...
    x = (Pointer)sbrk( 0 );
#if defined( cygwin ) || defined( macos )
    DXSetError( ERROR_NO_MEMORY, "cannot expand the data segment by %u bytes",
                n );
#else
...
#define MEMSTATS                                       \
  1 /* change to 0 to comment out this code.  (needs a \
  ...
  switch ( m )
  {
    ...
    case MEM_DATASEG:
      ...
      for ( i = 0; i < min( num_sbrk_calls, MAX_CHUNKS ); i++ )
        DXMessage( "sbrk %d returned 0x%08x, %lu bytes allocated", i + 1,
                   alloc_addr_start[i],
