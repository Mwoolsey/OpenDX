...
/***********************************************************************/
/*
 * Header: /usr/people/gresh/code/svs/src/libdx/RCS/displayx.c,v 5.2 93/01/25
 *17:06:43 gda Exp Locker: gresh
 *
 */
...
extern Error DXRegisterWindowHandlerWithCheckProc( Handler proc, Checker check,
...
Colormap getWindowColormap( Display *dpy, Window w )
{
...
#define BYTE_SCALAR_TO_INDEX_VECTOR ir = ig = ib = *(unsigned char *)_src;

#define BYTE_SCALAR_TO_INDEX i = *(unsigned char *)_src;
...
#define PIXEL_SUM i = ( ir ) + ( ig ) + ( ib )

#define PIXEL_1 i = ( ( ( ( ( ir ) * gg ) + ( ig ) ) * bb ) + ( ib ) )

#define PIXEL_3 i = ir | ig | ib

#define DIRECT_PIXEL_1

#define DIRECT_PIXEL_3 i = ( i << 16 ) | ( i << 8 ) | i
 ...
 */
#define STORE_INT8 *(unsigned char *)t_dst = i;

#define STORE_INT16 *(short *)t_dst = i;

#define STORE_INT24                                  \
  ( (unsigned char *)t_dst )[0] = i & 0xff;          \
  ( (unsigned char *)t_dst )[1] = ( i >> 8 ) & 0xff; \
  ( (unsigned char *)t_dst )[2] = ( i >> 16 ) & 0xff;
#define STORE_INT32 *(int32 *)t_dst = i;
...
Error _dxf_translateXImage(
    Object image,            /* object defining overall pixel grid          */
    ...
    translationP translation, unsigned char *src, int inType,
    unsigned char *dst )
{
  unsigned long *table;
  int width, height;
  ...
  if ( inType == IN_BYTE_SCALAR )
  {
    ...
    {
      if ( outType == OUT_INT8 )
        LOOP( sizeof( byte ), 1, 0, BYTE_SCALAR_TO_INDEX, NO_COLORMAP, NO_GAMMA,
              NO_DITHER, NO_XLATE, DIRECT_PIXEL_1, NO_XLATE, STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( sizeof( byte ), 2, 0, BYTE_SCALAR_TO_INDEX, NO_COLORMAP, NO_GAMMA,
              NO_DITHER, NO_XLATE, DIRECT_PIXEL_1, NO_XLATE, STORE_INT16 )
      else if ( outType == OUT_INT24 )
        LOOP( sizeof( byte ), 3, 0, BYTE_SCALAR_TO_INDEX, NO_COLORMAP, NO_GAMMA,
              NO_DITHER, NO_XLATE, DIRECT_PIXEL_1, NO_XLATE, STORE_INT24 )
      else
        LOOP( sizeof( byte ), 4, 0, BYTE_SCALAR_TO_INDEX, NO_COLORMAP, NO_GAMMA,
              NO_DITHER, NO_XLATE, DIRECT_PIXEL_1, NO_XLATE, STORE_INT32 );
    }
    else
    {
      /*
      ...
      if ( translation->translationType == OneMap )
      {
        ...
        if ( outType == OUT_INT8 )
          LOOP( sizeof( byte ), 1, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, NO_GAMMA,
                DITHER, NO_XLATE, PIXEL_1, ONE_XLATE, STORE_INT8 )
        else if ( 1 == OUT_INT16 )
          LOOP( sizeof( byte ), 2, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, NO_GAMMA,
                DITHER, NO_XLATE, PIXEL_1, ONE_XLATE, STORE_INT16 )
        else if ( 1 == OUT_INT24 )
          LOOP( sizeof( byte ), 3, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, NO_GAMMA,
                DITHER, NO_XLATE, PIXEL_1, ONE_XLATE, STORE_INT24 )
        else
          LOOP( sizeof( byte ), 4, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, NO_GAMMA,
                DITHER, NO_XLATE, PIXEL_1, ONE_XLATE, STORE_INT32 );
      }
      else if ( translation->translationType == GrayStatic )
      {
        ...
        if ( outType == OUT_INT8 )
          LOOP( sizeof( byte ), 1, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT8 )
        else if ( outType == OUT_INT16 )
          LOOP( sizeof( byte ), 2, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT16 )
        else if ( outType == OUT_INT24 )
          LOOP( sizeof( byte ), 3, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT24 )
        else
          LOOP( sizeof( byte ), 4, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT32 );
      }
      else if ( translation->translationType == GrayMapped )
      {
        ...
        if ( outType == OUT_INT8 )
          LOOP( sizeof( byte ), 1, 0, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE, STORE_INT8 )
        else if ( outType == OUT_INT16 )
          LOOP( sizeof( byte ), 2, 0, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE, STORE_INT16 )
        else if ( outType == OUT_INT24 )
          LOOP( sizeof( byte ), 3, 0, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE, STORE_INT24 )
        else
          LOOP( sizeof( byte ), 4, 0, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE, STORE_INT32 );
      }
      else if ( translation->translationType == ColorStatic )
      {
        ...
        if ( outType == OUT_INT8 )
          LOOP( sizeof( byte ), 1, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT8 )
        else if ( outType == OUT_INT16 )
          LOOP( sizeof( byte ), 2, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT16 )
        else if ( outType == OUT_INT24 )
          LOOP( sizeof( byte ), 3, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT24 )
        else
          LOOP( sizeof( byte ), 4, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT32 );
      }
      else if ( translation->translationType == ThreeMap )
      {
        ...
        if ( outType == OUT_INT8 )
          LOOP( sizeof( byte ), 1, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT8 )
        else if ( outType == OUT_INT16 )
          LOOP( sizeof( byte ), 2, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT16 )
        else if ( outType == OUT_INT24 )
          LOOP( sizeof( byte ), 3, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT24 )
        else
          LOOP( sizeof( byte ), 4, 1, BYTE_SCALAR_TO_INDEX, COLORMAP, GAMMA,
                DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT32 );
      }
    }
  }
  else if ( inType == IN_FLOAT_VECTOR )
  {
    if ( translation->translationType == NoTranslation )
    {
      if ( outType == OUT_INT8 )
        LOOP( 3 * sizeof( float ), 1, 0, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, NO_GAMMA, NO_DITHER, NO_XLATE, PIXEL_3, NO_XLATE,
              STORE_INT8 )
      else if ( outType == OUT_INT16 )
    ...
    else if ( translation->translationType == GrayStatic )
    {
      ...
      if ( outType == OUT_INT8 )
        LOOP( 3 * sizeof( float ), 1, 0, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE,
              STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( 3 * sizeof( float ), 2, 0, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE,
    ...
    else if ( translation->translationType == GrayMapped )
    {
      ...
      if ( outType == OUT_INT8 )
        LOOP( 3 * sizeof( float ), 1, 0, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE,
              STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( 3 * sizeof( float ), 2, 0, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE,
    ...
    else if ( translation->translationType == ColorStatic )
    {
      ...
      if ( outType == OUT_INT8 )
        LOOP( 3 * sizeof( float ), 1, 1, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, GAMMA, DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE,
              STORE_INT8 )
      else if ( outType == OUT_INT16 )
    ...
    else if ( translation->translationType == OneMap )
    {
      ...
      if ( outType == OUT_INT8 )
        LOOP( 3 * sizeof( float ), 1, 1, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, NO_GAMMA, DITHER, NO_XLATE, PIXEL_1, ONE_XLATE,
              STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( 3 * sizeof( float ), 2, 1, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, NO_GAMMA, DITHER, NO_XLATE, PIXEL_1, ONE_XLATE,
    ...
    else
    {
      ...
      if ( outType == OUT_INT8 )
        LOOP( 3 * sizeof( float ), 1, 1, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, GAMMA, DITHER, THREE_XLATE, PIXEL_3, NO_XLATE,
              STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( 3 * sizeof( float ), 2, 1, FLOAT_VECTOR_TO_INDEX_VECTOR,
              NO_COLORMAP, GAMMA, DITHER, THREE_XLATE, PIXEL_3, NO_XLATE,
  ...
  else if ( inType == IN_BIG_PIX )
  {
    ...
    {
      if ( outType == OUT_INT8 )
        LOOP( sizeof( struct big ), 1, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              NO_GAMMA, NO_DITHER, NO_XLATE, PIXEL_3, NO_XLATE, STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( sizeof( struct big ), 2, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              NO_GAMMA, NO_DITHER, NO_XLATE, PIXEL_3, NO_XLATE, STORE_INT16 )
      else if ( outType == OUT_INT24 )
        LOOP( sizeof( struct big ), 3, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              NO_GAMMA, NO_DITHER, NO_XLATE, PIXEL_3, NO_XLATE, STORE_INT24 )
      else
        LOOP( sizeof( struct big ), 4, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              NO_GAMMA, NO_DITHER, NO_XLATE, PIXEL_3, NO_XLATE, STORE_INT32 );
    }
    else if ( translation->translationType == GrayStatic )
    {
      rtable = translation->rtable;
      ...
      if ( outType == OUT_INT8 )
        LOOP( sizeof( struct big ), 1, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( sizeof( struct big ), 2, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT16 )
      else if ( outType == OUT_INT24 )
        LOOP( sizeof( struct big ), 3, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT24 )
      else
        LOOP( sizeof( struct big ), 4, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT32 );
    }
    else if ( translation->translationType == GrayMapped )
    {
      rtable = translation->rtable;
      ...
      if ( outType == OUT_INT8 )
        LOOP( sizeof( struct big ), 1, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE, STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( sizeof( struct big ), 2, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE, STORE_INT16 )
      else if ( outType == OUT_INT24 )
        LOOP( sizeof( struct big ), 3, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE, STORE_INT24 )
      else
        LOOP( sizeof( struct big ), 4, 0, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, NO_DITHER, THREE_XLATE, PIXEL_SUM, ONE_XLATE,
              STORE_INT32 );
    }
    else if ( translation->translationType == ColorStatic )
    {
      ...
      if ( outType == OUT_INT8 )
        LOOP( sizeof( struct big ), 1, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( sizeof( struct big ), 2, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT16 )
      else if ( outType == OUT_INT24 )
        LOOP( sizeof( struct big ), 3, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT24 )
      else
        LOOP( sizeof( struct big ), 4, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT32 );
    }
    else if ( translation->translationType == OneMap )
    {
      table = translation->table;
      ...
      if ( outType == OUT_INT8 )
        LOOP( sizeof( struct big ), 1, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              NO_GAMMA, DITHER, NO_XLATE, PIXEL_1, ONE_XLATE, STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( sizeof( struct big ), 2, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              NO_GAMMA, DITHER, NO_XLATE, PIXEL_1, ONE_XLATE, STORE_INT16 )
      else if ( outType == OUT_INT24 )
        LOOP( sizeof( struct big ), 3, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              NO_GAMMA, DITHER, NO_XLATE, PIXEL_1, ONE_XLATE, STORE_INT24 )
      else
        LOOP( sizeof( struct big ), 4, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              NO_GAMMA, DITHER, NO_XLATE, PIXEL_1, ONE_XLATE, STORE_INT32 );
    }
    else
    {
      rtable = translation->rtable;
      ...
      if ( outType == OUT_INT8 )
        LOOP( sizeof( struct big ), 1, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT8 )
      else if ( outType == OUT_INT16 )
        LOOP( sizeof( struct big ), 2, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT16 )
      else if ( outType == OUT_INT24 )
        LOOP( sizeof( struct big ), 3, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT24 )
      else
        LOOP( sizeof( struct big ), 4, 1, BIG_PIX_TO_INDEX_VECTOR, NO_COLORMAP,
              GAMMA, DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT32 );
    }
  }
...
static Object createTranslation( Private dpy, char *where, int desiredDepth,
                                 int *directMap, struct window *w );

static void free_ximage( struct window *w )
{
...
static Error reset_window( struct window *w )
{
  if ( w->pixmap )
  {
...
static Error delete_window( Pointer p )
{
...
int XChecker( int fd, Pointer p )
{
  struct window *w = p;
  return XPending( w->dpy );
}

static Error _DXSetSoftwareWindowActive( struct window *w, int active )
{
  if ( active )
  {
    if ( !DXRegisterWindowHandlerWithCheckProc( w->handler, XChecker, w->dpy,
                                                (Pointer)w ) )
      return ERROR;
  ...
  else
  {
    ...
    if ( !DXRegisterWindowHandlerWithCheckProc( NULL, NULL, w->dpy,
                                                (Pointer)w ) )
      return ERROR;
...
static Error display_either( Object image, int width, int height, int ox,
                             int oy, struct window *w )
{
  Display *dpy = w->dpy;
       ...
       DXGetComponentValue( (Field)image, IMAGE ) )
  {
    ...
    if ( (int)DXGetObjectTag( w->pixels_owner ) != image_tag )
    {
      ...
      if ( !w->pixels )
        goto error;
      w->ximage = XCreateImage( dpy, (Visual *)w->translation->visual,
                                w->translation->depth, ZPixmap, 0,
                                (char *)w->pixels, width, height, 8, 0 );
    }
  }
  ...
  else if ( width != w->pwidth || height != w->pheight )
  {
    ...
    free_ximage( w );
    w->pixels = (unsigned char *)DXAllocate(
        width * height * _dxf_GetXBytesPerPixel( w->translation ) );
    if ( !w->pixels )
      goto error;
    w->pwidth = width;
    w->pheight = height;
  ...
  if ( !w->pixels_owner )
  {
    ...
    if ( DXGetObjectClass( image ) != CLASS_FIELD )
    {
      ...
      {
        pixel = RGBMIX( w->translation->rtable[0], w->translation->gtable[0],
    ...
    if ( !_dxf_dither( image, width, height, ox, oy, w->translation,
                       w->pixels ) )
      goto error;
  }
...
static char *names[] = {
    "nothing",        "event 1",        "KeyPress",         "KeyRelease",
...
static Error handler_script( int fd, struct window *w )
{
  XEvent event;
  extern char *names[];
  ...
  if ( w->refresh )
  {
    XInstallColormap( w->dpy, (Colormap)w->translation->cmap );
    w->cmap_installed = 1;
    XPutImage( w->dpy, w->wid, w->gc, w->ximage, 0, 0, 0, 0, w->wwidth,
               w->wheight );
  }
...
static Error display_script( Object image, int width, int height, int ox,
                             int oy, struct window *w )
{
  Display *dpy;
  Window root;
  ...
  if ( !w->gc )
  {
    ...
    w->wid = XCreateWindow( dpy, root, 0, 0, width, height, border,
                            trans->depth, InputOutput, trans->visual,
                            CWColormap | CWBackPixel | CWBorderPixel, &xswa );

    w->winFlag = 1;

    XSetStandardProperties( dpy, w->wid, w->title, w->title, None, NULL, 0,
                            NULL );

    XChangeProperty( dpy, w->wid, XA_WM_PROTOCOLS, XA_ATOM, 32, PropModeReplace,
                     (unsigned char *)&XA_WM_DELETE_WINDOW, 1 );

    XSelectInput( dpy, w->wid, EnterWindowMask | LeaveWindowMask |
...
/* #X added */
static Error display_external( Object image, int width, int height, int ox,
                               int oy, struct window *w )
{
  Display *dpy;
  Window root;
...
static Error handler_ui( int fd, struct window *w )
{
  XEvent event;
  extern char *names[];
  ...
  while ( w->waitEvent || w->waitPixmap || XPending( w->dpy ) )
  {
    ...
    {
      DXDebug( "X", "atom 0x%x, state %d", event.xproperty.atom,
...
static Error display_ui( Object image, int width, int height, int ox, int oy,
                         struct window *w )
{
  ...
  if ( !w->gc )
  {
    ...
    XSelectInput( dpy, w->wid, PropertyChangeMask | StructureNotifyMask |
...
#define ASYNC 1
       ...
       w->translation->depth != w->wdepth )
  {
    ...
    new = XCreatePixmap( dpy, w->wid, width, height, w->translation->depth );
    if ( !new )
      goto error;
    XPutImage( w->dpy, new, w->gc, w->ximage, 0, 0, 0, 0, width, height );
    XChangeProperty( dpy, w->wid, w->pixmap_atom, XA_PIXMAP, 32,
  ...
  else
  {
    ...
    XPutImage( w->dpy, w->pixmap, w->gc, w->ximage, 0, 0, 0, 0, width, height );
  }
  ...
  /* flag ready */
  XChangeProperty( dpy, w->wid, w->flag_atom, XA_INTEGER, 32, PropModeReplace,
                   (unsigned char *)&( w->pixmap ), 1 );
...
Object DXDisplayX( Object image, char *host, char *window )
{
  return DXDisplayXAny( image, 0, host, window );
}
Object DXDisplayX4( Object image, char *host, char *window )
{
  return DXDisplayXAny( image, 4, host, window );
}
Object DXDisplayX8( Object image, char *host, char *window )
{
  return DXDisplayXAny( image, 8, host, window );
}
Object DXDisplayX12( Object image, char *host, char *window )
{
  return DXDisplayXAny( image, 12, host, window );
}
Object DXDisplayX15( Object image, char *host, char *window )
{
  return DXDisplayXAny( image, 15, host, window );
}
Object DXDisplayX16( Object image, char *host, char *window )
{
  return DXDisplayXAny( image, 16, host, window );
}
Object DXDisplayX24( Object image, char *host, char *window )
{
  return DXDisplayXAny( image, 24, host, window );
}
Object DXDisplayX32( Object image, char *host, char *window )
{
...
static Error delete_display( Pointer p )
{
  if ( p )
  {
...
static Private create_display( char *name, char *title, int depth,
                               Display **dpy )
{
  Private p = NULL;
  char *cacheid = NULL;
...
static Display *getDisplay( Private p )
{
...
static Error free_private_string( Pointer p )
{
  DXFree( p );
  return OK;
}
...
static char *getWindowCacheTag( char *host, char *title )
{
  char *cacheid = (char *)DXAllocate( strlen( WINDOW_TEMPLATE ) +
                                      strlen( host ) + strlen( title ) + 1 );
  if ( !cacheid )
    return NULL;
...
static char *getWindowIDCacheTag()
{
  char *modid;
  char *cacheid;
  ...
  cacheid =
      (char *)DXAllocate( strlen( WINDOW_ID_TEMPLATE ) + strlen( modid ) + 1 );
...
static Error setWindowIDInCache( char *tag, char *title )
{
  Private p = NULL;
  char *tmp = NULL;
...
static char *getWindowIDFromCache( char *tag )
{
  Private p = NULL;
  char *tmp, *p_data;
...
Error _dxf_SetSoftwareXWindowActive( char *where, int active )
{
  char *cachetag = NULL;
  Private p = NULL;
...
static Error getWindowStructure( char *host, int depth, char *title,
                                 int *directMap, struct window **window,
                                 Private *window_object, int required )
{
  Private p = NULL;
  char *cachetag = NULL;
...
static Array getColormap( Object o )
{
  Array iArray = NULL;
  Array map;
...
Object DXDisplayXAny( Object image, int depth, char *host, char *window )
{
  struct window *w = NULL;
  int directMap = 0;
  Object attr;
...
Error _dxf_CopyBufferToXImage( struct buffer *buf, Field image, int xoff,
                               int yoff )
{
  int iwidth, iheight;
  ...
  if ( !DXGetImageSize( image, &iwidth, &iheight ) )
    return ERROR;
  return _dxf_translateImage( (Object)image, iwidth, iheight, 0, 0, yoff, xoff,
                              buf->height, buf->width, 0, 1, translation, src,
                              inType, dst );
}
...
Field _dxf_ZeroXPixels( Field image, int left, int right, int top, int bot,
                        RGBColor c )
{
  int x, y, d, i;
  Object oo;
  ...
  else if ( translation->translationType == GrayStatic )
  {
    ...
    if ( outType == OUT_INT8 )
      ZERO_LOOP( 1, 0, NO_DITHER, NO_XLATE, DIRECT_PIXEL_1, NO_XLATE,
                 STORE_INT8 )
    else if ( outType == OUT_INT16 )
      ZERO_LOOP( 2, 0, NO_DITHER, NO_XLATE, DIRECT_PIXEL_1, NO_XLATE,
                 STORE_INT16 )
    else if ( outType == OUT_INT24 )
      ZERO_LOOP( 3, 0, NO_DITHER, NO_XLATE, DIRECT_PIXEL_1, NO_XLATE,
                 STORE_INT24 )
    else
      ZERO_LOOP( 4, 0, NO_DITHER, NO_XLATE, DIRECT_PIXEL_1, NO_XLATE,
                 STORE_INT32 );
  }
  else if ( translation->translationType == GrayMapped )
  {
  ...
  else if ( translation->translationType == ColorStatic )
  {
    ...
    if ( outType == OUT_INT8 )
      ZERO_LOOP( 1, 1, DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT8 )
    else if ( outType == OUT_INT16 )
      ZERO_LOOP( 2, 1, DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT16 )
    else if ( outType == OUT_INT24 )
      ZERO_LOOP( 3, 1, DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT24 )
    else
      ZERO_LOOP( 4, 1, DITHER, THREE_XLATE, PIXEL_SUM, NO_XLATE, STORE_INT32 );
  }
  else if ( translation->translationType == ThreeMap )
  {
    unsigned long *rtable = translation->rtable;
    ...
    {
      GAMMA;
      ZERO_LOOP( 1, 1, DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT8 )
    }
    else if ( outType == OUT_INT16 )
    {
      GAMMA;
      ZERO_LOOP( 2, 1, DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT16 )
    }
    else if ( outType == OUT_INT24 )
    {
      GAMMA;
      ZERO_LOOP( 3, 1, DITHER, THREE_XLATE, PIXEL_3, NO_XLATE, STORE_INT24 )
    }
    ...
    {
      DXSetError( ERROR_INTERNAL, "outType is invalid for OneMap translation" );
    }
  }
...
#define INDEXTOCOLOR( index, color )                                     \
  {                                                                      \
    int shift;                                                           \
                                                                         \
    for ( shift = ( BITSPERXCOLOR - usefulBits ), color = 0; shift >= 0; \
          shift -= usefulBits )                                          \
      color |= ( ( index ) << shift );                                   \
                                                                         \
    /*                                                                   \
...
static Error getGrayMappedTranslation( Display *dpy, translationT *trans,
                                       int force );
static Error getOneMapTranslation( Display *dpy, translationT *trans,
                                   int force );
...
static Error delete_translation( Pointer p )
{
  ...
  if ( !d->dpy_object )
  {
    DXSetError( ERROR_INTERNAL, "null display object in delete_translation" );
    return ERROR;
  }

  if ( d->cmap && d->ownsCmap &&
       (Colormap)d->cmap !=
           XDefaultColormap( d->dpy, XDefaultScreen( d->dpy ) ) )
...
static int getShift( unsigned long mask )
{
  int inc;
...
static int getUsefulBits( unsigned long mask )
{
  int start, end;
...
static struct classes
{
  int class;
  char *name;
} Classes[] = {{PseudoColor, "PseudoColor"},
...
static Visual *getBestVisual( Display *dpy, int *depth, int *directMap )
{
  int count, i, j;
  ...
  if ( *depth )
  {
    if ( *depth == XDefaultDepth( dpy, screen ) )
    {
      ...
      if ( visual->class == PseudoColor || visual->class == DirectColor ||
           visual->class == TrueColor || visual->class == StaticColor )
      {
    ...
    for ( i = 0; !ret && aVis[i].depth != -1; i++ )
      if ( aVis[i].class == PseudoColor || aVis[i].class == DirectColor ||
           aVis[i].class == TrueColor || aVis[i].class == StaticColor )
      {
  ...
  *depth = XDefaultDepth( dpy, XDefaultScreen( dpy ) );
  ...
  {
    for ( i = 0; aVis[i].depth != -1; i++ )
      if ( ( *depth == aVis[i].depth ) && ( visual->class == aVis[i].class ) )
      {
...
static Object createTranslation( Private dpy_object, char *where,
                                 int desiredDepth, int *directMap,
                                 struct window *w )
{
  char cacheid[100];
  Private p = NULL;
  ...
  if ( !p )
  {
    ...
    if ( w && w->winFlag && w->windowMode == WINDOW_LOCAL &&
         ( ( !w->translation ) ||
           ...
           ( ( *directMap == 1 ) &&
             w->translation->translationType != NoTranslation ) ||
           ( ( *directMap == 0 ) &&
             w->translation->translationType == NoTranslation ) ) )
    {
      if ( w->gc )
     ...
     */
    dummyXImage =
        XCreateImage( dpy, visual, trans->depth, ZPixmap, 0, NULL, 1, 1, 8, 0 );
     ...
     */
    if ( visual != XDefaultVisual( dpy, dscrn ) ||
         ( *directMap &&
    ...
    else if ( visual->class == PseudoColor )
    {
      if ( !getOneMapTranslation( dpy, trans, visual->class == StaticColor ) )
      {
        ...
        if ( !xcmap )
        {
          DXSetError( ERROR_INTERNAL, "unable to create private colormap" );
          goto error;
        }
...
static Error getGamma( int depth, float *gamma )
{
  ...
  {
    case 8:
      str = getenv( "DXGAMMA_8BIT" );
      break;
    case 12:
      str = getenv( "DXGAMMA_12BIT" );
      break;
    case 15:
      str = getenv( "DXGAMMA_15BIT" );
      break;
    case 16:
      str = getenv( "DXGAMMA_16BIT" );
      break;
    case 24:
      str = getenv( "DXGAMMA_24BIT" );
      break;
    case 32:
      str = getenv( "DXGAMMA_32BIT" );
      break;
  };
  ...
  if ( !str || *gamma == 0 )
  {
    ...
    {
      case 8:
        *gamma = DXD_GAMMA_8BIT;
        break;
      case 12:
        *gamma = DXD_GAMMA_12BIT;
        break;
      case 15:
        *gamma = DXD_GAMMA_15BIT;
        break;
      case 16:
        *gamma = DXD_GAMMA_16BIT;
        break;
      case 24:
        *gamma = DXD_GAMMA_24BIT;
        break;
      case 32:
        *gamma = DXD_GAMMA_32BIT;
        break;
    };
  }
...
#define COFFSET                                       \
  1 /* offset to first color in colors[], this allows \
     * us to avoid having a 0 colors index            \
     */
static Error getDirectTranslation( Display *dpy, translationT *t )
{
  float invgamma;
  int i;
...
static Error getThreeMapTranslation( Display *dpy, translationT *d )
{
  ...
  else
  {
    for ( n = cmapsize, nn = 0; n > 0; n /= 2 )
      while ( XAllocColorPlanes( dpy, cmap, False, pixels + nn, n, 0, 0, 0,
      ...
      else
      {
        ...
        if ( rwriteable[pixelIndex] )
          save[saveI++] = n;
                      ...
                      [PIXELTOREDINDEX( colors[d->rtable[colorIndex]].pixel )] )
        {
    ...
    for ( n = COFFSET; n < cmapsize + COFFSET; n++ )
    {
      ...
      else
      {
        ...
        if ( gwriteable[pixelIndex] )
          save[saveI++] = n;
        else if ( gwriteable[PIXELTOGREENINDEX(
                     colors[d->gtable[colorIndex]].pixel )] )
        {
    ...
    for ( n = COFFSET; n < cmapsize + COFFSET; n++ )
    {
      ...
      else
      {
        ...
        if ( bwriteable[pixelIndex] )
          save[saveI++] = n;
        else if ( bwriteable[PIXELTOBLUEINDEX(
                     colors[d->btable[colorIndex]].pixel )] )
        {
...
#define SWAPLONG( s1, s2 )                               \
  ...
  {
    colors[n].pixel =
        REDINDEXTOPIXEL( n ) | GREENINDEXTOPIXEL( n ) | BLUEINDEXTOPIXEL( n );
  }
  ...
  for ( i = 0; i < cmapsize; i++ )
  {
    DXDebug( "X", "%d: %d %d %d", i, colors[i].red >> 8, colors[i].green >> 8,
             colors[i].blue >> 8 );
  }
  DXDebug( "X", "Tables:" );
  for ( i = 0; i < cmapsize; i++ )
  {
    DXDebug( "X", "%d: %d %d %d", i, PIXELTOREDINDEX( d->rtable[i] ),
...
static Error getOneMapTranslation( Display *dpy, translationT *d, int force )
{
  Visual *visual = (Visual *)d->visual;
  Colormap cmap = (Colormap)d->cmap;
  XColor colors[MAXCMAPSIZE];
  int i, j, best = 0, r, g, b, n, nn, max_nbrhd;
  int comp[MAXCMAPSIZE], uncomp[MAXCMAPSIZE];
  unsigned char readOnly[MAXCMAPSIZE];
  struct
  {
    unsigned char r, g, b;
  } map[MAX_RR * MAX_GG * MAX_BB];
...
#define REALLOCATE_COLOR( index, dst )                         \
  ...
  if ( nApprox )
    DXDebug( "X", "%d colors approximated (max diff %f)", nApprox,
             sqrt( maxdiff / ( 3.0 * 256.0 * 256.0 ) ) );
  else
...
int grayCmp( const void *p1, const void *p2 )
{
...
static Error getGrayMappedTranslation( Display *dpy, translationT *d,
                                       int force )
{
  Visual *visual = (Visual *)d->visual;
  Colormap cmap = (Colormap)d->cmap;
  ...
  {
    while (
        XAllocColorCells( dpy, cmap, False, NULL, 0, pixels + nAvailable, i ) )
      nAvailable += i;
  }
...
static Error getStaticGrayTranslation( Display *dpy, translationT *d )
{
  int i;
  Visual *visual = (Visual *)d->visual;
  float invgamma;
...
static Error getStaticColorTranslation( Display *dpy, translationT *d )
{
  int i;
  float invgamma;
  ...
  for ( maxGreen = 1, greenWidth = 0; greenBits & maxGreen;
        maxGreen <<= 1, greenWidth++ )
    ;
  ...
  for ( maxBlue = 1, blueWidth = 0; blueBits & maxBlue;
        maxBlue <<= 1, blueWidth++ )
    ;
...
static Error CheckColormappedImage( Object image, Array colormap )
{
  ...
  if ( colormap )
  {
    ...
    if ( ( n > 256 ) || ( t != TYPE_FLOAT ) || ( c != CATEGORY_REAL ) ||
         ( r != 1 ) || ( s[0] != 1 && s[0] != 3 ) )
    {
      DXSetError( ERROR_DATA_INVALID,
...
static Error getPrivateColormap( Display *dpy, translationT *trans,
                                 Colormap xcmap )
{
...
#if DEBUG
    DXMessage( "copy: %d) %d %d %d", ncolors[i].pixel, ncolors[i].red,
               ncolors[i].green, ncolors[i].blue );
#endif
  }
...
#if DEBUG
        DXMessage( "set: %d) %d %d %d", ncolors[i].pixel, ncolors[i].red,
                   ncolors[i].green, ncolors[i].blue );
#endif
      }
...
static Field _dxfCaptureDereferencedOneMapImage( struct window *w,
                                                 translationT *t, Visual *v )
{
  ...
  if ( image )
  {
    ...
    if ( pixsize == 1 )
    {
      ...
      ubyte *dst =
          ( (ubyte *)DXGetArrayData( c ) ) + ( w->wheight - 1 ) * dst_rowsize;
...
static Field _dxfCaptureDelayedColorImage( struct window *w, translationT *t,
                                           Visual *v )
{
...
static Field _dxfCaptureThreeMapImage( struct window *w, translationT *t,
                                       Visual *v )
{
  Field image = DXMakeImageFormat( w->wwidth, w->wheight, "BYTE" );
  if ( image )
  {
    ...
    for ( notSimple = 0, i = 0; i < v->map_entries; i++ )
    {
      ...
      if ( ( xmap[i].red != i ) || ( xmap[i].green != i ) ||
           ( xmap[i].blue != i ) )
        notSimple = 1;
    }
    ...
    if ( notSimple )
    {
      ...
      {
#if !defined( WORDS_BIGENDIAN )
        int rbyte = ( rshift == 0 ) ? 0 : ( rshift == 8 ) ? 1 : ( rshift == 16 )
                                                                    ? 2
                                                                    : 3;
        int gbyte = ( gshift == 0 ) ? 0 : ( gshift == 8 ) ? 1 : ( gshift == 16 )
                                                                    ? 2
                                                                    : 3;
        int bbyte = ( bshift == 0 ) ? 0 : ( bshift == 8 ) ? 1 : ( bshift == 16 )
                                                                    ? 2
                                                                    : 3;
#else
        int rbyte = ( rshift == 0 ) ? 3 : ( rshift == 8 ) ? 2 : ( rshift == 16 )
                                                                    ? 1
                                                                    : 0;
        int gbyte = ( gshift == 0 ) ? 3 : ( gshift == 8 ) ? 2 : ( gshift == 16 )
                                                                    ? 1
                                                                    : 0;
        int bbyte = ( bshift == 0 ) ? 3 : ( bshift == 8 ) ? 2 : ( bshift == 16 )
                                                                    ? 1
                                                                    : 0;
#endif
...
Field _dxf_CaptureXSoftwareImage( int depth, char *host, char *window )
{
  struct window *w = NULL;
  Private w_obj = NULL;
  int directMap = 0;
  ...
  switch ( v->class )
  {
    ...
    {
      int mapSize =
          ( v->map_entries > MAXCMAPSIZE ) ? MAXCMAPSIZE : v->map_entries;
...
Error DXGetSoftwareWindow( char *where, Window *window )
{
  Private p = NULL;
  char *cachetag = NULL;
  struct window *w;
