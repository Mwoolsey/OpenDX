...
Error DXVisualizeMemory( int which, int procid )
{
...
static void pix_set( struct dispinfo *d, Pointer start, ulong size,
                     ubyte color );
...
Error DXVisualizeMemory( int which, int procid )
{
  ...
  {/* small arena */
    ...
    {
      d_small = (struct dispinfo *)DXAllocateZero( sizeof( struct dispinfo ) );
      if ( !d_small )
        return ERROR;
  ...
  {/* large arena */
    ...
    else
    {
      ...
      d_large = (struct dispinfo *)DXAllocateZero( sizeof( struct dispinfo ) );
      if ( !d_large )
        return ERROR;
...
static void resize_display( struct dispinfo *d, int new_width, int new_height )
{
  int total_squares;
  uint h;
  char *cp;
  ...
  d->memory_image = XCreateImage(
      d->disp, XDefaultVisual( d->disp, XDefaultScreen( d->disp ) ), 8, ZPixmap,
      0, (char *)d->memory_pixel, d->win_width, d->win_height, 8, 0 );

  XResizeWindow( d->disp, d->wind, d->win_width, d->win_height );
  XStoreName( d->disp, d->wind, d->title );
}

static void pix_set( struct dispinfo *d, Pointer memstart, ulong memsize,
                     ubyte color )
{
  ...
  start = ( ( (ubyte *)memstart ) - ( (ubyte *)d->arena_base ) ) / d->bytes_vp *
          d->pixel_xmult;
  ...
  while ( size > 0 )
  {
    ...
    for ( i = 0; i < d->pixel_ymult; i++ )
    {
      ...
      if ( ( where + part ) >= (ulong)d->memory_pixel + WinSize( d ) )
        part -=
            ( where + part ) - ( (ulong)d->memory_pixel + WinSize( d ) + 1 );
...
static Error memquery( int blocktype, Pointer start, ulong size, Pointer p )
{
  ...
  DXMessage(
      "  %4d%s%15s from 0x%08x to 0x%08x", size, cp,
...
static Error report_memory( struct dispinfo *d )
{
   ...
   */
  XPutImage( d->disp, d->wind, d->gc, d->memory_image, 0, 0, 0, 0, d->win_width,
             d->win_height );
  XFlush( d->disp );
...
static Error query_memory( struct dispinfo *d )
{
  ...
  if ( d->memory_pixel[pixel] == color_free )
    DXMessage( "no allocated memory blocks between 0x%08x and 0x%08x", qstart,
               qend );
  else if ( ( d->memory_pixel[pixel] == color_alloc ) ||
            ( d->memory_pixel[pixel] == color_smallalloc ) )
  {
    DXMessage( "allocated memory blocks in the range: 0x%08x to 0x%08x", qstart,
               qend );
    d->query_start = qstart;
    d->query_end = qend;

    if ( d->which == MEMORY_LOCAL )
      DXDebugLocalAlloc( d->nproc, MEMORY_ALLOCATED | MEMORY_FREE, memquery,
                         (Pointer)d );
    else
      DXDebugAlloc( d->which, MEMORY_ALLOCATED | MEMORY_FREE, memquery,
                    (Pointer)d );
  }
...
static int XChecker( int i, void *d )
{
...
static Error init_memory_visual( struct dispinfo *d )
{
  ...
  d->wind = XCreateSimpleWindow(
      d->disp, DefaultRootWindow( d->disp ), x, y, w, h, border,
      WhitePixel( d->disp, DefaultScreen( d->disp ) ),
      BlackPixel( d->disp, DefaultScreen( d->disp ) ) );

  XSetStandardProperties( d->disp, d->wind, d->title, d->title, None, NULL, 0,
                          &hint );

  XSelectInput( d->disp, d->wind, ExposureMask | StructureNotifyMask |
  ...
  DXRegisterInputHandlerWithCheckProc( (Handler)handler_script,
                                       (Checker)XChecker, d->fd, (Pointer)d );
  ...
  while ( XPending( d->disp ) )
  {
    ...
    switch ( event.type )
    {
      ...
      case ConfigureNotify:
        ...
        {
          resize_display( d, event.xconfigure.width, event.xconfigure.height );
          if ( xerror )
            goto error;
          d->refresh = 1;
        }
