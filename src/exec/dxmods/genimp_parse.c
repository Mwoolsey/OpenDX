...
/***********************************************************************/
/*
 * $Header: /src/master/dx/src/exec/dxmods/genimp_parse.c,v 1.9 2002/03/21
 * 02:57:35 rhh Exp $
 */
...
static Error readdesc( char **, struct parse_state *, int *, ByteOrder *,
                       Type *, char * );
...
static struct parameter para[] = {{"file", infoFile, 0},
                                  {"grid", infoGrid, 0},
                                  {"series", infoSeries, 0},
                                  {"field", infoField, 0},
...
static struct type_spec
{
  ...
  char *type_name;
      ...
      {TYPE_UBYTE, "byte"}, {TYPE_UBYTE, "ubyte"}, {TYPE_BYTE, "sbyte"},
      {TYPE_SHORT, "short"}, {TYPE_USHORT, "ushort"}, {TYPE_INT, "int"},
      {TYPE_UINT, "uint"}, {TYPE_STRING, "string"}, {TYPE_FLOAT, "float"},
      {TYPE_DOUBLE, "double"},
...
Error _dxf_gi_InputInfoTable( char *table, char *format, FILE **datafp )
{
  FILE *fp;
  char *p, *headerfile;
  int i;
    ...
    { /* A non-blank line */
      ...
      for ( i = 0; i < NUM_PARS; i++ )
      {
        ...
        else if ( !strcmp( para[i].name, p ) )
        {
          ...
          {
            DXWarning( "Ignoring extra '%s' statement in '%s'", para[i].name,
                       headerfile );
            i = NUM_PARS; /* Exit for loop */
          }
          else
...
static Error CheckInputs( void )
{
  ...
  if ( PARAM_SPECIFIED( PARAM_Depend ) )
  {
    ...
    {
      if ( _dxd_gi_interleaving == IT_FIELD &&
           firstdep != _dxd_gi_var[f]->datadep )
      {
  ...
  if ( PARAM_SPECIFIED( PARAM_RecSeparat ) )
  {
    ...
    if ( _dxd_gi_interleaving == IT_RECORD &&
         PARAM_SPECIFIED( PARAM_Structure ) )
    {
    ...
    if ( rec_sep[1]->type == SKIP_NOTHING )
    {
      ...
      for ( f = 2; f < n; f++ )
      {
        if ( ( rec_sep[f] = (struct header *)DXAllocate(
                   sizeof( struct header ) ) ) == NULL )
          goto error;
...
static Error MakeupDefault( char *name, int linenum )
{
  int i, j, n = 0;
  char s[10];
  int dim;
  ...
  if ( _dxd_gi_interleaving == IT_RECORD )
  {
    if ( PARAM_SPECIFIED( PARAM_RecSeparat ) )
    {
      for ( i = 0; i < _dxd_gi_numflds; i++ )
      {
        if ( _dxd_gi_var[i]->datatype == TYPE_STRING )
          dim = 1;
        else
          dim = _dxd_gi_var[i]->datadim;
        for ( j = 0; j < dim; j++ )
        {
...
static Error infoField( struct parse_state *ps )
{
  ...
  while ( FileTok( ps, ", \n\t", NO_NEWLINE, &p ) && p )
  {
    ...
    {
      if ( ( _dxd_gi_var[myGI_numflds] = (struct variable *)DXAllocate(
                 sizeof( struct variable ) ) ) == NULL )
        goto error;
    }
...
static Error infoStructure( struct parse_state *ps )
{
  char *p;
  int n = 0;
  ...
  while ( FileTok( ps, "=, \n\t", NO_NEWLINE, &p ) && p )
  {
    ...
    {
      if ( ( _dxd_gi_var[n] = (struct variable *)DXAllocate(
                 sizeof( struct variable ) ) ) == NULL )
        goto error;
    }
    if ( !strcmp( p, "scalar" ) )
...
static Error infoFormat( struct parse_state *ps )
{
  char *p;
...
static Error get_format( struct parse_state *ps, char **p, int *format,
                         ByteOrder *order )
{
  char *t;
  ...
  *p = t;
  ...
  if ( *order != BO_UNKNOWN )
  {/* msb/lsb specified, get next token */
    if ( !FileTok( ps, " ,\n\t", NO_NEWLINE, p ) )
    {
      DXAddMessage( "#10865", "format" );
      goto error;
...
/*
    FN: read in the datatype of each variable. If the variables haven't been
        constructed, then DXAllocate memory for variables and update the number
   of fields. Otherwise do the compatibility checking.
    EF: return the variables with datatype and databytes been set.
*/

static Error infoType( struct parse_state *ps )
{
  char *p;
  int n = 0;
  int bytes;
...
static Error get_type( struct parse_state *ps, char **p, Type *type,
                       int *bytes )
{
  char signedtype[7];
  int i, found;
  ...
  if ( !strcmp( *p, "unsigned" ) )
  {
    ...
    if ( !strcmp( *p, "byte" ) )
      strcpy( signedtype, "ubyte" );
    else if ( !strcmp( *p, "int" ) )
      strcpy( signedtype, "uint" );
    else if ( !strcmp( *p, "short" ) )
      strcpy( signedtype, "ushort" );
    else
    {
...
*/
static Error infoRecSeparat( struct parse_state *ps )
{
  char *p;
  int n = 0;

  if ( ( rec_sep = (struct header **)DXAllocate(
             sizeof( struct header * ) * max_vars * MAX_POS_DIMS ) ) == NULL )
    goto error;
  ...
  for ( n = 0;; n++ )
  {
    ...
    if ( ( rec_sep[n] = (struct header *)DXAllocate(
               sizeof( struct header ) ) ) == NULL )
      goto error;
    if ( !p )
    {
...
*/
static Error infoLayout( struct parse_state *ps )
{
  char *p;
  int n = 0;
...
static Error infoMajority( struct parse_state *ps )
{
  char *p;
...
static Error infoDepend( struct parse_state *ps )
{
  char *p;
  int n = 0;
...
*/
static Error infoHeader( struct parse_state *ps )
{
  char *p;
...
*/
static Error get_header( struct parse_state *ps, char *p, struct header *head )
{
...
/*
  ...
  */
static Error infoInterleaving( struct parse_state *ps )
{
  char *p;
...
*/
static Error infoBlock( struct parse_state *ps )
{
  char *p;
  int n = 0;
...
/*
  FN: read in the locations data from infomation table to _dxd_gi_dataloc[].
  construct
      locations variable _dxd_gi_var[locidx] when positions non-PRODUCT.
  otherwise
      construct variables _dxd_gi_var[locidx..locidx+_dxd_gi_numdims-1] for
  every axis.
  EF: return with valid locations variable(s) and update locidx.
*/

static Error infoPositions( struct parse_state *ps, struct place_state *dataps )
{
  char *p;
  ...
  if ( !isdigit( *p ) && *p != '.' && *p != '-' && strncmp( p, "reg", 3 ) &&
       strncmp( p, "irr", 3 ) )
  {
    if ( _dxd_gi_fromfile )
    {
    ...
    else if ( ( _dxd_gi_fromfile = (struct infoplace **)DXAllocate(
                    sizeof( struct infoplace * ) * MAX_POS_DIMS * 3 ) ) ==
              NULL )
      goto error;
    ...
    if ( ( _dxd_gi_dataloc =
               (float *)DXAllocate( posnums * sizeof( float ) ) ) == NULL )
      goto error;
    return OK;
  }
  ...
  if ( isalpha( *p ) )
  {
    ...
    n = 0;
    if ( ( _dxd_gi_dataloc = (float *)DXAllocate( posnums * _dxd_gi_numdims *
                                                  sizeof( float ) ) ) == NULL )
      goto error;
    do
    {/* DXRead the numbers */
  ...
  {/* Parse a string of numbers that can span more than 1 line */
    ...
    if ( size == 1 )
      tempi2 = 2 * _dxd_gi_numdims;
    else
      tempi2 = size * _dxd_gi_numdims;
    if ( ( _dxd_gi_dataloc = (float *)DXAllocate( 2 * _dxd_gi_numdims *
                                                  sizeof( float ) ) ) == NULL )
      goto error;
    do
    {/* DXRead the numbers */
      ...
      if ( n == tempi1 )
      {
        if ( ( _dxd_gi_dataloc = (float *)DXReAllocate(
                   (Pointer)_dxd_gi_dataloc,
                   size * _dxd_gi_numdims * sizeof( float ) ) ) == NULL )
          goto error;
      }
      if ( n >= tempi2 )
      {
...
static Error infoFile( struct parse_state *ps )
{
  char *current = ps->current;
  char *p;
...
static Error infoPoints( struct parse_state *ps )
{
  char *p;
  int max_values = 0;
  Type type = TYPE_INT;
  ...
  {/* points read from file */
    ...
    if ( !_dxd_gi_fromfile &&
         ( _dxd_gi_fromfile = (struct infoplace **)DXAllocate(
               sizeof( struct infoplace * ) * MAX_POS_DIMS * 3 ) ) == NULL )
      goto error;
...
static Error infoGrid( struct parse_state *ps )
{
  char *p;
  ...
  {/* grid dimensions are contained in the data file */
    if ( !_dxd_gi_fromfile &&
         ( _dxd_gi_fromfile = (struct infoplace **)DXAllocate(
               sizeof( struct infoplace * ) * MAX_POS_DIMS * 3 ) ) == NULL )
      goto error;
...
/*
  ...
  */
static Error infoSeries( struct parse_state *ps )
{
  char *p;
  int i = 0;
  ...
  if ( !( _dxd_gi_seriesvalues =
              (float *)DXAllocate( _dxd_gi_series * sizeof( float ) ) ) )
    goto error;
...
/*
  ...
  */
static Error infoEnd( struct parse_state *ps )
{
  return OK;
}

static Error parse_marker( char *p, char marker[MAX_MARKER] )
{
  int i;
  unsigned int c;
  ...
  marker[0] = '\0';
  if ( *p == '"' )
  {/* DXCopy everything inside of the two "'s */
    p++;
  ...
  {/* DXCopy everything until the first white space or , */
    for ( i = 0; ( i < MAX_MARKER ) && *p && ( *p != ' ' ) && ( *p != '\t' ) &&
                     ( *p != ',' );
          i++ )
    {
      if ( !nextchar( &p, &c ) )
 ...
 */
static Error parse_template( char *format, char **filename )
{
  char *file;
  ...
  if ( !*format )
    return OK; /* just space, no template */
 ...
 */
static Error readdesc( char **p, struct parse_state *ps, int *format,
                       ByteOrder *order, Type *type, char *statement )
{
  int bytes, form;
  ByteOrder or;
...
static Error readplacement( char **p, struct parse_state *ps, int which,
                            int *max_values, char *statement )
{
  int n = 0, bytes;
  ...
  t = fromfile->type;
  if ( ( fromfile->skip = (int *)DXAllocate( MAX_POS_DIMS * sizeof( int ) ) ) ==
       NULL )
    goto error;
  if ( ( fromfile->width =
             (int *)DXAllocate( MAX_POS_DIMS * sizeof( int ) ) ) == NULL )
    goto error;
  bytes = DXTypeSize( t );
...
static Error parse_datafile( struct place_state *dataps )
{
  int i, j = 0, k = 0, num;
  int read_header;
  float f = 0;
  ...
  if ( read_header == 1 )
  {/* header marker was already read */
  ...
  for ( i = 0; _dxd_gi_fromfile[i]; i++ )
  {
    ...
    {
      if ( (float)(int)( k / 2.0 ) == (float)k / 2.0 )
        _dxd_gi_dataloc[k++] = 0.0;
      else
        _dxd_gi_dataloc[k++] = 1.0;
    }
    else
    {
 ...
 */
static Error InitFileTok( struct parse_state *ps, FILE *fp, char *filename )
{
 ...
 * the token.
 */
static Error FileTok( struct parse_state *ps, char *sep, int newline,
                      char **token )
{
  char *current = ps->current;
  char *tp = &ps->token[0];
...
static Error _dxf_getline( struct parse_state *ps )
{
  ...
  while ( (int)strlen( str ) > MAX_DSTR - 2 )
  {
    ...
    n++;
    line = (char *)DXReAllocate( line,
                                 (unsigned int)strlen( str ) + n * MAX_DSTR );
    strcat( line, str );
  }
...
static Error getmorevar( int *num )
{
  int i, oldnum = *num;

  *num = *num + *num * .5;
  if ( ( _dxd_gi_var = (struct variable **)DXReAllocate(
             (Pointer)_dxd_gi_var, sizeof( struct variable * ) * *num ) ) ==
       NULL )
    return ERROR;
