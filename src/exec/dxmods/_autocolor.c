...
static Error AutoColorDelayedObject( Object, int, float, float, int, float,
                                     ...
                                     RGBColor );
static Error RecurseToIndividual( Object, float, float, float, float, float,
                                  float *, float *, Object *, int, RGBColor,
                                  RGBColor );
static Error MakeMapAndColor( Object, float, float, float, float, float,
                              float *, float *, Object *, int, RGBColor,
                              RGBColor );
                     ...
                     RGBColor colormin, RGBColor colormax )
{
  ...
  if ( !( RecurseToIndividual( (Object)g_out, opacity, intensity, phase, range,
                               saturation, inputmin, inputmax, outcolorfield,
...
static Error RecurseToIndividual( Object g_out, float opacity, float intensity,
                                  float phase, float range, float saturation,
                                  float *inputmin, float *inputmax,
                                  Object *outcolormap, int delayed,
        ...
        */
        ...
        default:
          ...
          if ( !( outcolorgroup = DXNewGroup() ) )
            return ERROR;
          for ( i = 0;
                ( subo = DXGetEnumeratedMember( (Group)g_out, i, NULL ) ); i++ )
          {
...
static Error MakeMapAndColor( Object g_out, float opacity_i, float intensity_i,
                              float phase, float range, float saturation,
                              float *inputmin, float *inputmax,
                              Object *outcolorfield, int delayed,
                              RGBColor colorvecmin, RGBColor colorvecmax )

{
  ...
  if ( !( _dxfIsVolume( (Object)g_out, &surface ) ) )
    return ERROR;
  ...
  if ( !( surface ) )
  {
    if ( !( _dxfBoundingBoxDiagonal( (Object)g_out, &thickness ) ) )
      return ERROR;
  }
  ...
  if ( surface )
  {
    if ( !_dxfRGBtoHSV( colorvecmin.r, colorvecmin.g, colorvecmin.b, &h, &s,
                        &v ) )
      return ERROR;
    lowerhsv = DXRGB( h, s, v );
    if ( !_dxfRGBtoHSV( colorvecmax.r, colorvecmax.g, colorvecmax.b, &h, &s,
                        &v ) )
      return ERROR;
  ...
  {
    /* get data statistices */
    if ( !DXStatistics( (Object)g_out, "data", &minvalue, &maxvalue, &avg,
                        &standdev ) )
    {
      return ERROR;
    }
  ...
  if ( ( givenmin > maxvalue ) || ( givenmax < minvalue ) )
  {
    ...
    if ( surface )
    {
      if ( setopacities )
      {
        oorigin = opacity;
        /*odelta = 0.0;*/
        if ( !( a_opacitydata = (Array)DXNewConstantArray(
                    count, ( Pointer ) & oorigin, TYPE_FLOAT, CATEGORY_REAL, 0,
                    NULL ) ) )
        {
          goto cleanup;
        }
      ...
      if ( !( a_colordata = (Array)DXNewConstantArray(
                  count, ( Pointer ) & corigin, TYPE_FLOAT, CATEGORY_REAL, 1,
                  3 ) ) )
      {
        goto cleanup;
      }
    ...
    {
      if ( !( AutoColorDelayedObject( (Object)g_out, setopacities, givenmin,
                                      givenmax, surface, opacity, huestart,
                                      range, saturation, intensity, lowerrgb,
                                      upperrgb ) ) )
      {
        goto cleanup;
      }
  ...
  else if ( ( minvalue == givenmin ) && ( minvalue == maxvalue ) &&
            ( maxvalue == givenmax ) )
  {
    count = 1;
    /* make simple maps of opacity and color */
    if ( surface )
    {
      ...
      if ( !( _dxfHSVtoRGB( huestart, saturation, intensity, &red, &green,
                            &blue ) ) )
      {
        goto cleanup;
      }
  ...
  else
  {
    if ( !delayed )
    {
      ...
      if ( setopacities )
      {
        if ( compactopacities )
        {
          ...
          /*odelta = 0.0;*/
          if ( !( a_opacitydata = (Array)DXNewConstantArray(
                      count, ( Pointer ) & oorigin, TYPE_FLOAT, CATEGORY_REAL,
                      0, NULL ) ) )
          {
            goto cleanup;
          }
        ...
        else
        {
          ...
          if ( !( DXSetComponentAttribute(
                    (Field)OpacityField, "data", "dep",
                    (Object)DXNewString( "positions" ) ) ) )
            goto cleanup;

          if ( !( DXSetComponentAttribute(
                    (Field)OpacityField, "connections", "ref",
                    (Object)DXNewString( "positions" ) ) ) )
            goto cleanup;
      ...
      if ( compactopacities )
      {
        if ( !( _dxfAutoColorObject( (Object)g_out, (Object)newcolorfield,
                                     (Object)a_opacitydata ) ) )
        {
          goto cleanup;
        }
    ...
    else
    {
      ...
      colorfield_positions =
          (Array)DXNewArray( TYPE_FLOAT, CATEGORY_REAL, 1, 1 );
      ...
      for ( ii = givenmin; ii <= givenmax; ii++ )
      {
        colorfield_posptr[ii - datamin] = (float)ii;
        hue =
            huestart + ( ( ii - givenmin ) / ( givenmax - givenmin ) ) * range;
...

{
  ...
  if ( ( !scalar ) || ( !floatflag ) )
  {
    ...
    if ( ( ( OpacityMap ) &&
           ( DXGetObjectClass( OpacityMap ) != CLASS_ARRAY ) ) ||
  ...
  if ( OpacityMap )
  {
    ...
    {
      if ( !DXMapCheck( (Object)f, OpacityMap, "data", NULL, NULL, NULL,
                        NULL ) )
      {
        goto error;
      }
  ...
  if ( DXGetComponentValue( (Field)f, "front colors" ) )
    if ( !( DXDeleteComponent( (Field)f, "front colors" ) ) )
      goto error;
  if ( DXGetComponentValue( (Field)f, "back colors" ) )
    if ( !( DXDeleteComponent( (Field)f, "back colors" ) ) )
      goto error;
  ...
  if ( savedfield )
  {
     ...
     * delete savedfield */
    f = (Field)DXReplace( (Object)f, (Object)savedfield, "data", "data" );
    if ( !f )
    {
  ...
  if ( OpacityMap )
    DXDeleteComponent( f, "opacity map" );
  if ( !DXEndField( f ) )
    goto error;
  return OK;
...
static Error AutoColorDelayedObject( Object o, int setopacities, float givenmin,
                                     float givenmax, int surface, float opacity,
                                     float huestart, float range,
                                     ...
                                     RGBColor lowerrgb, RGBColor upperrgb )
{
  ...
  switch ( DXGetObjectClass( o ) )
  {
    case CLASS_GROUP:
      ...
      for ( i = 0; ( subo = DXGetEnumeratedMember( (Group)o, i, NULL ) ); i++ )
      {
        if ( !AutoColorDelayedObject(
                  subo, setopacities, givenmin, givenmax, surface, opacity,
                  huestart, range, saturation, intensity, lowerrgb, upperrgb ) )
          return ERROR;
      }
      break;
    case CLASS_FIELD:
      ...
      /* and schedule autocolorfield to be called */
      if ( !( DXAddTask( AutoColorDelayedField, ( Pointer ) & a, sizeof( a ),
                         1.0 ) ) )
      {
        return ERROR;
      }
...

{
  ...
  for ( i = givenmin; i <= givenmax; i++ )
  {
    if ( givenmin == givenmax )
      ...
      hue = huestart + ( i - givenmin ) / ( givenmax - givenmin ) * range;
    if ( !_dxfHSVtoRGB( hue, saturation, intensity, &r, &g, &b ) )
      goto error;
  ...
  if ( !( DXAddArrayData( deferredcolors, 0, numentries,
                          (Pointer)colorarray ) ) )
    goto error;
  ...
  if ( setopacities )
  {
    if ( !( deferredopacities =
                (Array)DXNewArray( TYPE_FLOAT, CATEGORY_REAL, 0, NULL ) ) )
      goto error;
  ...
  if ((type != TYPE_UBYTE)&&(type != TYPE_BYTE)&&(type != TYPE_INT)&&
      (type != TYPE_UINT)&&(type != TYPE_SHORT)&&(type != TYPE_USHORT)) {
         DXSetError(ERROR_BAD_PARAMETER,
                    "delayed option only valid for byte, integer, and short
  scalar data");
         return ERROR;
  }
  */
...
int _dxfIsVolume( Object g_out, int *surface )
{
  ...
  *surface = 0;
  ...
  el_type = NULL;
  if ( ( el_type = DXGetString( (String)DXGetComponentAttribute(
             (Field)f, "connections", "element type" ) ) ) != NULL )
  {
...
Error _dxfHSVtoRGB( float h, float s, float v, float *red, float *green,
                    float *blue )
     ...
     by Foley and Van Dam, 1984   */
{
  ...
  else
  {
    ...
    switch ( i )
    {
      case 0:
        *red = v * valfactor;
        *green = t * valfactor, *blue = p * valfactor;
        break;
      case 1:
        *red = q * valfactor;
        *green = v * valfactor, *blue = p * valfactor;
        break;
      case 2:
        *red = p * valfactor;
        *green = v * valfactor, *blue = t * valfactor;
        break;
      case 3:
        *red = p * valfactor;
        *green = q * valfactor, *blue = v * valfactor;
        break;
      case 4:
        *red = t * valfactor;
        *green = p * valfactor, *blue = v * valfactor;
        break;
      case 5:
        *red = v * valfactor;
        *green = p * valfactor, *blue = q * valfactor;
        break;
    };
  }
  return OK;
...
Error RGBtoHLS( float r, float g, float b, float *h, float *l, float *s )
/*   algorithm is from Fundamentals of Interactive Computer Graphics
     by Foley and Van Dam, 1984   */
{
  float min, max, delta;
...
Error HLStoRGB( float h, float l, float s, float *r, float *g, float *b )
/*   algorithm is from Fundamentals of Interactive Computer Graphics
     by Foley and Van Dam, 1984   */
{
  float m2, m1;
...
Error _dxfRGBtoHSV( float r, float g, float b, float *h, float *s, float *v )
/*   algorithm is from Fundamentals of Interactive Computer Graphics
     by Foley and Van Dam, 1984   */
{
...
/* converts an hsv color map to an rgb color map */
{
  ...
  if ( !( datadep = DXGetString(
              (String)DXGetComponentAttribute( mapfield, "data", "dep" ) ) ) )
  {
  ...
  /* get size and shape */
  if ( !( DXGetArrayInfo( a_data, &countdata, &datatype, &category, &rank,
                          &shape ) ) )
    goto cleanup;
  ...
  /* make the new arrays*/
  if ( !( a_newdata = DXNewArray( TYPE_FLOAT, CATEGORY_REAL, 1, 3 ) ) )
    goto cleanup;
  if ( !( a_newpositions = DXNewArray( TYPE_FLOAT, CATEGORY_REAL, 1, 1 ) ) )
  {
  ...
  /* get size and shape */
  if ( !( DXGetArrayInfo( a_positions, &countpos, &type, &category, &rank,
                          &shape ) ) )
    goto cleanup;
  ...
  if ( deppos )
  {
    ...
    lastpos = pp[0];
    if ( !( _dxfHSVtoRGB( lasthue, lastsat, lastval, &red, &green, &blue ) ) )
      goto cleanupnofield;

    newcolor = DXRGB( red, green, blue );
    if ( !( DXAddArrayData( a_newdata, icountnew, 1,
                            ( Pointer ) & newcolor ) ) )
      goto cleanupnofield;
    if ( !( DXAddArrayData( a_newpositions, icountnew, 1,
                            ( Pointer ) & lastpos ) ) )
      goto cleanupnofield;
    icountnew++;
    icountoriginal++;
    ...
    for ( i = 3; i < 3 * countpos; i = i + 3 )
    {
      ...
      else
      {
        ...
        numnew2 = 0;
        numnew3 = 0;
        if ( abshue > maxhuediff )
          numnew1 = abshue / maxhuediff;
        if ( abssat > maxsatdiff )
          numnew2 = abssat / maxsatdiff;
        if ( abshue > maxvaldiff )
          numnew3 = absval / maxvaldiff;
        numnew = 1;
        if ( numnew1 > numnew )
          numnew = numnew1;
        if ( numnew2 > numnew )
        ...
        for ( j = 1; j < numnew; j++ )
        {
          ...
          if ( huestepdiff != 0 )
          {
            ...
            if ( !( _dxfHSVtoRGB( tmphue, tmpsat, tmpval, &red, &green,
                                  &blue ) ) )
              goto cleanupnofield;
            newcolor = DXRGB( red, green, blue );
  ...
  else
  {
    ...
    if ( !( a_newconnections = DXMakeGridConnectionsV( 1, &countpos ) ) )
      goto cleanup;
  }
  ...
  if ( !( DXSetComponentValue( (Field)newmapfield, "positions",
  ...
  if ( !( DXSetComponentAttribute( (Field)newmapfield, "connections", "ref",
                                   (Object)DXNewString( "positions" ) ) ) )
    goto cleanup;
  if ( !( DXSetComponentAttribute( (Field)newmapfield, "connections",
                                   "element type",
                                   (Object)DXNewString( "lines" ) ) ) )
    goto cleanup;
...
/* converts an hsv color map to an rgb color map */
{
  ...
  /* get size and shape */
  if ( !( DXGetArrayInfo( a_data, &count, &type, &category, &rank, &shape ) ) )
    goto cleanupnofield;
  if ( ( rank != 1 ) || ( category != CATEGORY_REAL ) ||
       ( type != TYPE_FLOAT ) || ( shape != 3 ) )
  {
    DXSetError(
        ERROR_BAD_PARAMETER,
  ...
  /* get size and shape */
  if ( !( DXGetArrayInfo( a_positions, &count, &type, &category, &rank,
                          &shape ) ) )
    goto cleanup;
  if ( ( category != CATEGORY_REAL ) || ( type != TYPE_FLOAT ) )
  {
  ...
  for ( i = 3; i < 3 * count; i = i + 3 )
  {
         ...
         ( ABS( dp[i + 2] - lastb ) > maxsteprgb ) )
    {
      ...
      if ( ABS( dp[i + 1] - lastg ) > biggest )
        biggest = ABS( dp[i + 1] - lastg );
      if ( ABS( dp[i + 2] - lastb ) > biggest )
        biggest = ABS( dp[i + 2] - lastb );
      ...
      for ( j = 1; j < numnewsteps; j++ )
      {
        ...
        {
          if ( !( _dxfRGBtoHSV( newr, newg, newb, &hue, &saturation,
                                &value ) ) )
            goto cleanupnofield;
          newcolor = DXRGB( hue, saturation, value );
          if ( !( DXAddArrayData( a_newdata, icountnew, 1,
      ...
      if ( !( _dxfRGBtoHSV( dp[i], dp[i + 1], dp[i + 2], &hue, &saturation,
                            &value ) ) )
        goto cleanupnofield;
      newcolor = DXRGB( hue, saturation, value );
      newpos = pp[i / 3];
...
Error _dxfSetMultipliers( Object g_out, float unitvalue, float unitopacity )
{
  ...
  if ( class == CLASS_GROUP )
  {
    ...
    {
      for ( i = 0; ( subo = DXGetEnumeratedMember( (Group)g_out, i, NULL ) );
            i++ )
      {
        if ( !( DXSetAttribute( (Object)subo, "color multiplier",
                                (Object)DXAddArrayData(
                                    DXNewArray( TYPE_FLOAT, CATEGORY_REAL, 0 ),
