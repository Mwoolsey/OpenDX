...
static Error AutoGrayScaleDelayedObject( Object, int, float, float, int, float,
...
static Error RecurseToIndividual( Object, float, float, float, float, float,
                                  ...
                                  RGBColor );
static Error MakeMapAndColorGray( Object, float, float, float, float, float,
                                  float *, float *, Object *, int, RGBColor,
                                  RGBColor );
...
static Pointer AllocateBest( int n )
{
  Pointer p;
...
Group _dxfAutoGrayScale( Object g, float opacity, float hue, float intensity,
                         ...
                         RGBColor colorvecmin, RGBColor colorvecmax )
{
  ...
  if ( !( RecurseToIndividual( (Object)g_out, opacity, hue, intensity, range,
...
static Error RecurseToIndividual( Object g_out, float opacity, float hue,
  ...
  switch ( class )
  {
    case CLASS_GROUP:
      ...
      switch ( groupclass )
      {
        ...
        case CLASS_COMPOSITEFIELD:
          if ( !( MakeMapAndColorGray( g_out, opacity, hue, phase, range,
        ...
        default:
          ...
          if ( !( outcolorgroup = (Group)DXNewGroup() ) )
            return ERROR;
          for ( i = 0; ( subo = (Object)DXGetEnumeratedMember( (Group)g_out, i,
                                                               NULL ) );
                i++ )
          {
    ...
    case CLASS_SCREEN:
      if ( !( DXGetScreenInfo( (Screen)g_out, &subo, NULL, NULL ) ) )
        return ERROR;
      if ( !RecurseToIndividual( subo, opacity, hue, phase, range, saturation,
                                 inputmin, inputmax, outcolormap, delayed,
                                 colorvecmin, colorvecmax ) )
        return ERROR;
      break;
    case CLASS_CLIPPED:
    ...
    default:
      ...
      DXSetError( ERROR_BAD_CLASS,
                  "not a group, field, xform or screen object" );
      return ERROR;
  }
  return OK;
}
...

{
  ...
  if ( ( givenmin > maxvalue ) || ( givenmax < minvalue ) )
  {
    ...
    else
    {
      ...
      if ( !( AutoGrayScaleDelayedObject( (Object)g_out, setopacities, givenmin,
            ...
            ( maxvalue == givenmax ) )
  {
    ...
    if ( surface )
    {
      ...
      if ( !( _dxfHSVtoRGB( hue, saturation, intensitystart, &red, &green,
                            &blue ) ) )
        goto cleanup;
...
static Error AutoGrayScaleDelayedObject( Object o, int setopacities,
                                         float givenmin, float givenmax,
                                         int surface, float opacity,
                                         float intensitystart, float range,
                                         ...
                                         RGBColor lowerrgb, RGBColor upperrgb )
{
  ...
  switch ( DXGetObjectClass( o ) )
  {
    case CLASS_GROUP:
      ...
      for ( i = 0; ( subo = DXGetEnumeratedMember( (Group)o, i, NULL ) ); i++ )
      {
        if ( !AutoGrayScaleDelayedObject(
                  subo, setopacities, givenmin, givenmax, surface, opacity,
                  intensitystart, range, saturation, hue, lowerrgb, upperrgb ) )
          return ERROR;
      }
      break;
    case CLASS_FIELD:
      ...
      if ( !( DXAddTask( AutoGrayScaleDelayedField, ( Pointer ) & a,
                         sizeof( a ), 1.0 ) ) )
      {
        return ERROR;
      }
...

{
  ...
  for ( i = givenmin; i <= givenmax; i++ )
  {
    if ( givenmin == givenmax )
      ...
      intensity =
          intensitystart + ( i - givenmin ) / ( givenmax - givenmin ) * range;
    if ( !_dxfHSVtoRGB( hue, saturation, intensity, &r, &g, &b ) )
      return ERROR;
  ...
  if ( setopacities )
  {
    ...
    if ( !( DXAddArrayData( deferredopacities, 0, 256,
