...
static int SimplifySurface_worker(
    Field in_field, int p_knt, int p_dim, float *p_positions, int c_knt,
    int c_nv, int *c_connections, float *old_positional_error,
    int data_dimension, float *original_surface_data, float *max_error,
    float *max_data_error, int *preserve_volume, int *simplify_boundary,
    int *preserve_boundary_length, int *preserve_data, int *statistics,
...
Error m_SimplifySurface( Object *in, Object *out )
{
  int i;
  ...
  if ( !DXInvalidateUnreferencedPositions( new_in[0] ) )
    goto error;
  if ( !DXCull( new_in[0] ) )
    goto error;
  ...
  if ( new_in[0] != in[0] && new_in[0] != NULL )
    DXDelete( new_in[0] );
...
/*
   traverse() explores recursively the object hierarchy, runs each object
   through
   ...
   Apparently, DXCopy cannot be used for this purpose, even using the
   DX_COPY_STRUCTURE
   parameter, because the positions and connections could not be changed when
   using
   DXCopy()
*/
...
static Error traverse( Object *in, Object *out )
{
  ...
  switch ( DXGetObjectClass( in[0] ) )
  {
    ...
    case CLASS_GROUP
        : /* if we encounter a group or a series, we need to recreate
    ...
    {
      int is_series = 0, i, inputknt, memknt;

      float position;

      Group new_group; /* create a new group to avoid type
      ...
      /* other inputs are values: pass references to the other input objects
         other
      ...
      for ( i = 0; i < memknt; i++ )
      {
        ...
        if ( is_series )
          DXSetSeriesMember( (Series)new_group, i, (double)position,
                             new_out[0] );
    ...
    /* this next class is the class of rigid transformations. Transformations
       are
       applied at rendering time, and are not used for changing the object
       positions
    ...
    case CLASS_XFORM:
    {
      ...
      /* we also store the parameters of the transformation in the Matrix mat.
         We will have to call DXNewXform later to create an copy of the
         transformation
         used in the new field. Unless we prefer the new field to simply point
         to the old transformation rather than have its own copy of the
         transformation.
         ...
         if one object gets transformed and not the other.
            ...
            Are we getting the same image 1 and 2, or does 2 use the old
         transformation
            and 1 the old. In other words, does Translate create a new
         transformation
            (adds a translation, that is later composed with all other
         transformations
             in a combined transformation matrix) or modifies the transformation
         matrix, or
             creates a new matrix and unreferences the old matrix.

             Since "simplified" is a new object with its own downstream visual
         program
    ...
    case CLASS_CLIPPED:
    {
      ...
      out[0] = (Object)DXNewClipped( (Object)new_out[0], (Object)clipping[0] );
...
static int doLeaf( Object *in, Object *out )
{
  ...
  static char *in_name[8] = {
      "\"original_surface\"",  "\"max_error\"",
  ...
  else
  {
    ...
    /* we accept triangles as well as quads, that we convert directly to
     * triangles */
  ...
  else
  {
    ...
    else if ( DXGetObjectClass( in[0] ) != CLASS_STRING )
    {
      ...
      /* I am putting NULL into the "shape" slot since I don't know what the
         rank is
      ...
      /* otherwise the rank is one and I am fetching the data dimension now */
        ...
        DXGetArrayInfo( array, &in_knt[0], &type, &category, &rank,
                        &data_dimension );

      /* we are not interested in this data for simplification if the dimension
         is
      ...
      else if ( DXQueryConstantArray( array, NULL, NULL ) )
      {
        ...
        /* then get the data directly. No need to convert it to floats since it
      ...
      else
      {
        ...
        if ( !data_array )
        {
          ...
          in_data[0] = DXGetArrayData( data_array );
      }
    }
  ...
  GET_POSITIONAL_ERROR:
    ...
    if ( array )
    {
      ...
      if ( !strcmp( "positions", DXGetString( (String)attr ) ) )
      {
        ...
        if ( type == TYPE_FLOAT && category == CATEGORY_REAL && rank < 2 &&
             knt == p_knt )
  ...
  else
  {
    ...
    else
    {
      ...
      {
        DXSetError( ERROR_MISSING_DATA,
                    "\"max_data_error\" has no data component" );
        goto error;
      }
      ...
      {
        DXSetError( ERROR_BAD_CLASS,
                    "data component of \"max_data_error\" should be an array" );
        goto error;
      }
    }
    ...
    else
    {
      ...
      else
      {
        ...
        if ( !array )
        {
          ...
          sprintf( the_message, " %s has no data component",
                   in_name[the_input] );
        ...
        if ( DXGetObjectClass( (Object)array ) != CLASS_ARRAY )
        {
          sprintf( the_message, "data component of %s should be an array",
                   in_name[the_input] );
          DXSetError( ERROR_BAD_CLASS, the_message );
          goto error;
        }
      ...
      {
        DXGetArrayInfo( array, &in_knt[the_input], &type, &category, &rank,
                        &shape );
  ...
  /*
   Process Input 4, which is a flag specifying whether the surface boundary
   should
...

  {
    ...
    float max_error, max_error_data;

    /* default values for the options "volume", "boundary" "length" and "data"
     */

    int preserve_volume = 1, simplify_boundary = 0,
        preserve_boundary_length = 1, preserve_data = 1, statistics = 0;

    Field f =
        DXNewField(); /* create a new field to contain the simplified surface */
    ...
    if ( in_data[1] ==
         NULL ) /* this is the \"maximum_simplification_error\" parameter */
    {
      max_error =
          _dxfFloatDataBoundingBoxDiagonal( (float *)p_positions, p_knt, 3 );

      /* use one percent of the bounding box diagonal */

      max_error /= 100.;

      DXMessage( "SimplifySurface: 'max_error' defaults to 1 percent of bbox "
                 "diagonal: %g .",
                 max_error );
    ...
    if ( in_data[2] == NULL )
    {
      ...
      if ( in_data[0] ) /* if the data is an array of floats */
      {
        ...
        if ( preserve_data_flag[0] == 1 )
          ...
          /* then, in that case, the bounding box diagonal for the data still
             should
             be larger than zero, otherwise, the data does not need preserving
             */

          if ( ( max_error_data = _dxfFloatDataBoundingBoxDiagonal(
                     (float *)in_data[0], in_knt[0], data_dimension ) ) > 0.0 )
          {
            /* by default, we allow ten percent error on the data bounding box
               diagonal
               more than on the positions */

            max_error_data /= 10.;

            DXMessage( "SimplifySurface: 'max_error_data' defaults to 10 "
                       "percent of data bbox diagonal: %g .",
                       max_error_data );
    ...
    if ( in_data[3] ==
         NULL ) /* parameter instructing DX whether to preserve volume
    ...
    if ( in_data[4] ==
         NULL ) /* parameter instructing DX whether to simplify the boundary
    ...
    if ( in_data[5] == NULL ) /* in case the boundary edges are simplified,
                                 parameter specifying
                                 whether the length of the boundary should be
                                 preserved */
    {
      in_knt[5] = 1;
    ...
    if ( in_data[7] ==
         NULL ) /* instructs SimplifySurface to provide limited statistics
    ...
    result = SimplifySurface_worker(
        (Field)in[0], p_knt, p_dim, (float *)p_positions, c_knt, c_nv,
        (int *)c_connections, old_positional_error, data_dimension,
        (float *)in_data[0], (float *)in_data[1], (float *)in_data[2],
        (int *)in_data[3], (int *)in_data[4], (int *)in_data[5],
        (int *)in_data[6], (int *)in_data[7], &f );
    if ( !result )
    {
  ...
  /*
   * In either event: success or failure of the simplification, clean up
   * allocated memory
   */

  if ( data_array )
    DXFree( (Pointer)data_array );
    ...
    Field *simplified_surface )
{
...

  {
    ...
    {
      int nT = c_knt, new_nV = 0, new_nT = 0,
          old_nV_after_manifold_conversion = 0,
          old_nT_after_manifold_conversion = 0,
          *connections = (int *)c_connections, *new_connections = NULL,
          *vertex_parents = NULL, *face_parents = NULL, *vertex_lut = NULL,
          *face_lut = NULL;

      float *new_positions = NULL, *vertex_data = NULL,
            vertex_data_max_error = 0, *new_vertex_data = NULL,
            *new_positional_error = NULL, *face_normals = NULL,
            *old_face_areas = NULL, *new_face_areas = NULL;
      ...
      /* if simple statistics are requested, we print the number of vertices and
         triangles
         before and after simplification, as well as the percentage of the
         original number
         of triangles that they represent */

      if ( statistics[0] )
        DXMessage(
            "SimplifySurface: 'original_surface' has %d vertices and %d %s.",
            p_knt, c_knt, element_type[c_nv] );
      ...
      /* since we don't know beforehand how many new positions there
         ...
         Accordingly, I run DXFree() on each array after completion of the
         routine */

      /* convert the quads to triangles */

      if ( ( c_nv == 4 ) &&
           !( _dxfQuads2Triangles( &nT, &connections, &face_lut ) ) )
        goto failed_creating_surface;

      if ( _dxfSimplifySurfaceDriver(
              p_knt, p_positions, nT, connections, data_dimension, vertex_data,
              max_error[0], vertex_data_max_error,
              &old_nV_after_manifold_conversion,
              &old_nT_after_manifold_conversion, &new_nV, &new_positions,
              &new_vertex_data, &new_nT, &new_connections, &vertex_parents,
              &face_parents, &vertex_lut, &face_lut, old_positional_error,
              &new_positional_error, &face_normals, &old_face_areas,
              &new_face_areas, preserve_volume[0], simplify_boundary[0],
        ...
        else
        {
          ...
          else
          {
            ...
            else
            {
              ...
              if ( statistics[0] )
              {
                DXMessage( "SimplifySurface: 'simplified' has %d vertices "
                           "(%2.1f percent of original)...",
                           new_nV, 100. * (double)new_nV / (double)p_knt );
                DXMessage( "SimplifySurface: ...and %d triangles (%2.1f "
                           "percent of original).",
              ...
              _dxfVertexNormalsfromTriangleNormals(
                  nor, new_nV, new_nT, new_connections, face_normals,
                  new_face_areas );
              ...
              DXSetComponentValue( *simplified_surface, "positions",
                                   (Object)pos );
              DXSetComponentValue( *simplified_surface, "positional error",
                                   (Object)pos_error );
              DXSetComponentValue( *simplified_surface, "normals",
                                   (Object)nor );
              ...
              if ( !DXSetComponentAttribute(
                        *simplified_surface, "positions", "dep",
              ...
              if ( !DXSetComponentAttribute(
                        *simplified_surface, "positional error", "dep",
                        (Object)DXNewString( "positions" ) ) )
                goto failed_creating_surface;
              ...
              if ( !DXSetComponentAttribute(
                        *simplified_surface, "normals", "dep",
                        (Object)DXNewString( "positions" ) ) )
                goto failed_creating_surface;

              /* create the data component only if it was carried along the
               * simplification */

              if ( new_vertex_data )
              {
                Array data =
                    DXNewArray( TYPE_FLOAT, CATEGORY_REAL, 1, data_dimension );
                ...
                DXSetComponentValue( *simplified_surface, "data",
                                     (Object)data );

                /* set data as dependent on positions */

                if ( !DXSetComponentAttribute(
                          *simplified_surface, "data", "dep",
                          (Object)DXNewString( "positions" ) ) )
                  goto failed_creating_surface;
              }
              ...
              /* resample the components dep on positions and dep on connections
                 ...
                 the old values (vectors) attached to the original surface
                 vertices
                 ...
                 to compute new values (vectors) attached to the new vertices
                 and
                 triangles */

              _dxfResampleComponentValuesAfterSimplification(
                  in_field, simplified_surface,
                  old_nV_after_manifold_conversion,
...
int _dxfResampleComponentValuesAfterSimplification(
    ...
    int *face_lut, float *old_face_areas )
{
  ...
  Array array = NULL, new_array = NULL;

  while ( ( array = (Array)DXGetEnumeratedComponentValue(
                original_surface, num_component++, &component_name ) ) )
  {

    Object attr = DXGetAttribute( (Object)array, "dep" );

    /* reject "positions", "connections",  "normals", "neighbors", "positional
      error", components
      "invalid positions", "invalid_connections" because the surface was
      DXCulled so
      they are irrelevant.
    ...
    if ( strcmp( "positions", component_name ) != 0 &&
         strcmp( "connections", component_name ) != 0 &&
         strcmp( "normals", component_name ) != 0 &&
         strcmp( "neighbors", component_name ) != 0 &&
         ...
         strcmp( "box", component_name ) != 0 )
    {
      ...
      if ( strcmp( "data", component_name ) !=
               0 || /* or if it *is* the data component
        ...
        if ( attr && DXGetObjectClass( attr ) == CLASS_STRING )
        {
          ...
          if ( !strcmp( "positions", DXGetString( (String)attr ) ) )
          {
            ...
            new_array = _dxfResampleComponentDepPosAfterSimplification(
                array, old_nV_after_conversion, new_nV, vertex_parents,
                vertex_lut );

            if ( new_array )
            {
              DXSetComponentValue( *simplified_surface, component_name,
                                   (Object)new_array );

              /* set component as dependent on positions */

              if ( !DXSetComponentAttribute(
                        *simplified_surface, component_name, "dep",
                        (Object)DXNewString( "positions" ) ) )
                goto error;
            }
          }
          ...
          else if ( !strcmp( "connections", DXGetString( (String)attr ) ) )
          {
            ...
            new_array = _dxfResampleComponentDepConAfterSimplification(
                array, old_nT_after_conversion, new_nT, face_parents, face_lut,
                old_face_areas );
    ...
    int *vertex_lut )
{
  ...
  int *vertex_weights = NULL, rank, shape[3], size, tensor_size = 1, old_nV, i;
  ...
  {
    if ( rank == 0 )
      new_array = (Array)DXNewConstantArray(
          new_nV, DXGetConstantArrayData( array ), type, category, rank, 1 );

    else
      new_array =
          (Array)DXNewConstantArrayV( new_nV, DXGetConstantArrayData( array ),
...

  {
    ...
    vertex_data_averages =
        (float *)DXAllocateZero( new_nV * tensor_size * sizeof( float ) );

    if ( !vertex_data_averages )
      goto error;

    switch ( type )
    {
      case TYPE_BYTE:
        _dxfRESAMPLE_DEP_POS_AFTER_SIMPLIFICATION( byte );
        break;

      case TYPE_UBYTE:
        _dxfRESAMPLE_DEP_POS_AFTER_SIMPLIFICATION( ubyte );
        break;

      case TYPE_DOUBLE:
        _dxfRESAMPLE_DEP_POS_AFTER_SIMPLIFICATION( double );
        break;

      case TYPE_FLOAT:
        _dxfRESAMPLE_DEP_POS_AFTER_SIMPLIFICATION( float );
        break;

      case TYPE_INT:
        _dxfRESAMPLE_DEP_POS_AFTER_SIMPLIFICATION( int );
        break;

      case TYPE_UINT:
        _dxfRESAMPLE_DEP_POS_AFTER_SIMPLIFICATION( uint );
        break;

      case TYPE_SHORT:
        _dxfRESAMPLE_DEP_POS_AFTER_SIMPLIFICATION( short );
        break;

      case TYPE_USHORT:
        _dxfRESAMPLE_DEP_POS_AFTER_SIMPLIFICATION( ushort );
        break;
...
error:
  ...
  if ( new_array )
    DXDelete( (Object)new_array );
  if ( new_array_data )
    DXFree( new_array_data );
  if ( vertex_weights )
    DXFree( vertex_weights );
  if ( vertex_data_averages )
    DXFree( vertex_data_averages );
    ...
    int *face_lut, float *old_face_areas )
{
  ...
  int rank, shape[3], size, tensor_size = 1, old_nT, i;

  float *face_data_weights = NULL, *face_data_averages = NULL;
  ...
  if ( DXQueryConstantArray( array, &old_nT, NULL ) )
  {
    if ( rank == 0 )
      new_array = (Array)DXNewConstantArray(
          new_nT, DXGetConstantArrayData( array ), type, category, rank, 1 );

    else
      new_array =
          (Array)DXNewConstantArrayV( new_nT, DXGetConstantArrayData( array ),
                                      type, category, rank, shape );
...

  {
    ...
    face_data_averages =
        (float *)DXAllocateZero( new_nT * tensor_size * sizeof( float ) );

    if ( !face_data_averages )
      goto error;

    face_data_weights =
        (float *)DXAllocateZero( new_nT * tensor_size * sizeof( float ) );

    if ( !face_data_weights )
      goto error;

    switch ( type )
    {
      case TYPE_BYTE:
        _dxfRESAMPLE_DEP_CON_AFTER_SIMPLIFICATION( byte );
        break;

      case TYPE_UBYTE:
        _dxfRESAMPLE_DEP_CON_AFTER_SIMPLIFICATION( ubyte );
        break;

      case TYPE_DOUBLE:
        _dxfRESAMPLE_DEP_CON_AFTER_SIMPLIFICATION( double );
        break;

      case TYPE_FLOAT:
        _dxfRESAMPLE_DEP_CON_AFTER_SIMPLIFICATION( float );
        break;

      case TYPE_INT:
        _dxfRESAMPLE_DEP_CON_AFTER_SIMPLIFICATION( int );
        break;

      case TYPE_UINT:
        _dxfRESAMPLE_DEP_CON_AFTER_SIMPLIFICATION( uint );
        break;

      case TYPE_SHORT:
        _dxfRESAMPLE_DEP_CON_AFTER_SIMPLIFICATION( short );
        break;

      case TYPE_USHORT:
        _dxfRESAMPLE_DEP_CON_AFTER_SIMPLIFICATION( ushort );
        break;
...
error:
  ...
  if ( new_array_data )
    DXFree( new_array_data );
  if ( face_data_averages )
    DXFree( face_data_averages );
  if ( face_data_weights )
    DXFree( face_data_weights );
...

{
  ...
  else
  {
    ...
    {
      double norm = the_vertex_normal[0] * the_vertex_normal[0] +
...
float _dxfFloatDataBoundingBoxDiagonal( float *data, int n_data, int data_dim )
{
  ...
  /* update the bounding box if data values are smaller than the minimum or
  ...
  for ( i = 1; i < n_data; i++, cur_data += data_dim )
    ...
    for ( j = 0; j < data_dim; j++ )
    {
      if ( cur_data[j] < bbox[j] )
        bbox[j] = cur_data[j];

      else if ( cur_data[j] > bbox[j + data_dim] )
        bbox[j + data_dim] = cur_data[j];
    }

  /* once the bounding box is complete, compute its diameter. If the dimension
     of the data is
     one, we just compute max-min and thus avoid the computation of squares and
     of a square root */
  ...
  {
    double one_coordinate, double_diagonal = 0.0;
    ...
    if ( double_diagonal > 0.0 )
      double_diagonal = sqrt( double_diagonal );
...
int _dxfQuads2Triangles( int *nQ, int **connections, int **face_lut )
{
  ...
  int i, new_nT = 2 * *nQ, *quads = *connections, *new_t = NULL, *t, *lut;
  ...
  *face_lut = (int *)DXAllocateZero( new_nT * sizeof( int ) );
  ...
  /* the triangles are created according to the following simple rule:
         ...
         (v0,v1,v2,v3)        ---->  (v0,v2,v1) + (v1,v2,v3) see page 3-6 of the
     user's guide
