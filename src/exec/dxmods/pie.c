...
static Error MakePie( Object, Object, int, float, Array, Group *, Group *,
                      Array *, Array *, Field, int *, int );
...
static Error CheckHeight( Object, Object, Object, Object, Object, int, int *,
                          float ** );
static Error CheckRadius( Object, Object, Object, Object, Object, int, int *,
                          float ** );
...
static Error GetParameters( Object, Object, Object, Object, Object, float *,
                            float *, float *, float * );
...
Error m_Pie( Object *in, Object *out )
{
  ...
  if ( !MakePie( In_percents, In_colors, percentflag, quality, labellist, &pie,
                 &edges, &labeloutput, &coloroutput, labelfield, &numwedges,
                 show_percents ) )
    goto error;
  ...
  if ( ( dim == 1 ) || ( dim == 2 ) )
  {
    ...
    if ( !MakePie3D( (Object)pie, (Object)edges, labelfield, height, dim ) )
      goto error;
...
static Error MakePie( Object In_percents, Object In_colors, int percentflag,
                      float quality, Array labellist, Group *pie, Group *edges,
                      Array *labeloutput, Array *coloroutput, Field labelfield,
                      int *numwedges, int showpercents )
{
  Array data, c;
  Array colors = NULL;
  ...
  *labeloutput =
      DXNewArray( TYPE_STRING, CATEGORY_REAL, 1, maxstringlength + 10 );
  if ( !*labeloutput )
    goto error;
  *coloroutput = DXNewArray( TYPE_FLOAT, CATEGORY_REAL, 1, 3 );
  ...
  for ( i = 0; i < numitems; i++ )
  {
    ...
    while ( angle + arc < percentarray[i] + cumpercent )
    {
      ...
      if ( !DXAddArrayData( wedgesconnections, concount, 1,
                            ( Pointer ) & tri ) )
        goto error;
    ...
    {
      if ( !CopyComponents( (Field)In_percents, F1, F2, concount, concount + 2,
                            i ) )
        goto error;
    }
    else
  ...
  if ( !DXSetComponentValue( labelfield, "positions", (Object)labelpositions ) )
    goto error;
  labelpositions = NULL;
  if ( !DXSetComponentValue( labelfield, "data", ( Object ) * labeloutput ) )
    goto error;
...
static Error ScalePie( Object pie, int radtype, float *radius, int index )
{
  Object member;
  ...
  {
    case( CLASS_GROUP ) :
      for ( j = 0; ( member = DXGetEnumeratedMember( (Group)pie, j, NULL ) );
            j++ )
      {
...
static RGBColor *CheckColors( Array colors, Field F, int numdata )
{
  ...
  if ( !DXGetArrayInfo( colors, &numcolors, &type, &category, &rank, shape ) )
    goto error;
  ...
  switch ( type )
  {
    ...
    case( TYPE_INT ) :
      ...
      {
        colorRGBint_ptr =
            DXIterateArray( handle, i, colorRGBint_ptr, scratchRGBint );
    ...
    case( TYPE_UBYTE ) :
      ...
      if ( ( rank == 0 ) || ( ( rank == 1 ) && ( shape[0] == 1 ) ) )
      {
        ...
        {
          colorbyte_ptr =
              DXIterateArray( handle, i, colorbyte_ptr, scratchbyte );
      ...
      else if ( ( rank == 1 ) && ( shape[0] == 3 ) )
      {
        ...
        for ( i = 0; i < numcolors; i++ )
        {
          colorbytecolor_ptr = DXIterateArray( handle, i, colorbytecolor_ptr,
                                               scratchbytecolors );
...
static Error MakePie3D( Object pie, Object edges, Field labelfield, float *h,
                        int dim )
{
  ...
  for ( j = 0; ( member = DXGetEnumeratedMember( (Group)edges, j, NULL ) );
        j++ )
  {
    ...
    if ( dim == 1 )
    {
      for ( i = 0; i < numitems; i++ )
      {
        ...
        pos_3D_ptr[2 * i + 1] =
            DXPt( pos_2D_ptr[i].x, pos_2D_ptr[i].y, height );
      }
    }
    else
    {
    ...
    if ( !DXSetComponentValue( (Field)member, "connections",
      ...
      goto error;
    newconnections = NULL;
    if ( !DXSetComponentAttribute( (Field)member, "connections", "element type",
...
static Error CheckLabels( int numitems, Array labellist, int *maxstringlength )
{
  ...
  if ( !DXGetArrayInfo( (Array)labellist, &numstrings, NULL, NULL, NULL,
                        shape ) )
    goto error;
...
static Error CopyComponents( Field input, Field pie, Field edges, int numwedges,
                             int numedges, int index )
{
  ...
  {
    DXSetError( ERROR_DATA_INVALID, "data dependency attribute not string" );
    goto error;
  }

  while ( NULL != ( component = (Array)DXGetEnumeratedComponentValue(
                        input, compcount, &name ) ) )
  {
    Object attr;
    ...
    /* we only bother with components which dep the same thing that
       data deps */
    if ( strcmp( DXGetString( (String)attr ),
                 DXGetString( (String)dataattr ) ) )
      goto component_done;

    componentsdone++;
    if ( !DXGetArrayInfo( (Array)component, &numitems, &type, &category, &rank,
                          shape ) )
      goto error;
    ...
    newwedgescomponent = (Array)DXNewConstantArrayV( numwedges, dataptr, type,
                                                     category, rank, shape );
    if ( !DXSetComponentValue( pie, name, (Object)newwedgescomponent ) )
      goto error;
    newwedgescomponent = NULL;

    newedgescomponent = (Array)DXNewConstantArrayV( numedges, dataptr, type,
                                                    category, rank, shape );
    if ( !DXSetComponentValue( edges, name, (Object)newedgescomponent ) )
      goto error;
    newwedgescomponent = NULL;
    if ( !DXSetComponentAttribute( (Field)pie, name, "dep",
                                   (Object)DXNewString( "connections" ) ) )
      goto error;
    if ( !DXSetComponentAttribute( (Field)edges, name, "dep",
                                   (Object)DXNewString( "connections" ) ) )
      goto error;
                      ...
                      int numitemspie, int numitemsedges )
{
  ...
  piedata = (Array)DXNewConstantArray( numitemspie, percentval, TYPE_FLOAT,
                                       CATEGORY_REAL, 0 );
  if ( !piedata )
    goto error;
  edgesdata = (Array)DXNewConstantArray( numitemsedges, percentval, TYPE_FLOAT,
                                         CATEGORY_REAL, 0 );
  if ( !edgesdata )
    goto error;
...
static Error CheckHeight( Object input, Object heightscale, Object heightmin,
                          Object heightmax, Object heightratio, int numwedges,
                          int *dim, float **h )
{
  int numitems = 1, i;
  ...
  if ( !GetParameters( input, heightscale, heightmin, heightmax, heightratio,
                       &scaleval, &minval, &maxval, &ratioval ) )
    goto error;
  ...
  switch ( DXGetObjectClass( input ) )
  {
    case( CLASS_ARRAY ) :
      ...
      if ( numitems == 1 )
      {
        *h = DXAllocate( sizeof( float ) );
        ...
        {
          DXSetError( ERROR_DATA_INVALID, "height must be a positive scalar" );
          goto error;
        }
        if ( ( **h < 0 ) )
        {
      ...
      else
      {
        *h = DXAllocate( numitems * sizeof( float ) );
        ...
        {
          DXSetError( ERROR_DATA_INVALID, "height must be a single value or a "
                                          "list which matches percents" );
          goto error;
        }
        *dim = 2;
      }
    ...
    default:
      DXSetError( ERROR_DATA_INVALID, "height must be a scalar list or field" );
      goto error;
  }
...
static Error CheckRadius( Object input, Object radiusscale, Object radiusmin,
                          Object radiusmax, Object radiusratio, int numwedges,
                          int *radtype, float **radius )
{
  ...
  if ( input )
  {
    ...
    switch ( DXGetObjectClass( input ) )
    {
      case( CLASS_ARRAY ) :
        /* see how many */
        if ( !DXGetArrayInfo( (Array)input, &numitems, &type, &category, &rank,
                              shape ) )
          goto error;
        ...
        *radius = DXAllocate( numitems * sizeof( float ) );
        ...
        if ( numitems == 1 )
        {
          ...
          {
            DXSetError( ERROR_DATA_INVALID, "radius must be a scalar list " );
            goto error;
          }
          if ( **radius < 0 )
          {
            DXSetError( ERROR_DATA_INVALID, "radius must be positive scalars" );
            goto error;
          }
          *radtype = 0;
        }
        ...
        {
          if ( !DXExtractParameter( input, TYPE_FLOAT, 1, numitems, *radius ) )
          {
            DXSetError( ERROR_DATA_INVALID, "radius must be a scalar list " );
            goto error;
          }
          ...
          {
            DXSetError( ERROR_DATA_INVALID, "radius must be a single value or "
                                            "a list which matches percents" );
            goto error;
          }
          for ( i = 0; i < numitems; i++ )
          {
      ...
      case( CLASS_FIELD ) :
        ...
        /* see how many */
        if ( !DXGetArrayInfo( (Array)data, &numitems, &type, &category, &rank,
                              shape ) )
          goto error;
        if ( numitems != numwedges )
        {
          DXSetError( ERROR_DATA_INVALID, "radius must be a single value or a "
                                          "list which matches percents" );
          goto error;
        }
      ...
      default:
        DXSetError( ERROR_DATA_INVALID,
                    "radius must be a scalar list or field" );
        goto error;
    }
...
static Error AddColors( Group *pie, Group *edges, int numitems, RGBColor *cptr )
{
  ...
  for ( i = 0; i < numitems; i++ )
  {
    ...
    if ( !DXGetArrayInfo( (Array)connections, &numcon, NULL, NULL, NULL,
                          NULL ) )
      goto error;
    newcolors = (Array)DXNewConstantArray( numcon, &tmp_ptr[i], TYPE_FLOAT,
                                           CATEGORY_REAL, 1, 3 );
...
static Array GetLabels( Object labels, Object labelformat )
{
  ...
  switch ( DXGetObjectClass( labels ) )
  {
    case( CLASS_FIELD ) :
      ...
      {
        DXSetError( ERROR_MISSING_DATA,
                    "labels field is missing data component" );
        goto error;
      }
      component = data;
      break;
...
static Error GetParameters( Object input, Object scale, Object min, Object max,
                            Object ratio, float *scaleval, float *minval,
