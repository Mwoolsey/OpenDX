...
int _dxfSimplifySurfaceDriver(
    int nV, float *v, int nT, int *t, int dim_data, float *vertex_data,
    float tolerance, float data_tolerance,
    int *old_nV_after_manifold_conversion,
    int *old_nT_after_manifold_conversion, int *new_nV, float **new_v,
    float **new_vertex_data, int *new_nT, int **new_t, int **vertex_parents,
    int **face_parents, int **vertex_lut, /* for fixing non-manifolds */
    int **face_lut, /* same thing: for eliminating degenerate faces */
    float *old_positional_error, float **new_positional_error,
    float **face_normals, float **old_face_areas, float **new_face_areas,
    int preserve_volume, int simplify_boundary, int preserve_boundary_length )
...

{
  ...
  bzero( simp_data, sizeof( SimpData ) );

  bzero( edge_table, sizeof( Htable ) );

  /* the first step is to convert the surface to a manifold, in case
     Isosurface would have created non-manifold vertices (I know that happens
     sometimes)

     It is also possible that a surface provided directly by the user would not
     be a manifold */

  if ( _dxfToOrientedManifold( nV, v, nT, t, new_nV, new_v, new_nT, new_t,
                               vertex_lut, face_lut, &nE, edge_table, &e ) )
  {
    ...
    *old_nT_after_manifold_conversion = *new_nT;
    ...
    simp_data->nE =
        nE; /* after conversion to manifold and before simplification */
    simp_data->nV = *new_nV;
    ...
    *old_face_areas =
        (float *)DXAllocateZero( simp_data->nT * sizeof( float ) );
    ...
    *face_parents = (int *)DXAllocateZero( simp_data->nT * sizeof( int ) );
    ...
    if ( *old_face_areas && *vertex_parents && *face_parents &&
         ( _dxfCreateSimplificationDataStructure( simp_data, vertex_data,
                                                  old_positional_error ) ) &&
         ( _dxfSimplifyManifoldSurface( simp_data ) ) )

    {
      /* free only the simplification data structures that are not exported by
         the driver routine,
         to avoid running out of memory in _dxfBuildSimplifiedSurface(): */

      _dxfPartialFreeSimplificationDataStructure( simp_data );

      /* build a simplified surface using the vertex_father and triangle_father
       * arrays */
      ...
      _dxfFreeSimplificationDataStructure( simp_data, vertex_data,
                                           old_positional_error );
    }
  ...
  /* if the conversion to oriented manifold fails, vertex_lut, new_v, new_t ...
     etc.
...
int _dxfToOrientedManifold( int nV, float *v, int nT, int *t, int *new_nV,
                            float **new_v, int *new_nT, int **new_t, int **vlut,
                            int **flut, int *n_edges, Htable *e_table,
                            EdgeS **edges )

{
  ...
  if ( _dxfEliminateDegenerateTriangles( nV, nT, t, new_nT, new_t, flut ) )
  {
    ...
    if ( _dxfEliminateStandaloneVertices( nV, v, new_nV, new_v, nT, *new_t,
                                          vlut ) )
    {
      ...
      if ( fathers )
      {
        if ( _dxfJoinTriangleCorners( nT, *new_t, e_table, n_edges, edges,
                                      fathers ) )
        {

          if ( _dxfBuildOrientedManifold( new_nV, new_v, nT, *new_t, vlut,
                                          n_edges, edges, e_table, fathers ) )
...
int _dxfJoinTriangleCorners( int nT, int *t, Htable *e_table, int *n_edges,
                             EdgeS **edges, int *fathers )
...
/* we join two triangle corners only if they share a manifold edge
   ...
   and such that the two triangles are consistently oriented. Also, the two
   triangles
...

{
  ...
  /* we first determine the edges that are manifold
     ...
     in a second pass, we take all manifold edges and we join the corresponding
     triangle corners */
  ...
  if ( edge_array )
  {
    ...
    EdgeS the_edge[1], *cur_edge, *edge_in_table;

    int *incident_tri = SurfEdgeGetTriangles( the_edge ),
        *endpoints = SurfEdgeGetVertices( the_edge ), nE = 0, total_nE = nT3,

        /* initialize a pointer to the current triangle */
        *the_triangle = t, first_tri, second_tri, corner_v0first,
        corner_v1first, corner_v0second, corner_v1second,
        edge_paired_as_manifold = 0, v0, /* surface edge end-points */
        v1, *triangles_in_table;

    /* initialize the fathers array */

    _dxfInitFather( nT3, fathers );

    bzero( (char *)e_table, sizeof( Htable ) ); /* reset the hash table */

    /* the size of the table should be chosen such that 2**tbl_size ~ 2 * number
       of  edges
       for best performances. Here taking nT is plenty enough*/

    if ( !_dxfInitHashTable(
              e_table, _dxfFlog2( nT ), (int (*)( KEYTYPE ))_dxfEdgeSKey,
              (int (*)( RECTYPE, RECTYPE ))_dxfEdgeSFilter, NULL ) )
      goto error;
    ...
    /* for each triangle, add the three edges to the hash table if they are not
       already
    ...
    for ( the_triangle = t, i = 0; i < nT; i++, the_triangle += 3 )
    {
...

      {
        ...
        if ( !edge_in_table )
        {
          ...
          if ( !_dxfAddRecord( e_table, ( RECTYPE )( edge_array + nE ),
                               ( KEYTYPE )( edge_array + nE ) ) )
            goto error;
        ...
        {/* there is already an edge (v0,v1) in the table */
          ...
          /* either one of the following two cases apply:
             ...
             CASE ONE: we have already determined that the edge is not a
             manifold edge,
          ...
          if ( SurfEdgeLabel( edge_in_table ) == 0 )
          {
            ...
            bzero( edge_array + nE, sizeof( EdgeS ) );

            nE++;
          }

          /* CASE TWO: the edge is supposed to be a manifold edge with a label
           == 2
           ...
           *but now* there is a third triangle in the picture.
           ...
           two, plus the new one is three. So we increment the edge counter by
           two.
           */
          ...
          /* CASE THREE: we did not establish whether  the edge is a manifold
             edge :
          ...
          else if ( SurfEdgeLabel( edge_in_table ) == 1 )
          {
            ...
            /* if the orientations of the faces do match,
               ...
               and we will *later*  join the corresponding vertices using the
               father array,
               if it is established in the edge that the edge (v0,v1) *is
               indeed* manifold */
            ...
            if ( ( v0 >= v1 ) && triangles_in_table[0] == -1 )
            {
              ...
              /* so v0 and v1 are visited in that order in the_triangle.
                 they have to be visited in the opposite order in
                 triangle_in_table */

              second_tri = triangles_in_table[1];

              /* we know that v1 appears first in second_tri (actually I am not
                 sure what
              ...
              j = 0;
              while ( j < 3 && t[second_tri * 3 + j] != v1 )
                j++;

              /* for the orientation to match, v0 must appear next, right after
                 v1

                 also, we do not want the two triangles to have exactly the same
                 three
                 vertices */

              if ( j < 3 && t[second_tri * 3 + ( j + 1 ) % 3] == v0 &&
                   t[second_tri * 3 + ( j + 2 ) % 3] !=
                       t[first_tri * 3 + ( k + 2 ) % 3] )
              {
            ...
            {
              /* so it worked, and we indeed have a manifold edge (for the
                 moment)
            ...
            {
              /* so there are (at least) two triangles sharing that edge, but
                 they
    ...
    /* Now the manifold edges have been identified.
       For each manifold edge, I join the triangle corners associated with this
       edge */
    ...
    for ( cur_edge = edge_array, i = 0; i < nE; i++, cur_edge++ )
    {
      ...
      if ( SurfEdgeLabel( cur_edge ) == 2 )
      {
        ...
        /* what are the corners associated with the first triangle
         * incident_tri[0] ? */

        /* ANSWER we loop on k until v0 is found */

        k = 0;
        while ( k < 3 && t[first_tri * 3 + k] != v0 )
          k++;
        ...
        if ( k < 3 )
        {
          ...
          if ( j < 3 )
          {
            /* note that we have already verified that first_tri and second_tri
               have different vertices when we established that the edge
               cur_edge was
    ...
    bzero( (char *)e_table, sizeof( Htable ) );
...
int _dxfBuildOrientedManifold( int *nV, float **v, int nT, int *t, int **vlut,
                               int *nE, EdgeS **edges, Htable *e_table,
                               int *fathers )
{

  int success = 0;

  int nT3 = 3 * nT, new_nV = 0;

  float *new_v = NULL;

  int *new_t = NULL, *old_vlut = *vlut, *new_vlut = NULL;

  register int i;

  /* create a conversion table from old vertices to new vertices (triangle
     corners)
  ...
  *vlut = new_vlut;
  ...
  /* copy the vertices only if the corner is a father: only for each actual new
   * vertex */
  ...
  *nV = new_nV;
  ...
  if ( _dxfIndexEdges( nE, edges, e_table, nT, t ) )
    /* if this fails, we do not go to error, because there is nothing to
       free (the hash table is freed in the error: of _dxfIndexEdges and so are
       the edges)
...
int _dxfIndexEdges( int *n_edges, EdgeS **new_edges, Htable *e_table, int nT,
                    int *t )
{
  /*
     It is very important to count the edges again!

      particularly, we may have implicitely joined edges that we thought were
     not joined */

  EdgeS *edges = NULL, the_edge[1], *edge_in_table;

  int nE = *n_edges, *the_triangle = t,
      *incident_tri = SurfEdgeGetTriangles( the_edge ),
      *endpoints = SurfEdgeGetVertices( the_edge ), second_tri,
  ...
  *new_edges = (EdgeS *)DXReAllocate( *new_edges, nE * sizeof( EdgeS ) );
  ...
  if ( !_dxfInitHashTable(
            e_table, _dxfFlog2( nE ), (int (*)( KEYTYPE ))_dxfEdgeSKey,
            (int (*)( RECTYPE, RECTYPE ))_dxfEdgeSFilter, NULL ) )
    goto error;
  ...
  for ( the_triangle = t, nE = i = 0; i < nT; i++, the_triangle += 3 )
  {
...

    {
      ...
      else
      {
        ...
        if ( !_dxfAddRecord( e_table, ( RECTYPE )( edges + nE ),
                             ( KEYTYPE )( edges + nE ) ) )
          goto error;
...
int _dxfEliminateDegenerateTriangles( int nV, int nT, int *t, int *new_nT,
                                      int **new_t, int **flut )
{
  int success =
      0; /* variable converted to one and returned if the procedure executes
            successfully */

  int trisize = 3 * sizeof( int ), *t1 = t, *new_tris = NULL, *face_lut = NULL;
  ...
  *new_nT = 0;
  ...
  /* 4/20/97: with respect to the original implementation, we now have the
     difference
     that there may be a pre-existing "flut" face look-up table, which was
     created
  ...
  if ( new_tris && ( *new_t ) )
  {
    ...
    for ( i = 0; i < nT; i++, t1 += 3 )
      ...
      /* if all three triangle indices are different, add the triangle to the
       * new triangle list */
      ...
      if ( t1[0] >= 0 && t1[0] < nV && t1[1] >= 0 && t1[1] < nV && t1[2] >= 0 &&
           t1[2] < nV )
        ...
        if ( t1[0] != t1[1] && t1[1] != t1[2] && t1[0] != t1[2] )
        {
          if ( *new_nT < i )
            face_lut[*new_nT] =
                face_lut[i]; /* i is always larger (>=) than *new_nT
...
int _dxfEliminateStandaloneVertices( int nV, float *v, int *new_nV,
                                     float **new_v_exported, int nT, int *t,
                                     int **vlut )
{
  ...
  int *new_tris = t, *vertex_lut = NULL, *old_to_new_vertex_index = NULL;
  ...
  *new_nV = 0; /* reset the number of new vertices */
  ...
  for ( i = i1 = 0; i < nT; i++, i1 += 3 )
    ...
    for ( k = 0; k < 3; k++ )
      new_tris[i1 + k] = old_to_new_vertex_index[t[i1 + k]];

  /* if there are fewer vertices than before, we reallocate the vertex array and
   * the vertex lut */

  if ( *new_nV != nV )
  {
    *new_v_exported =
        (float *)DXReAllocate( *new_v_exported, *new_nV * sizeof( Vertex ) );
                       ...
                       KEYTYPE ( *compute_key )( RECTYPE ) )
{
  ...
  table->bucket =
      (Nodetype **)DXAllocateZero( sizeof( Nodetype * ) * table->size );
  ...
  if ( compute_key ==
       NULL ) /* function that computes the key from the record */
...
Nodetype *_dxfgetnode( Htable *table )
{
  ...
  else
  {
    ...
    if ( !_dxfNewBlock( table ) )
      goto error;

    /* allocate a new array of nodes */

    table->block->rec =
        (RECTYPE)DXAllocateZero( sizeof( Nodetype ) * table->nodes_per_block );
    ...
    for ( retval = (Nodetype *)table->block->rec, i = 0;
          i < table->nodes_per_block - 1; i++, retval++ )
    {
...
Nodetype *_dxfnewnode( Htable *table, RECTYPE rec, int code )
{
  ...
  /* if _dxfgetnode(table) returns a NULL pointer, we should not try to access
   * it */
...
void _dxfHashTableReset( Htable *table )
{
  ...
  Block *block = table->block, *next;

  while ( block )
  {
    next = block->next;
    if ( block->rec )
      DXFree( (Pointer)block->rec );
...
char *_dxfNextRecord( Htable *table )
{
  ...
  /* if a filter function was provided, the first node with the same code AND
     such
     that the filter function comparing the node's record and the search key
     "agrees"
     is selected */
...
/* when a filtering function IS provided, we need to compare a current record
   with
   the search key. In the general case, a "compute_key" function must be
   provided
   to convert the key to a record or something that can be directly compared
   with
   a record
   in many cases the search key is the same as a record, or can be used without
   conversion.
...
int _dxfJoin( int i, int j, int *father )
{
  ...
  if ( ( rep_i = _dxfFather( i, father ) ) !=
       ( rep_j = _dxfFather( j, father ) ) )
  {
...
int _dxfCreateSimplificationDataStructure( SimpData *simp_data,
                                           float *vertex_data,
                                           float *old_positional_error )
{
  ...
  simp_data->edge2index =
      (int *)DXAllocateZero( simp_data->nE * sizeof( int ) );
  ...
  simp_data->normal =
      (Vertex *)DXAllocateZero( simp_data->nT * sizeof( Vertex ) );
  ...
  _dxfTrianglesNormalAreaCompactness(
      simp_data->nT, (Face *)simp_data->triangles, simp_data->vert,
      simp_data->normal, simp_data->area, simp_data->compactness );

  /* save the areas in the "old_face_areas" array */

  memcpy( simp_data->old_face_areas, simp_data->area,
          simp_data->nT * sizeof( float ) );
  ...
  simp_data->valence =
      (u_short *)DXAllocateZero( simp_data->nV * sizeof( u_short ) );

  if ( !simp_data->valence )
    goto error;

  _dxfComputeVertexValence( simp_data->nT, (int *)simp_data->triangles,
                            simp_data->valence );

  /* allocate and compute the "boundary_vert" array that flags boundary vertices
   * with a flag value of 1 */

  simp_data->boundary_vert =
      (u_char *)DXAllocateZero( simp_data->nV * sizeof( u_char ) );

  if ( !simp_data->boundary_vert )
    goto error;

  _dxfFlagBoundaryVertices( simp_data->nE, simp_data->edge_array,
                            simp_data->boundary_vert );
  ...
  if ( simp_data->err_volume )
  {
    ...
    simp_data->err_volume =
        (float *)DXAllocateZero( simp_data->nV * sizeof( float ) );

    if ( !simp_data->err_volume )
      goto error;

    memcpy( simp_data->err_volume, old_error_volume,
            simp_data->nV * sizeof( float ) );
  }
  else
  {
    simp_data->err_volume =
  ...
  _dxfInitArray( (char *)simp_data->tol_volume, (char *)&simp_data->tolerance,
                 simp_data->nV, sizeof( float ) );
  ...
  if ( simp_data->vx_data && simp_data->data_dim > 0 )
  {
    ...
    simp_data->vx_data = (float *)DXAllocateZero(
        simp_data->nV * simp_data->data_dim * sizeof( float ) );

    if ( !simp_data->vx_data )
      goto error;

    simp_data->vx_data_error =
        (float *)DXAllocateZero( simp_data->nV * sizeof( float ) );

    simp_data->vx_data_potential_values =
        (float *)DXAllocateZero( 3 * simp_data->data_dim * sizeof( float ) );

    if ( !simp_data->vx_data_potential_values )
      goto error;

    simp_data->vx_old_data =
        simp_data->vx_data_potential_values + simp_data->data_dim;
  ...
  _dxfInitArray( (char *)simp_data->edge2index,
                 (char *)&simp_data->heap_initial_index, simp_data->nE,
                 sizeof( int ) );
...
int _dxfFreeSimplificationDataStructure( SimpData *simp_data,
                                         float *vertex_data,
                                         float *old_positional_error )
{
...
int _dxfTrianglesNormalAreaCompactness( int nT, Face *t, Vertex *v,
                                        Vertex *t_normal, float *t_area,
                                        float *t_compactness )
{
  int success = 1;
...
/* computation of the triangle normal in double precision
   ...
   this method is much more numerically stable than the cross product of the
   vectors.

   it uses Householder orthogonalization of a set of two basis vectors chosen
   for the
   triangle. Experimental studies show that it is better to take both the
   shortest
...
int _dxfTriangleNormalQR2( Vertex *tri, Vertex n )
{
  ...
  bzero( (char *)n, 3 * sizeof( float ) );

  _dxfTriangleBasisVectors( tri, x1, x2, origin );

  {

    float sign1 = ( x1[0] >= 0. ) ? 1. : -1., sign2, norm1, norm2;
...
/* compute the vector product using QR decomposition (which, to me, is the
   ...
   In the case of a 3 * 2 system, either one or two Householder transformations
   are required.
...
int _dxfVectorProductQRD( VertexD x1, VertexD x2, double *n )
{
...

  {
    ...
    double sign1 = ( x1[0] >= 0. ) ? 1. : -1., sign2, norm1, norm2;
...
static void HeapDown( int father, Heap *h )
{
  ...
  */
  ...
  if ( 0 <= father && father < h->last - 1 )
  {
    ...
    {
      child = father;
      if ( h->key[left] < h->key[child] )
        child = left;
      if ( ( right = RightSon( father ) ) < h->last &&
           h->key[right] < h->key[child] )
        child = right;
      if ( child == father )
        break;
...
Heap *_dxfNewHeap( int n )
{
  ...
  if ( n > 0 )
  {
    ...
    heap->invp = (int *)( block + sizeof( Heap ) + n * sizeof( float ) +
                          n * sizeof( int ) );
...
unsigned long _dxfHeapSize( int n )
{
  ...
  unsigned long bytes =
      sizeof( Heap ) + n * sizeof( float ) + 2 * n * sizeof( int );
...
int _dxfAddEdge2Heap( SimpData *simp_data, int the_edge )
{
  ...
  if ( !HeapFull( simp_data->edge_heap ) &&
       simp_data->edge2index[the_edge] >= 0 )
  {
    /* compute the representatives of the edge vertices */
    int *v = SurfEdgeGetVertices( simp_data->edge_array + the_edge ),
        v0 = FATHER( v[0], simp_data->vertex_father ),
        v1 = FATHER( v[1], simp_data->vertex_father );

    /* 1) to compute the key to the heap, we choose the edge length to start
     * with */
    float weight = DIST3( simp_data->vert[v0], simp_data->vert[v1] );
    ...
    simp_data->index2edge[simp_data->edge2index[the_edge] = _dxfHeapAdd(
                              weight, simp_data->edge_heap )] = the_edge;
    added = 1;
  }
...
{
  int idx = _dxfHeapDelMin( simp_data->edge_heap );
...
/*
    ...
    the simplified surface. The vertex_parent and face_parent array are
   reshuffled
    to be used later for the purpose of interpolating data (positions-dependent
    or connections-dependent) from the original surface to the simplified
   surface)
...

{
  ...
  int *vertex_parents = simp_data->vertex_father,
  ...
  int *t = NULL, *new_v_number = NULL, *new_t_number = NULL;
  ...
  *new_positional_error = (float *)DXAllocateZero( *new_nV * sizeof( float ) );
  ...
  if ( !( *new_positional_error ) )
    goto error;

  if ( simp_data->vx_data )
  {
    *new_vertex_data = (float *)DXAllocateZero( *new_nV * simp_data->data_dim *
                                                sizeof( float ) );
  ...
  /* copy the vertices and other vertex attributes and update the vertex_parents
   * array */

  for ( j = i = 0; i < simp_data->nV; i++ )
    if ( i == vertex_parents[i] )
    {
      ...
      if ( simp_data->vx_data )
        memcpy( *new_vertex_data + j * simp_data->data_dim,
                simp_data->vx_data + i * simp_data->data_dim,
                simp_data->data_dim * sizeof( float ) );
  ...
  for ( i = 0; i < simp_data->nV; i++ )
    vertex_parents[i] = new_v_number[vertex_parents[i]];

  DXFree( (Pointer)new_v_number );
  new_v_number = NULL;
  ...
  *new_nT = 0;
  ...
  for ( i = 0; i < simp_data->nT; i++ )
    if ( i == FATHER( i, face_parents ) )
  ...
  *face_normals = (float *)DXAllocateZero( *new_nT * sizeof( Vertex ) );
  ...
  /* copy the triangles and other triangle attributes and update the
   * face_parents array */

  for ( j = i = 0; i < simp_data->nT; i++ )
    if ( i == face_parents[i] )
    {
      register u_char k;

      for ( k = 0; k < 3; k++ )
        t[j * 3 + k] = vertex_parents[( *new_t )[i * 3 + k]];
  ...
  for ( i = 0; i < simp_data->nT; i++ )
    face_parents[i] = new_t_number[face_parents[i]];

  DXFree( (Pointer)new_t_number );
  new_t_number = NULL;
...
/*
  As the name indicates, I am just using two different flags for edges adjacent
  to
  ...
  This routine is executed *only* if the user does not wish to simplify the
  boundary.
...
int _dxfMarkEdgesAdjacent2Boundary( SimpData *simp_data )
{
  ...
  int e = 0, nE = simp_data->nE, edges_adjacent_2_boundary;

  u_char *boundary_vertex = simp_data->boundary_vert, boundary1, boundary2;
  ...
  for ( edges_adjacent_2_boundary = nE, e = 0; e < nE; e++, current_edge++ )
  {

    /* if the edge label is 1, then it is a boundary edge of the second kind */

    if ( SurfEdgeLabel( current_edge ) == 1 )
      simp_data->edge2index[e] = -2;

    else
    {

      int *v = SurfEdgeGetVertices( current_edge );

      boundary1 = boundary_vertex[v[0]];
      boundary2 = boundary_vertex[v[1]];

      if ( boundary1 || boundary2 )
        simp_data->edge2index[e] = -1;
...
int _dxfBuildEdgeHeap( SimpData *simp_data )
{
  ...
  int can_add_edge = 1, n_edges_added = 0, nE = simp_data->nE;
  ...
  while ( can_add_edge && simp_data->last_edge_added2heap < nE )
  {
    if ( simp_data->edge2index[simp_data->last_edge_added2heap] ==
      ...
      n_edges_added += ( can_add_edge = simp_data->add_edge(
                             simp_data, simp_data->last_edge_added2heap ) );
                              ...
                              int *tri_v_rotated )
{
  ...
  else if ( tri_v_parents[1] == parent_vertex )
  {
    ...
    tri_v_rotated[0] =
        tri_v_parents[0]; /* use tri_v_rotated[0] as a temporary buffer */
...
int _dxfRotateParentEdge( int parent_vertex, int *vertex_fathers, int *edge_v,
                          int *edge_t, int *edge_v_parents, int *edge_v_rotated,
                          int *edge_t_rotated )
{
  ...
  */
  ...
  {
    edge_v_rotated[0] =
        edge_v_parents[0]; /* use edge_v_rotated[0] as a temporary buffer */
...
int _dxfDirectedParentVertexStar( int parent_vertex, int first_triangle,
                                  int valence, int *star, SimpData *simp_data,
                                  int direction )
{
  ...
  /* labeling system for directed triangle stars
                  ...
                  v*1\      /v*0
                   e*1\  *1/e*0
               *val0-1 \  / *0
             ___________\/___________ we omit on purpose the first vertex,
     because in the case
  ...
  int *vstar = star + valence, *estar = vstar + valence,
      ...
      star_triangle = first_triangle, i;
  ...
  for ( i = 0; i < valence; i++ )
  {
    ...
    _dxfRotateParentEdge(
        parent_vertex, simp_data->vertex_father,
...
int _dxfManifoldLinkTest( int *link0, int *link1, int val0, int val1 )
{
  ...
  int link_is_manifold = 1, val = val0 + val1;

  /* since
        val0 + val1 <= SIMPLIFY_VALENCE_MAX4,
        I can use a static array here. */

  static int link[SIMPLIFY_VALENCE_MAX4];

  {
    ...
    qsort( link, val, sizeof( int ),
           (int (*)(const void *, const void *))_dxfCmpIntSmallFirst );
...
int _dxfSolveQuadraticEquation( double A, double B, double C, double *sol1,
                                double *sol2, double eps )
{
  /*

    returns the number of real solutions of a second degree equation as well as
   the solutions
...
int _dxfSimplifiedBoundaryVertexLocation( SimpData *simp_data, int v0, int v1,
                                          int *vstar0, int *vstar1, int val0,
                                          int val1, int method )
{
  int success = 1;
  ...
  if ( method == 0 )
    ...
    memcpy( simp_data->simplified_vertex, simp_data->vert[v0],
            sizeof( Vertex ) );
  ...
  else if ( method == 1 )
  {
    ...
    */
    ...
    float a, a2, c, eps1 = (float)1e-6,
                    *ve = (float *)simp_data->simplified_vertex;

    Vertex v_zero, v_one, vr, vl, vm;
    ...
    a = ( DIST3( simp_data->vert[v0], simp_data->vert[v1] ) +
    ...
    if ( a == 0.0 || /* if a == 0, the length to preserve is zero, then the
                        segment middle is fine! */
         1. - c / a < eps1 )
    ...
    else /* if (a > c ) */
    {
      ...
      /* then we consider the middle of v0 and v1, vm,
       ...
       -c                     x              c
                         ...
                         vp, and the coordinates of vm in the coordinate system
       formed by
       ...
       also we should not execute the part of the code relative to t == 0 or t
       == 1
       */
      ...
      /* what do we do if y == 0, in which case vm is accidentally
        ...
        vl   vp0      vp      vr
                            ...
                            ----->
               and we use the direction vp0 v0 for projecting the point vp on
        the  ellipse
      ...
      {
        Vertex u, v;
        double cos_uv, cos_uv_2, sin_uv_2, A, B, C, sol1, sol2, eps2 = 1e-12;

        MakeVec( vl, vr,
                 u ); /* if the segment (vr,vl) is of zero length, which is
      ...
      else /* vm projects normally inside the segment */
      {
        ...
        /* this is to avoid floating point exceptions when computing the square
         * root */
      ...
      /* test whether the boundary length is preserved */
      {
        ...
        float l_before = 2. * a, l_after = DIST3( vl, ve ) + DIST3( ve, vr ),
              eps3 = simp_data->tolerance *
                     eps1; /* this is a relative accuracy
...
float _dxfFastCompactness3DTriangle2( Vertex *tri, float *triangle_area )
{
  ...
  {
    double m0 = tri[1][1] - tri[0][1], m1 = tri[2][2] - tri[1][2],
           m2 = tri[1][2] - tri[0][2], m3 = tri[2][1] - tri[1][1],
           m4 = tri[2][0] - tri[1][0], m5 = tri[1][0] - tri[0][0],
           n0 = m0 * m1 - m2 * m3, n1 = m2 * m4 - m5 * m1,
           n2 = m5 * m3 - m0 * m4;

    tmp = n0 * n0 + n1 * n1 + n2 * n2;

    if ( tmp > 0.0 )
      *triangle_area = sqrt( tmp ) / 2.;
...
int _dxfBoundaryCollapse2KndGeometricallyOk(
    SimpData *simp_data, int v0, int v1,
...

{
  ...
  float *s_comp0 = s_area0 + val0, min_compactness_before,
  ...
  while ( collapsible && i < val0 - 1 )
  {
    ...
    min_compactness_before =
        MIN( simp_data->compactness[star0[i + 1]], min_compactness_before );
    ...
    s_comp0[i + 1] =
        _dxfFastCompactness3DTriangle2( faceVert, s_area0 + i + 1 );
    ...
    collapsible =
        ( ( SCALPROD3( s_normal0[i + 1], simp_data->normal[star0[i + 1]] ) ) >
          min_scalprod_nor );
  ...
  if ( collapsible )
    ...
    collapsible =
        min_compactness_after > ( compactness_ratio * min_compactness_before );
...
/*
   ...
   update the potential data error value at the potential simplified vertex
   location

   */

int _dxfErrorTestDataOnSurfaceUpdate( SimpData *simp_data, int new_v, int old_v,
                                      int new_v2, int new_v3, float alpha_sv,
                                      float alpha_n_v2, float alpha_n_v3,
                                      int old_v1, int old_v2, int old_v3,
                                      float alpha_o_v1, float alpha_o_v2,
                                      float alpha_o_v3 )
{

  int do_collapse = 1;

  float o_alpha[3], n_alpha[3], *o_data[3], *n_data[3], o_data_error[3],
      data_difference, new_error_link, tmp_data_diff = 0, new_error_sv,
                                       old_error;
  ...
  for ( data_difference = 0., j = 0; j < simp_data->data_dim; j++ )
  {
    ...
    for ( k = 0; k < old_v; k++ )
      simp_data->vx_old_data[j] += o_alpha[k] * o_data[k][j];

    simp_data->vx_new_data[j] = 0.0;

    for ( k = 0; k < new_v; k++ )
      simp_data->vx_new_data[j] += n_alpha[k] * n_data[k][j];
  ...
  /* there are two cases, either the data dimension is one or it is larger than
     one
     ...
     floating point computations, I am avoiding taking this square root if I
     can.
     */

  if ( simp_data->data_dim == 1 )
  {
    data_difference = ( tmp_data_diff > 0.0 ) ? tmp_data_diff : -tmp_data_diff;
  }

  else if ( data_difference > 0.0 )
    data_difference = sqrt( data_difference );

  old_error = 0.0;

  for ( k = 0; k < old_v; k++ )
    old_error += o_alpha[k] * o_data_error[k];
  ...
  new_error_link = ( new_v == 3 )
                       ? alpha_n_v2 * simp_data->vx_data_error[new_v2] +
                             alpha_n_v3 * simp_data->vx_data_error[new_v3]
                       : ( new_v == 2 )
                             ? alpha_n_v2 * simp_data->vx_data_error[new_v2]
                             : 0.0;

  /* there is a special case if the simplified vertex is associated with an
     alpha that's zero: in this case, the error must be explained with the error
     at
     the link */
  ...
  else /* alpha_sv * error_sv + new_error_link > old_error + data_difference */
  {
    ...
    if ( new_error_sv > simp_data->vx_data_potential_err )
      simp_data->vx_data_potential_err = new_error_sv;

    do_collapse =
        ( simp_data->vx_data_potential_err <= simp_data->data_tolerance );
  }
...
float _dxfSegmentIntersection( Vertex A, Vertex B, Vertex C, Vertex D,
                               float *lambda, float *mu, int changed_AorB,
                               int changed_C, int changed_D )
{
  ...
  if ( changed_AorB )
  {
    ...
    if ( scale > 0. )
    {
      ...
      _dxfHouseholderPreMultiplicationDbl( (double *)w[1], 3, (double *)w[0], 3,
                                           2, work );
  ...
  else
  {
    if ( changed_C )
    {
      ...
      _dxfHouseholderPreMultiplicationDbl( (double *)w[1],
                                           3, /* apply Householder */
                                           (double *)w[0], 3, 1, work );
    }
    ...
    if ( changed_D )
    {
      ...
      _dxfHouseholderPreMultiplicationDbl( (double *)w[2],
                                           3, /* apply Householder */
                                           (double *)w[0], 3, 1, work );
    }
  }
...
int _dxfHouseholderPreMultiplication( float *A, /* input matrix, given column
                                                   by column */
...
int _dxfErrorWithinToleranceVBoundary2ndKnd( SimpData *simp_data, int v0,
                                             int v1, int *vstar0, int *vstar1,
                                             int val0, int val1 )
{
  ...
  float potential_err_pivot, largest_err_pivot = 0.,
  ...
  if ( simp_data->err_volume[v0] > tol_pivot ||
       simp_data->err_volume[v1] > tol_pivot )
    edge_collapsible = 0;
  ...
  if ( edge_collapsible )
  {
    ...
    dist =
        _dxfClosestPointOnEdge( simp_data->simplified_vertex, wp[0],
                                simp_data->vert[v0], simp_data->vert[v1], &t );

    _dxfErrorTestDataOnSurfaceInterpolate2( simp_data, v0, v1, t );

    potential_err_pivot = dist + t * simp_data->err_volume[v0] +
                          ( 1. - t ) * simp_data->err_volume[v1];
    ...
    if ( edge_collapsible )
    {
      ...
      dist = _dxfClosestPointOnEdge( simp_data->vert[v0], wp[0],
                                     simp_data->vert[vstar0[val0 - 1]],
                                     simp_data->simplified_vertex, &t );

      /* t * err[vl] + 1-t * err[vs] >= dist + err[v0] */

      if ( t < 1. )
      {
        potential_err_pivot = ( dist + simp_data->err_volume[v0] -
                                t * simp_data->err_volume[vstar0[val0 - 1]] ) /
                              ( 1. - t );
      ...
      else if ( simp_data->err_volume[vstar0[val0 - 1]] <
                dist + simp_data->err_volume[v0] )
      ...
      if ( edge_collapsible )
      {
        if ( simp_data->vx_data )
          ...
          edge_collapsible = _dxfErrorTestDataOnSurfaceUpdate(
              simp_data, 2, 1, vstar0[val0 - 1], 0, 1. - t, t, 0., /* vs, vl */
              v0, 0, 0, 1., 0., 0. );
        ...
        {
          potential_err_pivot =
              ( dist + simp_data->err_volume[v1] -
                t * simp_data->err_volume[vstar1[val1 - 1]] ) /
              ( 1. - t );

          UPDATE_ERR_PIVOT;
        }

        else if ( simp_data->err_volume[vstar1[val1 - 1]] <
                  dist + simp_data->err_volume[v1] )
        ...
        if ( simp_data->vx_data && edge_collapsible )
          ...
          edge_collapsible = _dxfErrorTestDataOnSurfaceUpdate(
              simp_data, 2, 1, vstar1[val1 - 1], 0, 1. - t, t, 0., /* vs, vl */
              v1, 0, 0, 1., 0., 0. );
        ...
        if ( edge_collapsible )
        {
          ...
          while ( edge_collapsible && i < val0 )
          {
            ...
            while ( edge_collapsible && j < val0 )
            {
              ...
              dist = _dxfSegmentIntersection(
                  simp_data->simplified_vertex, simp_data->vert[vstar0[i]],
                  simp_data->vert[v0], simp_data->vert[vstar0[j]], &lambda1,
                  &lambda2, !j, 0, 1 );
              ...
              if ( lambda2 >= 0. && lambda2 <= 1. ) /* inside 2 */
              {
                ...
                if ( simp_data->vx_data && lambda1 >= 0. && lambda1 <= 1. )
                  ...
                  edge_collapsible = _dxfErrorTestDataOnSurfaceUpdate(
                      simp_data, 2, 2, vstar0[i], 0, lambda1, 1. - lambda1, 0.,
...

                {
                  potential_err_pivot =
                      ( dist -
                        ( 1. - lambda1 ) * simp_data->err_volume[vstar0[i]] ) /
                      lambda1;
          ...
          if ( edge_collapsible )
          {
            ...
            while ( edge_collapsible && i < val1 )
            {
              ...
              while ( edge_collapsible && j < val1 )
              {
                ...
                dist = _dxfSegmentIntersection(
                    simp_data->simplified_vertex, simp_data->vert[vstar1[i]],
                    simp_data->vert[v1], simp_data->vert[vstar1[j]], &lambda1,
                    &lambda2, !j, 0, 1 );

                if ( lambda2 >= 0. && lambda2 <= 1. ) /* inside 2 */
                {

                  dist += lambda2 * simp_data->err_volume[v1] +
                          ( 1. - lambda2 ) * simp_data->err_volume[vstar1[j]];

                  if ( simp_data->vx_data && lambda1 >= 0. && lambda1 <= 1. )

                    edge_collapsible = _dxfErrorTestDataOnSurfaceUpdate(
                        simp_data, 2, 2, vstar1[i], 0, lambda1, 1. - lambda1,
                        0., v1, vstar1[j], 0, lambda2, 1. - lambda2, 0. );
  ...
  if ( edge_collapsible )
  {
    ...
    simp_data->tol_volume[v0] =
        MIN( simp_data->tol_volume[v0], simp_data->tol_volume[v1] );
   ...
   (were in the queue before, were deleted and are now reintered */
{
  ...
  *num_edg_added = 0;
  ...
  for ( i = 0; i < val; i++ )
  {
    int outside = ( simp_data->edge2index[estar[i]] ==
                    simp_data->heap_outside_index ),
...
int _dxfCollapseBoundaryEdge2ndKnd( SimpData *simp_data, int edg_num, int v0,
                                    int v1, int val0, int val1, int *star0,
                                    int *star1, int *vstar0, int *vstar1,
                                    int *estar0, int *estar1, Vertex *s_normal0,
                                    Vertex *s_normal1, float *s_area0,
                                    float *s_area1 )
{
  int n_edges_added[2], n_edges_reinstated = 0;
  ...
  n_edges_reinstated += _dxfReinstateEdgesInHeap(
      estar1 + 1, val1 - 1, simp_data, n_edges_added + 1 );
  ...
  /* replace the triangle normals, areas, and compactness in the simplified
   * stars */

  {
    register int i;

    float *s_comp0 = s_area0 + val0, *s_comp1 = s_area1 + val1;
                                                ...
                                                int move_simplified_vertex )
{
       ...
       ( edge_t[0] == -1 || edge_t[1] == -1 ) )
  {
         ...
         ( val0 + val1 - 2 < simp_data->valence_max ) )
    {
      ...
      bzero( star0, ( val0 + val1 ) * 3 * sizeof( int ) );
      ...
      {
        int *vstar0 = star0 + val0, *estar0 = vstar0 + val0,
            *star1 = estar0 + val0, *vstar1 = star1 + val1,
            *estar1 = vstar1 + val1;

        first_triangle = FATHER( first_triangle, simp_data->triangle_father );

        _dxfDirectedParentVertexStar( v0, first_triangle, val0, star0,
                                      simp_data, DIRECTION_COUNTER_CLOCKWISE );

        /* compute the star of v1 in reverse order */

        _dxfDirectedParentVertexStar( v1, first_triangle, val1, star1,
                                      simp_data, DIRECTION_CLOCKWISE );
        ...
        if ( _dxfManifoldLinkTest( vstar0 + 1, vstar1 + 1, val0 - 1,
                                   val1 - 1 ) )
        {
          Vertex *s_normal1;
          float *s_area1;
          ...
          bzero( s_normal0, ( val0 + val1 ) * sizeof( Vertex ) );

          s_normal1 = s_normal0 + val0;

          s_area0 = (float *)simp_data->edge_star_buffer_fl;

          bzero( s_area0, 2 * ( val0 + val1 ) * sizeof( float ) );
          ...
          if ( ( _dxfSimplifiedBoundaryVertexLocation(
                   simp_data, v0, v1, vstar0, vstar1, val0, val1,
                   move_simplified_vertex ) ) &&

               /* compute the difference in orientation for the
                  triangle normals before and after simplification */

               /* compute the minimum compactnesses before and after
                  simplification */
               ( _dxfBoundaryCollapse2KndGeometricallyOk(
                   simp_data, v0, v1, star0, vstar0, val0, s_normal0, s_area0,
                   DIRECTION_COUNTER_CLOCKWISE, simp_data->min_scalprod_nor,
                   simp_data->compactness_ratio ) ) &&
               ( _dxfBoundaryCollapse2KndGeometricallyOk(
                   simp_data, v1, v0, star1, vstar1, val1, s_normal1, s_area1,
                   DIRECTION_CLOCKWISE, simp_data->min_scalprod_nor,
                   simp_data->compactness_ratio ) ) ) /* geometry test */

          {
            /* compute the tolerance after simplification */

            if ( _dxfErrorWithinToleranceVBoundary2ndKnd(
                    simp_data, v0, v1, vstar0, vstar1, val0, val1 ) )
            {

              simp_data->num_edg_collapsed += 2;

              simp_data->num_edg_weights += _dxfCollapseBoundaryEdge2ndKnd(
                  simp_data, edge_num, v0, v1, val0, val1, star0, star1, vstar0,
                  vstar1, estar0, estar1, s_normal0, s_normal1, s_area0,
                  s_area1 );
...
int _dxfManifoldLinkTest1stKnd( int v0, int val0, int *star1, int *link1,
                                int val1, SimpData *simp_data )
{
  ...
  /*
     ...
     we take from vstar0 as many vertices as val0 (remember that vstar0 has one
     element
...

  {
    ...
    int i = 0, tri_v_rotated[3], tri_v_parents[3],

        /* rotated edge vertices and adjacent triangles: */
        edge_v_rotated[2], edge_v_parents[2], edge_t_rotated[2],
        triangle = star1[0], edge;
...
int _dxfBoundaryCollapse1stKndGeometricallyOk( SimpData *simp_data, int v0,
                                               int *star0, int *vstar0,
                                               int val0, Vertex *s_normal0,
                                               float *s_area0,
                                               float min_scalprod_nor,
...

{
  ...
  min_compactness_before =
      MIN( simp_data->compactness[star0[1]], simp_data->compactness[star0[0]] );
...
int _dxfTriangle3DBarycentricCoordinates2( Vertex *tri, Vertex w, Vertex wp,
                                           Vertex bary, float *residual )
...
*/
{
  ...
  inside = !( bary[0] < eps_vertex_outside || bary[1] < eps_vertex_outside ||
...
int _dxfClosestPointOnTriangle( Vertex *tri, Vertex w, Vertex wp, Vertex bary,
                                float *dist )
{
...
int _dxfErrorWithinToleranceVBoundary1stKnd( SimpData *simp_data, int v0,
                                             int v1, int *vstar1, int val1 )
{
  int edge_collapsible = 1;

  register int i;

  float tol_pivot, potential_err_pivot, largest_err_pivot = 0., dist, lambda1,
                                        lambda2;
  ...
  if ( edge_collapsible )
  {
    Vertex triangle[3],
        ...
        wp[2], alpha[SIMPLIFY_VALENCE_MAX1]; /* val0 + val1 -4 <= valence max
                                                <=> val0, val1 <= valence_max +1
    ...
    bzero( (char *)vs, sizeof( Vertex ) );

    bzero( (char *)wp, 2 * sizeof( Vertex ) );

    bzero( (char *)alpha, SIMPLIFY_VALENCE_MAX1 * sizeof( Vertex ) );
    ...
    if ( simp_data->vx_data )
      memcpy( simp_data->vx_data_potential_values,
              simp_data->vx_data + v0 * simp_data->data_dim,
              simp_data->data_dim * sizeof( float ) );

    /* 1) determine the closest point to the vertex v1 on the triangles of star1
     * after simplification
     */
                       ...
                       v*[0]____\/____
                          ...
                          v*[1]/  \
                               *[2]
                               ...
                               both triangles *[0] and *[1] disappear after edge
  collapse
    ...
    while ( i < val1 - 1 )
    {
      ...
      _dxfClosestPointOnTriangle( triangle, simp_data->vert[v1], wp[0],
                                  alpha[i], &dp );
      ...
      if ( alpha[i][0] > 0. )
      {
        dp = ( dp - alpha[i][1] * simp_data->err_volume[vstar1[i]] -
      ...
      else
      {
        /* otherwise v1 projects onto the link edge of "triangle"
           ...
           but it is possible that the error values at the link are already
           sufficient */
...

    {
      ...
      if ( edge_collapsible && simp_data->vx_data )
        ...
        edge_collapsible = _dxfErrorTestDataOnSurfaceUpdate(
            simp_data, 3, 1, vstar1[i_min], vstar1[i_min + 1], alpha[i_min][0],
            alpha[i_min][1], alpha[i_min][2], v1, 0, 0, 1., 0., 0. );
    }
        ...
        while ( edge_collapsible && j < val1 )
        {
          ...
          if ( lambda2 >= 0. && lambda2 <= 1. ) /* inside 2 */
          {
            ...
            if ( simp_data->vx_data && lambda1 >= 0. && lambda1 <= 1. )
              ...
              edge_collapsible = _dxfErrorTestDataOnSurfaceUpdate(
                  simp_data, 2, 2, vstar1[i], 0, lambda1, 1. - lambda1, 0., v1,
                  vstar1[j], 0, lambda2, 1. - lambda2, 0. );
...

            {
              potential_err_pivot =
                  ( dist +
                    ( lambda1 - 1. ) * simp_data->err_volume[vstar1[i]] ) /
                  lambda1;
...
int _dxfCollapseBoundaryEdge1stKnd( SimpData *simp_data, int edg_num, int v0,
                                    int v1, int val0, int val1, int *star1,
                                    int *vstar1, int *estar1, Vertex *s_normal1,
                                    float *s_area1 )
{
  int n_edges_added = 0, n_edges_reinstated = 0, et, eb, tri_v_rotated[3],
      tri_v_parents[3];
      ...
      labeling system          *[0]\  /
                           v*[0]____\/____
                              ...
                              v*[1]/  \
                                ...
                                both triangles *[0] and *[1] disappear after
     edge collapse
  ...
  */
  ...
  /* take the triangle star1[0], orient it with respect to v0
     and use the oriented triangle vertices to determine the next edge counter
     clockwise starting
     from v0: et */

  _dxfRotateParentTriangle( v0, simp_data->vertex_father,
                            simp_data->triangles + 3 * star1[0], tri_v_parents,
                            tri_v_rotated );

  et = (int)( ( (EdgeS *)_dxfFindEdge( simp_data->e_table, tri_v_rotated[0],
                                       tri_v_rotated[2] ) ) -
              simp_data->edge_array );

  et = FATHER( et, simp_data->edge_father );

  /* do the same with the triangle star1[1] to determine the next edge clockwise
     starting
     from v0: eb */

  _dxfRotateParentTriangle( v0, simp_data->vertex_father,
                            simp_data->triangles + 3 * star1[1], tri_v_parents,
                            tri_v_rotated );

  eb = (int)( ( (EdgeS *)_dxfFindEdge( simp_data->e_table, tri_v_rotated[0],
                                       tri_v_rotated[1] ) ) -
              simp_data->edge_array );
  ...
  if ( simp_data->edge2index[estar1[val1 - 1]] !=
       simp_data->heap_outside_index )
    simp_data->num_edg_remaining_4_testing--;

  simp_data->edge2index[estar1[1]] = simp_data->edge2index[estar1[val1 - 1]] =
      -4;
  simp_data->edge2index[edg_num] = -7;

  /* 5) reinstate all but the two edges and last edge  in the star of v1 */

  n_edges_reinstated += _dxfReinstateEdgesInHeap( estar1 + 2, val1 - 3,
                                                  simp_data, &n_edges_added );
  ...
  /* replace the triangle normals, areas and compactness in the simplified star
   * of v1 */
                                                ...
                                                int val0, int val1 )
{
  ...
  if ( ( val0 + val1 - 4 >= 1 ) &&
       ( val0 + val1 - 4 < simp_data->valence_max ) )
  {
    int *edge_t =
            (int *)SurfEdgeGetTriangles( simp_data->edge_array + edge_num ),
        first_tri = edge_t[0], second_tri = edge_t[1],
    ...
    if ( _dxfManifoldLinkTest1stKnd( v0, val0, star1, vstar1, val1,
                                     simp_data ) )
    {
      ...
      bzero( s_normal1, val1 * sizeof( Vertex ) );

      /* initialize the storage pointer for the buffer s_area1 */

      s_area1 = (float *)simp_data->edge_star_buffer_fl;

      bzero( s_area1, 2 * val1 * sizeof( float ) );

      if ( _dxfBoundaryCollapse1stKndGeometricallyOk(
              simp_data, v1, star1, vstar1, val1, s_normal1, s_area1,
              simp_data->min_scalprod_nor, simp_data->compactness_ratio ) )

      {
        /* perform the error tolerance test */

        if ( _dxfErrorWithinToleranceVBoundary1stKnd( simp_data, v0, v1, vstar1,
                                                      val1 ) )
        {
          simp_data->num_edg_collapsed += 3;

          simp_data->num_edg_weights += _dxfCollapseBoundaryEdge1stKnd(
              simp_data, edge_num, v0, v1, val0, val1, star1, vstar1, estar1,
              s_normal1, s_area1 );
...
int _dxfCollapseTopologicallyFeasible( SimpData *simp_data, int *vstar0,
                                       int *vstar1, u_short val0, u_short val1 )
{
...
void _dxfBuildParentEdgeStars( EdgeS *edg, int v0, int v1, u_short val0,
                               u_short val1, int *star0, int *star1,
                               SimpData *simp_data )
...
void _dxfParentVertexStar( int vf, int t0, u_short val, int *star,
                           SimpData *simp_data )
...
static void CentroidEdgeLink( Vertex centroid, Vertex *vert, int val0,
                              int val02, int val014 );
                              ...
                              int val02, int val014 )
{
  ...
  bzero( (char *)centroid, sizeof( Vertex ) );
...
static void BuildEdgeStarVal1Is3( Vertex *, Face *, int, int, int, int, int,
                                  int *, Vertex * );
...
static void BuildEdgeStarVal1Is3( Vertex *vert, Face *face, int v0, int v1,
                                  int val0, int val01, int val02, int *vstar0,
                                  Vertex *surfvert )
{
  register int i, i1;
...
void _dxfBuildEdgeStar( Vertex *star_vert, Face *star_face, Plane *star_plane,
                        float *star_areas, float *star_comp, int v0, int v1,
                        u_short val0, u_short val1, int *star0, int *star1,
                        SimpData *s )
...

{
  ...
  int star_val = val0 + val1 - 2, val013 = star_val - 1, val014 = val013 - 1,
      val01 = val0 - 1, val02 = val01 - 1, *vstar0 = star0 + 2 * val0,
      *vstar1 = star1 + 2 * val1;
  ...
  if ( val1 == 3 )
  {
    BuildEdgeStarVal1Is3( star_vert, star_face, v0, v1, val0, val01, val02,
                          vstar0, s->vert );
  }
  else
  {
...
.. /  \ val0-1          /  \val0+1
  ...
  _dxfCopyFaceNormalsAreasCompactness( star_plane, star_areas, star_comp, star0,
                                       star1, val0, val013, s->normal, s->area,
                                       s->compactness );
...
void _dxfCopyFaceNormalsAreasCompactness( Plane *plane, float *s_area,
                                          float *s_comp, int *star0, int *star1,
                                          int val0, int val013,
                                          Vertex *t_normal, float *t_area,
                                          float *t_comp )
{
...
double _dxfStarPlaneEquationsAndVolume( Vertex *star_vert, Face *star_face,
                                        Plane *star_plane, float *star_areas,
                                        int star_val )
/*...........................................................................*/
/*
...
/*...........................................................................*/
{
  ...
  for ( j = 0; j < star_val; j++ )
  {
    ...
    for ( k = 0; k < 3; k++ )
      ...
      star_plane[j][3] -=
          SCALPROD3( star_plane[j], star_vert[star_face[j][k]] );
...
int _dxfPositionVertexLSConstraints2( Plane *star_plane, u_short star_val,
...
/*...........................................................................*/
{
  ...
  double A1[SIMPLIFY_VALENCE_MAX4], A2[SIMPLIFY_VALENCE_MAX4],
  ...
  double
      ...
      delta = star_plane[star_val][3] * star_plane[star_val][2];
...
void _dxfFPlaneForIdenticalVolume( u_short val0, u_short val1, Plane p,
                                   Vertex *star_vert, Face *star_face,
                                   float star_volume )
{
  static Vertex n;
  ...
  bzero( (char *)p, sizeof( Vertex ) );

  for ( i = 1; i <= val02; i++ )
  {
    u_short v1 = star_face[i][1], v2 = star_face[i][2];
...
void _dxfSimplifiedVertexLocation(
    SimpData *s, u_short val0, u_short val1, u_short star_val,
    Vertex *star_vert, Face *star_face, Plane *star_plane, float *star_areas,

    /* workspace needed for the householder transform: */
    float *work, u_char method )
/*
...
*/
{
  ...
  else if ( method == 1 )
  {
    ...
    float star_volume, /* volume of the edge star (x 6) */
        sign;

    int star_val_1 = star_val + 1,
        size_planes = ( star_val_1 ) * sizeof( Plane );

    float *star_plane_sav = (float *)( star_plane + star_val_1 );

    static Vertex v, z = {0, 0, 1.};

    star_volume = _dxfStarPlaneEquationsAndVolume(
        star_vert, star_face, star_plane, star_areas, star_val );
    ...
    _dxfFPlaneForIdenticalVolume( val0, val1, star_plane[star_val], star_vert,
                                  star_face, star_volume );
    ...
    _dxfHouseholderPreMultiplication( (float *)star_plane, 4, v, 3, star_val_1,
                                      work );
    ...
    _dxfPositionVertexLSConstraints2( star_plane, star_val,
                                      s->simplified_vertex );
  ...
  else if ( method == 2 )
  {
    ...
    for ( j = 0; j < 3; j++ )
      ...
      s->simplified_vertex[j] =
          ( star_vert[val01][j] + star_vert[val013][j] ) / 2.;
  }
...
int _dxfCollapseGeometricallyFeasible( Vertex *svert, Face *sface,
                                       Plane *splane, float *old_comp,
                                       float new_comp_min, Vertex *snor0,
                                       Vertex *snor1, u_short val0,
                                       u_short sval, float min_scalprod,
                                       float compactness_ratio )
{
  int feasible = 1;
...
int _dxfNoFlipOverCheck( u_short first_face, u_short last_face, Vertex *nor,
                         Plane *plane, float min_scalprod_nor )
{
...
int _dxfErrorWithinToleranceV( SimpData *simp_data, Vertex *star_vert,
                               Face *star_face, Plane *star_plane, int *star0,
                               int *star1, int val0, int val1 )

{
  ...
  int edge_collapsible = 1,
      ...
      /* vertex stars */
      *vstar0 = star0 + 2 * val0, *vstar1 = star1 + 2 * val1, i, j, i_min = -1;

  static Vertex vs[1], /* local copy of the simplified vertex */
      a_triangle[3], wp[2], alpha[SIMPLIFY_VALENCE_MAX1];
  /* val0 + val1 -4 <= valence max <=> val0, val1 <= valence_max +1
     because val0 >=3 and val1 >=3 */

  static float dp[2];

  float tol_pivot, potential_err_pivot, largest_err_pivot = 0., d_min, lambda1,
                                        lambda2;
  ...
  if ( edge_collapsible )
  {
    ...
    dp[0] += alpha[0][0] * simp_data->err_volume[vstar0[0]] +
    ...
    dp[1] += alpha[1][0] * simp_data->err_volume[vstar1[0]] +
    ...
    {
      /* use the barycentric coordinates to interpolate the potential data
       * values */

      _dxfErrorTestDataOnSurfaceInterpolate3( simp_data, vstar0[0], vstar1[0],
                                              vstar1[1], alpha[0][0],
                                              alpha[0][1], alpha[0][2] );
    ...
    else
    {
      ...
      _dxfErrorTestDataOnSurfaceInterpolate3( simp_data, vstar1[0], vstar0[0],
                                              vstar0[1], alpha[1][0],
                                              alpha[1][1], alpha[1][2] );
    ...
    if ( edge_collapsible )
    {
      ...
      while ( i < val0 - 1 )
      {
        ...
        _dxfClosestPointOnTriangle( qi, simp_data->vert[vstar1[0]], wp[0],
                                    alpha[i], &dp[0] );

        if ( alpha[i][0] > 0. )

        /* If v0 does not project on the link, then the projection is useful and
           can produce
           a valid constraint */

        {
          dp[0] = ( dp[0] + simp_data->err_volume[vstar1[0]] -
                    alpha[i][1] * simp_data->err_volume[vstar0[i]] -
                    alpha[i][2] * simp_data->err_volume[vstar0[i + 1]] ) /
                  alpha[i][0];

          if ( i_min == -1 || dp[0] < d_min )
          {
            d_min = dp[0];
            i_min = i;
          }
        ...
        else
        {
          /* otherwise v0 projects onto the link edge of "triangle"
             the epsilon value at the simplified vertex cannot compensate
             but it is possible that the error values at the link are high
             enough
               ...
               dp[0] )
          {
            ...
            i_min = i;
            i = val0 -
                1; /* next i will be = val0 and the loop will immediately stop
                      */
            d_min = 0.;
          }
        }
      ...
      if ( i_min >= 0 )
      {
        ...
        if ( simp_data->vx_data && edge_collapsible )
          ...
          edge_collapsible = _dxfErrorTestDataOnSurfaceUpdate(
              simp_data, 3, 1, vstar0[i_min], vstar0[i_min + 1],
              alpha[i_min][0], alpha[i_min][1], alpha[i_min][2], vstar1[0], 0,
              0, 1., 0., 0. );
        ...
        edge_collapsible = 0; /* no projection was useful */
      ...
      if ( edge_collapsible )
      {
        ...
        while ( edge_collapsible && i < val0 )
        {
          ...
          while ( edge_collapsible && j < val0 )
          {
            ...
            d_min = _dxfSegmentIntersection(
                vs[0], simp_data->vert[vstar0[i]], simp_data->vert[vstar1[0]],
                simp_data->vert[vstar0[j]], &lambda1, &lambda2, !j, 0, 1 );
            ...
            if ( lambda2 >= 0. && lambda2 <= 1. ) /* inside 2 */
            {
...

              {
                potential_err_pivot =
                    ( d_min +
                      ( lambda1 - 1. ) * simp_data->err_volume[vstar0[i]] ) /
                    lambda1;
        ...
        if ( edge_collapsible )
        {
          ...
          while ( i < val1 - 1 )
          {
            ...
            _dxfClosestPointOnTriangle( qi, simp_data->vert[vstar0[0]], wp[0],
                                        alpha[i], &dp[0] );

            if ( alpha[i][0] > 0. )

            {
              dp[0] = ( dp[0] + simp_data->err_volume[vstar0[0]] -
...

          {
            ...
            if ( simp_data->vx_data && edge_collapsible )
              ...
              edge_collapsible = _dxfErrorTestDataOnSurfaceUpdate(
                  simp_data, 3, 1, vstar1[i_min], vstar1[i_min + 1],
                  alpha[i_min][0], alpha[i_min][1], alpha[i_min][2], vstar0[0],
                  0, 0, 1., 0., 0. );
          }
          else
            edge_collapsible = 0; /* no projection was useful */
          ...
          if ( edge_collapsible )
          {
            ...
            while ( edge_collapsible && i < val1 - 1 )
            {
              ...
              while ( edge_collapsible && j < val1 )
              {
                ...
                d_min = _dxfSegmentIntersection(
                    vs[0], simp_data->vert[vstar1[i]],
                    simp_data->vert[vstar0[0]], simp_data->vert[vstar1[j]],
                ...
                if ( lambda2 >= 0. && lambda2 <= 1. ) /* inside 2 */
                {
                  ...
                  if ( simp_data->vx_data && lambda1 >= 0. && lambda1 <= 1. )
                    ...
                    edge_collapsible = _dxfErrorTestDataOnSurfaceUpdate(
                        simp_data, 2, 2, vstar1[i], 0, lambda1, 1. - lambda1,
                        0., vstar0[0], vstar1[j], 0, lambda2, 1. - lambda2,
                        0. );
...
/*...........................................................................*/
{
  ...
  for ( i = first_face, i1 = 0; i < last_face; i++, i1++ )
  {
    ...
    u_short v1 = sface[i][1], v2 = sface[i][2];
...
void _dxfReplaceFaceNormals( int *star, Vertex *new_nor, float *new_area,
                             float *new_comp, Vertex *nor, float *area,
                             float *comp, u_short val )
{
...
int _dxfCollapseEdge( SimpData *simp_data, int v0, int v1, int *star0,
                      int *star1, u_short val0, u_short val1, Vertex *nor0,
                      Vertex *nor1, float *area0, float *area1, float *comp0,
                      float *comp1 )

/*...........................................................................*/
/*

    Perform the simplification of an interior edge, and update the edge heap, by
 removing
...
/*...........................................................................*/
{
  int *estar0 = star0 + val0, *vstar0 = estar0 + val0, *estar1 = star1 + val1,
      *vstar1 = estar1 + val1, n_edges_added[2], n_edges_reinstated = 0;
  ...
  simp_data->edge_father[estar0[0]] = simp_data->edge_father[estar1[val1 - 1]] =
      estar0[1];
  ...
  _dxfRemoveEdgesFromHeap( estar0 + 1, val0 - 1, simp_data->edge2index,
                           simp_data );

  /* 3.2 remove all edges but v0-v1 in the star of v1 */

  _dxfRemoveEdgesFromHeap( estar1 + 1, val1 - 1, simp_data->edge2index,
                           simp_data );
  ...
  simp_data->edge2index[estar1[val1 - 1]] = simp_data->edge2index[estar1[1]] =
      -4;
  ...
  n_edges_reinstated = _dxfReinstateEdgesInHeap( estar0 + 1, val0 - 1,
                                                 simp_data, n_edges_added );

  /* 2.3 reinstate in the heap all edges but 0 and 1 and val1-1 in estar1 */

  n_edges_reinstated += _dxfReinstateEdgesInHeap(
      estar1 + 2, val1 - 3, simp_data, n_edges_added + 1 );
  ...
  /* 4. replace the face normals, areas and compactness for the simplified stars
   * with the new ones */

  _dxfReplaceFaceNormals( star0, nor0, area0, comp0, simp_data->normal,
                          simp_data->area, simp_data->compactness, val0 );
  _dxfReplaceFaceNormals( star1, nor1, area1, comp1, simp_data->normal,
                          simp_data->area, simp_data->compactness, val1 );
...
{
  int e, num_edg_tested = 0, /* number of edges extracted from the heap */
      num_edg_added_to_heap = 0;

  /* allocate buffers for storing various information on the vertex and edge
   * stars */

  /* since the valence of the simplified vertex can be as low as  v0 + v1 - 4
     (for an interior vertex)
     and is less than SIMPLIFY_VALENCE_MAX, assuming that v0 and v1 >= 1 we
     can be sure that the valence of v0 and v1 that are accepted for processing
     is <= SIMPLIFY_VALENCE_MAX+3.
     conservatively, we use SIMPLIFY_VALENCE_MAX+4 as a bound */

  int vertex_star_buffer_size = 6 * SIMPLIFY_VALENCE_MAX4 * sizeof( int ),
  ...
  while ( ( num_edg_added_to_heap = _dxfBuildEdgeHeap( simp_data ) ) )
  {
    ...
    do
    {
      ...
      if ( e != -1 )
      {
        ...
        int collapsible = 0;
        EdgeS *edg = simp_data->edge_array + e;
        int *endpoints = SurfEdgeGetVertices( edg ),
            v0 = FATHER( endpoints[0], simp_data->vertex_father ),
            v1 = FATHER( endpoints[1], simp_data->vertex_father );
        u_short val0 = simp_data->valence[v0], val1 = simp_data->valence[v1];

        num_edg_tested++;
        /* mark the edge as removed from the heap: give an index such that
         * HeapDelete would fail + 1*/
        ...
        if ( simp_data->simplify_boundary && simp_data->boundary_vert[v0] &&
             simp_data->boundary_vert[v1] )

          collapsible = _dxfCollapsibilityTestsBoundaryEdge2ndKind(
              simp_data, e, v0, v1, val0, val1,
              simp_data->preserve_boundary_length );

        /* treat a boundary vertex of the fisrt kind */

        else if ( simp_data->simplify_boundary &&
                  ( simp_data->boundary_vert[v0] ||
                    simp_data->boundary_vert[v1] ) )

          collapsible = _dxfCollapsibilityTestsBoundaryEdge1stKind(
              simp_data, e, v0, v1, val0, val1 );
        ...
        else
        {
               ...
               ( val0 + val1 - 4 < simp_data->valence_max ) )
          {
            ...
            int *star0 = (int *)vertex_star_buffer, *estar0 = star0 + val0,
                *vstar0 = estar0 + val0, *star1 = vstar0 + val0,
                *estar1 = star1 + val1, *vstar1 = estar1 + val1;

            bzero( star0, 3 * ( val0 + val1 ) * sizeof( int ) );

            /* 3) construct the edge star */

            _dxfBuildParentEdgeStars( edg, v0, v1, val0, val1, star0, star1,
                                      simp_data );

            /* 3.1) test whether the simplification is topologically feasible */

            if ( _dxfCollapseTopologicallyFeasible( simp_data, vstar0, vstar1,
                                                    val0, val1 ) )
            {
              /* 3.2) compute a simplified vertex, and test whether the
                 simplification is
                       geometrically possible, if not, go to END */

              /* the number of vertices is given by val0 + val1 - 2 and the
                 number of
                 triangles is also val0 + val1 - 2 */

              u_short sval = val0 + val1 - 2, sval1 = sval + 1,
                      val02 = val0 - 2, val12 = val1 - 2;

              /* BUILD THE EDGE STAR */

              Vertex *svert = (Vertex *)edge_star_buffer_vx,
                     *snor0 = svert + sval1, *snor1 = snor0 + val02;
              Face *sface = (Face *)edge_star_buffer_fa;
              Plane *splane = (Plane *)edge_star_buffer_pl;
              float *scomp0 = (float *)edge_star_buffer_fl,
                    *sarea0 = scomp0 + val02, *scomp1 = sarea0 + val02,
                    *sarea1 = scomp1 + val12, *scomp = sarea1 + val12,
                    *sarea = scomp + sval, *wspace = sarea + sval,
                    min_comp_after;

              bzero( (char *)svert, edge_star_buffer_vxsize );
              bzero( (char *)sface, edge_star_buffer_fasize );
              bzero( (char *)splane, edge_star_buffer_plsize );
              bzero( (char *)scomp0, edge_star_buffer_flsize );

              /* the new origin is placed at the centroid of the edge link */

              _dxfBuildEdgeStar( svert, sface, splane, sarea, scomp, v0, v1,
                                 val0, val1, star0, star1, simp_data );

              _dxfSimplifiedVertexLocation( simp_data, val0, val1, sval, svert,
                                            sface, splane, sarea, wspace,
                                            simp_data->preserve_volume );
              ...
              _dxfSimplifiedStarNormals( sface, svert, snor0, sarea0, scomp0,
                                         simp_data->simplified_vertex, 1,
                                         val0 - 1 );
              _dxfSimplifiedStarNormals( sface, svert, snor1, sarea1, scomp1,
                                         simp_data->simplified_vertex, val0,
                                         sval );
              ...
              if ( _dxfCollapseGeometricallyFeasible(
                      svert, sface, splane, scomp, min_comp_after, snor0, snor1,
                      val0, sval, simp_data->min_scalprod_nor,
                      simp_data->compactness_ratio ) )

              {
                /* 3.3) if collapsible, test whether the new errors are within
                   the
                   tolerance limits  */

                collapsible = _dxfErrorWithinToleranceV(
                    simp_data, svert, sface, splane, star0, star1, val0, val1 );
...

                {
                  ...
                  simp_data->num_edg_weights += _dxfCollapseEdge(
                      simp_data, v0, v1, star0, star1, val0, val1, snor0, snor1,
                      sarea0, sarea1, scomp0, scomp1 );
                }
                else
                {
            ...
            {/* topologically infeasible */
              ...
              /* this condition will not be modified even if the surface is
                 simplified around
                 the edge */
          ...
          else
          {
            ...
            /* If the valence is too high or too low, reject also for topology,
