...
int m_Integer( Object *in, Object *out )
{
...
Error _dxfinteger_base( Object *in, Object *out, int islist )
{
  ...
  if ( min != DXD_MAX_FLOAT )
    min = ceil( (double)min );
  if ( max != -DXD_MAX_FLOAT )
    max = floor( (double)max );
  /* for case when range of data is less than 1 set min=max */
  if ( min != DXD_MAX_FLOAT && max != -DXD_MAX_FLOAT && min > max )
    min = max;

  /* cache attributes */
  if ( !_dxfinteract_float( id, idobj, &min, &max, &incr, NULL, label, 0,
                            method, &nitem, iprint ) )
    return ERROR;
      ...
      else if ( change == 1 )
      {
        ...
        else
        {
          for ( i = 0; i < item; i++ )
          {
            ...
            valtemp = val;
            if ( in[4] )
              valtemp = CLAMPMIN( val, min );
            if ( in[5] )
              valtemp = CLAMPMAX( val, max );
            if ( val != valtemp )
              iprint[4] = 1;
            ival[i] = valtemp;
          }
...
static Error int_reset( float min, float max, int item, start_type start,
                        int *ivalue )
{
  int i;
