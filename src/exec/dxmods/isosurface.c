...
/***********************************************************************/
/*
 * $Header: /src/master/dx/src/exec/dxmods/isosurface.c,v 1.6 2003/07/11
 * 05:50:35 davidt Exp $
 */
...
#define DXErrorGoto4( e, s, a, b, c ) \
  {                                   \
    DXSetError( e, s, a, b, c );      \
    goto error;                       \
  }
...
static iso_arg_type ISO_ARG_INITIALIZER = {
    ISOSURFACE, NO_NORMALS, -1,            NULL,           ORIGINAL_DATA, NULL,
    NULL,       0,          0,             0,              -1,            -1.0,
    NULL,       0,          DXD_MAX_FLOAT, -DXD_MAX_FLOAT, NULL,          0};

Error m_Isosurface( Object *in, Object *out )
{
...
#define O_surface out[0]
  ...
  if ( !I_data )
    DXErrorGoto2( ERROR_MISSING_DATA, "#10000",
                  /* %s must be specified */ "'data' parameter" );
  ...
  if ( !I_value )
  {
    ...
    else
    {
      if ( !DXExtractInteger( I_number, &iso_arg.number ) ||
           ( iso_arg.number < 1 ) )
        DXErrorGoto2( ERROR_BAD_PARAMETER,
  ...
  else
  {
    ...
    if ( ERROR == ( _dxd_user_def_values = (float *)DXAllocate(
                        sizeof( float ) * iso_arg.number ) ) ||
         !DXExtractParameter( I_value, TYPE_FLOAT, 1, iso_arg.number,
                              (Pointer)_dxd_user_def_values ) )
      goto error;

    if ( I_number )
      DXWarning( "#4110", /* %s has been specified; %s assignment ignored */
  ...
  if ( I_flag )
  {
    if ( !DXExtractInteger( I_flag, &add_normals ) || ( add_normals < 0 ) ||
         ( add_normals > 1 ) )
      DXErrorGoto2( ERROR_BAD_PARAMETER, "#10070",
                    /* %s must be either 0 or 1 */ "'flag' parameter" )
  ...
  if ( add_normals && I_gradient )
  {
    ...
    if ( ( ( class = DXGetObjectClass( I_gradient ) ) != CLASS_FIELD ) &&
         ( ( class != CLASS_GROUP ) ||
           ( DXGetGroupClass( (Group)I_gradient ) != CLASS_COMPOSITEFIELD ) ) )
      DXErrorGoto2( ERROR_DATA_INVALID, "#10191",
                    /* %s must be a field */ "'gradient'" );

    if ( !DXMapCheck( I_data, I_gradient, "positions", &grad_type, &grad_cat,
                      &grad_rank, grad_shape ) )
    {
      DXAddMessage( "#10310", /* %s does not match hierarchy of %s */
    ...
    if ( ( grad_type != TYPE_FLOAT ) || ( grad_cat != CATEGORY_REAL ) ||
      ...
      DXErrorGoto3( ERROR_DATA_INVALID, "#10230", /* %s must be a %d-vector */
                    "'gradient' parameter data", 3 );

    if ( ERROR == ( iso_arg.gradient_interpolator = DXNewInterpolator(
                        I_gradient, INTERP_INIT_PARALLEL, -1.0 ) ) )
    {
      DXAddMessage( "Some types of external Gradient are not supported." );
      DXAddMessage( "Try removing the 'gradient' parameter setting." );
  ...
  iso_arg.normal_type =
      ( add_normals ) ? ( ( iso_arg.gradient_interpolator ) ? GRADIENT_NORMALS
                                                            : NORMALS_COMPUTED )
                      : NO_NORMALS;
  ...
  if ( I_direction )
  {
    if ( !DXExtractInteger( I_direction, &iso_arg.normal_direction ) ||
         ( ( iso_arg.normal_direction != -1 ) &&
           ( iso_arg.normal_direction != 1 ) ) )
      DXErrorGoto4( ERROR_BAD_PARAMETER,
