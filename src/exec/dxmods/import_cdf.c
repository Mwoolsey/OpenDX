...
static Error queryCDFvars( Infocdf cdfp, Infovar *vpdata, Infovar *vploc,
                           Infovar *vptime, char **varname, int startframe,
...
static Error completeVarsInfo( Infocdf cdfp, Infovar vpdata, Infovar vploc,
                               Infovar vptime );
...
static Field impcdffield( int rec, Infocdf cdfp, Infovar vpdata, Array pos,
                          Array con, int *posdel, int *condel );
static Object impcdf0dim( Infocdf cdfp, Infovar vpdata, Array pos,
                          int *posdel );
                    ...
                    int *endframe, int *deltaframe )
{
  ...
  else
  {
    lframe = *endframe;
    if ( lframe < 0 )
      DXErrorGoto( ERROR_BAD_PARAMETER, "Last frame cannot be less than zero" );
    if ( lframe < fframe )
      DXErrorGoto( ERROR_BAD_PARAMETER,
  ...
  else
  {
    if ( *deltaframe <= 0 )
    {
      DXSetError( ERROR_BAD_PARAMETER, "Delta value has to be greater than 0" );
      goto error;
    }
    else
      iframe = *deltaframe;
  ...
  if ( !( obj = impcdfgroup( filename, fieldlist, fframe, lframe, iframe ) ) )
    goto error;
...

{
  ...
  if ( queryCDFvars( cdfp, &vpdata, &vploc, &vptime, varname, startframe,
                     endframe, deltaframe ) == ERROR )
    goto error;

  if ( cdfp->maxRec > 0 )
  {
    if ( ( timeval = (double *)DXAllocate( ( cdfp->maxRec + 1 ) *
                                           sizeof( double ) ) ) == NULL )
      goto error;
...
static Error loadSerPos( Infocdf cdfp, Infovar vptime, double *timeval )
{
  ...
  else if ( vptime != NULL && cdfp->maxRec > 0 )
  {
    ...
    if ( ( d = (char *)DXAllocate( ( cdfp->maxRec + 1 ) *
                                   vptime->numbytes ) ) == NULL )
      goto error;
    ...
    status =
        CDFvarHyperGet( vptime->id, vptime->varNum, 0, cdfp->maxRec + 1, 1,
                        vptime->idx, vptime->cnt, vptime->intval, (void *)d );
    if ( status < CDF_OK )
    {
      CDFerror( status, msg );
                            ...
                            Array pos, Array con, int *posdel, int *condel )
{
  ...
  for ( i = 0, fn = vpdata->strframe; fn <= maxframe;
        fn += vpdata->dltframe, i++ )
  {
...
static Object impcdf0dim( Infocdf cdfp, Infovar vpdata, Array pos, int *posdel )
{
  ...
  status = CDFvarHyperGet( vpdata->id, vpdata->varNum, recStart, recCount,
                           recInterval, vpdata->idx, vpdata->cnt,
                           vpdata->intval, (void *)data );
  if ( status < CDF_OK )
  {
    CDFerror( status, msg );
    DXSetError( ERROR_MISSING_DATA, msg );
                          ...
                          Array con, int *posdel, int *condel )
{
  ...
  {
    if ( ( datacm = (void *)DXAllocate( vpdata->size * vpdata->numbytes ) ) ==
         NULL )
      goto error;
...
static Error cdfGetData( Infovar vp, long rec, void *data )
{
  ...
  status = CDFvarHyperGet( vp->id, vp->varNum, rec, 1, 1, vp->idx, vp->cnt,
                           vp->intval, (void *)data );
  if ( status < CDF_OK )
  {
    CDFerror( status, msg );
...
inquire:
  ...
  if ( ( *cdfp = (struct infocdf *)DXAllocate( sizeof( struct infocdf ) ) ) ==
       NULL )
    goto error;
...
static Error cdfrotate( Infocdf cdfp, Infovar vpdata, void *data )
{
  ...
  if ( cdfp->numDims == 1 )
  {
    if ( cdfp->rotate[0] != 0 )
    {
      if ( ( tmparr = (void *)DXAllocate( vpdata->size * vpdata->numbytes ) ) ==
           NULL )
        goto error;
  ...
  else if ( cdfp->numDims == 3 )
  {
    if ( cdfp->rotate[0] != 0 )
    {
      if ( ( tmparr = (void *)DXAllocate( vpdata->size * vpdata->numbytes ) ) ==
        ...
        goto error;
      memcpy( (char *)tmparr, (char *)data, vpdata->size * vpdata->numbytes );
      off1 = cdfp->dimSizes[2] * cdfp->dimSizes[1] * cdfp->rotate[0] *
             vpdata->numbytes;
...
static Error GetAttribute( Infovar vp, Object f, int var )
{
  ...
  while ( ( status = CDFattrInquire( vp->id, i, aname, &ascope, &numentry ) ) >=
          CDF_OK )
  {

    if ( var == 1 &&
         ( ascope == VARIABLE_SCOPE || ascope == VARIABLE_SCOPE_ASSUMED ) )
      entry = CDFvarNum( vp->id, vp->name );
    else if ( var == 0 &&
              ( ascope == GLOBAL_SCOPE || ascope == GLOBAL_SCOPE_ASSUMED ) )
      entry = numentry;
    else
    {
      i++;
...
static Error buildAxis( int n, Infocdf cdfp, Infovar vp, float *data,
                        float *isreg );
...
Array impcdfpos( Infovar vploc, Infocdf cdfp )
{
  ...
  if ( vploc == NULL && cdfp->numDims >= 1 )
  {
    ...
    if ( !( a = DXMakeGridPositionsV( cdfp->numDims, dimsize, origin,
                                      deltas ) ) )
      goto error;
    goto ok;
  }
  ...
  else if ( cdfp->numDims > 1 )
  {
    for ( i = 0; i < cdfp->numDims; i++ )
    {
      ...
      if ( isreg )
      {
        ...
        DXDelete( (Object)a );
        a = NULL;
        a = (Array)DXNewRegularArray( TYPE_FLOAT, (int)( cdfp->numDims ),
                                      p->size, origin, deltas );
        if ( !a )
          goto error;
      }
...
static Error buildAxis2( Infocdf cdfp, Infovar vp, float *data, int cntvar )
{
  ...
  while ( ptr )
  {
    ...
    {
      status =
          CDFvarHyperGet( ptr->id, ptr->varNum, recStart, recCount, recInterval,
                          ptr->idx, ptr->cnt, ptr->intval, (void *)d );
                           ...
                           int endframe, int deltaframe )
{
  ...
  {
    ptr = NULL;
    if ( ( ptr = (struct infovar *)DXAllocate( sizeof( struct infovar ) ) ) ==
         NULL )
      goto error;
    ...
    */
    ...
    ptr->varNum = i;
    status = CDFvarInquire( ptr->id, ptr->varNum, ptr->name, &( ptr->dataType ),
                       ...
                       char **varname, int numDims )
{
  ...
  {/* var can't be positions variable */
    for ( i = 0; i < varcnt; i++ )
    {
      ptr = *vpdata;
      match = 0;
      ...
      if ( match == 0 )
      {
        DXSetError( ERROR_BAD_PARAMETER, "%s can't be variable, it is imported "
                                         "as positions or series positions ",
                    varname[i] );
        return ERROR;
      }
    }
...
static Error cleanPosVar( Infovar *vploc, Infocdf cdfp, Infovar *vpdata )
{
  ...
  if ( cdfp->numDims > 2 )
  {
    ...
    if ( i != cdfp->numDims )
      DXErrorGoto( ERROR_BAD_PARAMETER, "Number of positions variables are not "
...
static int categorizeVar( long dims, Infovar vp )
{
  ...
  {
    if ( ( !strcmp( vp->name, "LATITUDE" ) ) ||
         ( !strcmp( vp->name, "LONGITUD" ) ) )
      category = POSITIONCOMP;
                               ...
                               Infovar vptime )
{
  ...
  Infovar ptr;
  int maxframe;
  static char *nameConnect[] = {"point",       /* rank 0 */
                                "lines",       /* rank 1 */
                                "quads",       /* rank 2 */
                                "cubes",       /* rank 3 */
                                "hypercubes",  /* rank 4 */
  ...
  while ( ptr )
  {
    for ( i = 0, d = 0, s = 1, k = cdfp->numDims - 1; i < cdfp->numDims;
          i++, k-- )
    {
    ...
    else if ( !( ptr->numbytes = dxtype2bytes( ptr->dxtype ) ) )
      goto error;
    ptr = ptr->next;
  }
...
ImportStatReturn _dxfstat_cdf( char *filename )
{
  Infocdf cdf;
...
Object DXImportCDF( char *filename, char **fieldlist, int *startframe,
                    int *endframe, int *deltaframe )
{
  DXSetError( ERROR_NOT_IMPLEMENTED, "CDF libs not included" );
  return ERROR;
}
