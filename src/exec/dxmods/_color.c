...
#define ISGENERICGROUP( x )                             \
  ( ( DXGetObjectClass( (Object)x ) == CLASS_GROUP ) && \
...
static Error ColorIndividual( Object, Object, Object, char *, int, int, int,
                              int, RGBColor, float, int );
static Error ColorObject( Object, Object, Object, char *, int, int, int, int,
                          RGBColor, float, int );

extern Array DXScalarConvert( Array ); /* from libdx/stats.h */

Error _dxfColorRecurseToIndividual( Object o, Object color, Object opacity,
                                    char *component, int setcolor,
                                    int setopacity, int colorfield,
                                    ...
                                    float opacityval, int immediate )
{
  ...
  switch ( class )
  {
        ...
        default:
                ...
                i++ )
          {
            ...
            if ( ( opacityfield ) && ( ISGENERICGROUP( opacity ) ) )
            {
              ...
              subopacity = opacity;
            if ( !_dxfColorRecurseToIndividual(
                      subo, subcolor, subopacity, component, setcolor,
                      setopacity, colorfield, opacityfield, colorvec,
                      opacityval, immediate ) )
              return ERROR;
          }
    ...
    case CLASS_FIELD:
      ...
      if ( !ColorIndividual( o, color, opacity, component, setcolor, setopacity,
                             colorfield, opacityfield, colorvec, opacityval,
                             immediate ) )
        return ERROR;
      break;
    case CLASS_SCREEN:
      if ( !( DXGetScreenInfo( (Screen)o, &subo, NULL, NULL ) ) )
        return ERROR;
      if ( !_dxfColorRecurseToIndividual(
                subo, color, opacity, component, setcolor, setopacity,
                colorfield, opacityfield, colorvec, opacityval, immediate ) )
        return ERROR;
      break;
    case CLASS_CLIPPED:
...
static Error ColorObject( Object o, Object color, Object opacity,
                          char *component, int setcolor, int setopacity,
                          int colorfield, int opacityfield, RGBColor colorvec,
                          float opacityval, int immediate )
{
  int i;
  Object subo, subcolor, subopacity;
  ...
  switch ( DXGetObjectClass( o ) )
  {
    ...
    case CLASS_SCREEN:
      if ( !( DXGetScreenInfo( (Screen)o, &subo, NULL, NULL ) ) )
        return ERROR;
      if ( !ColorObject( subo, color, opacity, component, setcolor, setopacity,
                         colorfield, opacityfield, colorvec, opacityval,
                         immediate ) )
        return ERROR;
      break;
...
static Error ColorField( Pointer ptr )
{
  ...
  if ( !colorfield )
    corigin =
        (Array)DXNewConstantArray( 1, &c, TYPE_FLOAT, CATEGORY_REAL, 1, 3 );
  if ( !opacityfield )
    oorigin =
        (Array)DXNewConstantArray( 1, &o, TYPE_FLOAT, CATEGORY_REAL, 0, NULL );
  newdata = NULL;
  savedfield = NULL;
  ...
  if ( !( a_d = (Array)DXGetComponentValue( f, "data" ) ) )
  {
    ...
    else
    {
      ...
      {
        if ( !( att = DXGetString(
                    (String)DXGetComponentAttribute( f, "colors", "dep" ) ) ) )
        {
          DXSetError( ERROR_DATA_INVALID,
                      "bad or missing color dep attribute" );
          goto error;
        }
  ...
  else
  {
    ...
    if ( !( att = DXGetString(
                (String)DXGetComponentAttribute( f, "data", "dep" ) ) ) )
    {
  ...
  if ( setcolor )
  {
    ...
    if ( immediate )
    {
      if ( color )
      {
        ...
        if ( !floatcolors )
        {
          if ( !DXMapCheck( (Object)f, (Object)copycolor, "data", NULL, NULL,
                            NULL, NULL ) )
            goto error;
    ...
    else
    {
      if ( color )
      {
        ...
        if ( !( DXGetArrayInfo( (Array)mapposarray, &mapcounts, NULL, NULL,
                                NULL, NULL ) ) )
        {
          goto error;
        }
        ...
        for ( j = 0; j < mapcounts - 1; j++ )
        {
          if ( ( dp_m[j] > dp_m[j + 1] ) )
          {
            DXSetError( ERROR_BAD_PARAMETER, "non-monotonic color map" );
            goto error;
          }
        }
        ...
        if ( dp_m[0] == dp_m[mapcounts - 1] )
        {
          for ( k = 0; k <= 255; k++ )
          {
            ...
            {
              colortable[k] = DXRGB( dp_md[0], dp_md[1], dp_md[2] );
            }
          }
        }
        else
        {
          for ( k = minmap; k <= maxmap; k++ )
          {
            ...
            else
            {
              ...
              if ( dp_m[j - 1] == k )
                colortable[k] =
                    DXRGB( dp_md[3 * ( j - 1 )], dp_md[3 * ( j - 1 ) + 1],
                           dp_md[3 * ( j - 1 ) + 2] );
              else
              {
                fraction = ( k - dp_m[j - 1] ) / ( dp_m[j] - dp_m[j - 1] );
                red = fraction * ( dp_md[3 * j] - dp_md[3 * ( j - 1 )] ) +
                      dp_md[3 * ( j - 1 )];
                green =
                    fraction * ( dp_md[3 * j + 1] - dp_md[3 * ( j - 1 ) + 1] ) +
                    dp_md[3 * ( j - 1 ) + 1];
                blue =
                    fraction * ( dp_md[3 * j + 2] - dp_md[3 * ( j - 1 ) + 2] ) +
                    dp_md[3 * ( j - 1 ) + 2];
      ...
      {
        DXSetError( ERROR_BAD_PARAMETER, "delayed=1 requires data component" );
        goto error;
      }
  ...
  if ( setopacity )
  {
    ...
    if ( immediate )
    {
      if ( opacity )
      {
        ...
        if ( !floatopacities )
        {
          if ( !DXMapCheck( (Object)f, (Object)copyopacity, "data", NULL, NULL,
                            NULL, NULL ) )
            goto error;
        ...
        {
          if ( !DXMapCheck( (Object)f, opacity, "data", NULL, NULL, NULL,
                            NULL ) )
            goto error;
    ...
    else
    {
      if ( opacity )
      {
        if ( !( mapposarray = (Array)DXGetComponentValue( (Field)opacity,
                                                          "positions" ) ) )
        {
          DXSetError( ERROR_BAD_PARAMETER, "bad map positions" );
          goto error;
        ...
        for ( j = 0; j < mapcounts - 1; j++ )
        {
          if ( ( dp_m[j] > dp_m[j + 1] ) )
          {
            DXSetError( ERROR_BAD_PARAMETER, "non-monotonic opacity map" );
            goto error;
          }
        }
        minmap = dp_m[0];
        ...
        for ( k = minmap; k <= maxmap; k++ )
        {
          ...
          else
          {
            ...
            else
            {
              fraction = ( k - dp_m[j - 1] ) / ( dp_m[j] - dp_m[j - 1] );
              opacitytable[k] =
                  fraction * ( dp_md[j] - dp_md[j - 1] ) + dp_md[j - 1];
            }
          }
        }
      }
...
static Error ColorIndividual( Object o, Object color, Object opacity,
                              char *component, int setcolor, int setopacity,
...

{
  ...
  if ( !surface )
  {
    ...
    else
    {
      if ( setopacity )
      {
        ...
        {
          opacitymult =
              (float)( -LN( 1.0 - opacityval ) / ( thickness * opacityval ) );
          intensitymult = opacitymult;
        }
      }
      else
  ...
  if ( !ColorObject( o, color, opacity, component, setcolor, setopacity,
                     colorfield, opacityfield, colorvec, opacityval,
                     immediate ) )
    return ERROR;
...
static Error ConvertColors( Object copycolor )
{
  ...
  switch ( DXGetObjectClass( copycolor ) )
  {
    case( CLASS_FIELD ) :
      ...
      if ( !( newcolors = DXArrayConvert( oldcolors, TYPE_FLOAT, CATEGORY_REAL,
                                          1, 3 ) ) )
        goto error;
      if ( !( DXSetComponentValue( (Field)copycolor, "data",
                                   (Object)newcolors ) ) )
        goto error;
      break;
    case( CLASS_GROUP ) :
      for ( i = 0;
            ( subo = DXGetEnumeratedMember( (Group)copycolor, i, NULL ) ); i++ )
      {
  ...
  switch ( DXGetObjectClass( copyopacity ) )
  {
    case( CLASS_FIELD ) :
      ...
      if ( !( newopacities = DXArrayConvert( oldopacities, TYPE_FLOAT,
        ...
        goto error;
      if ( !( DXSetComponentValue( (Field)copyopacity, "data",
                                   (Object)newopacities ) ) )
        goto error;
      break;
    case( CLASS_GROUP ) :
      for ( i = 0;
            ( subo = DXGetEnumeratedMember( (Group)copyopacity, i, NULL ) );
            i++ )
      {
