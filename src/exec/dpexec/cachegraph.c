...
static uint32 computeDefaultRecipe( Object obj )
{
...
static uint32 computeStringRecipe( String a )
{
  uint32 crc;
  unsigned char *p;
...
static uint32 computeArrayRecipe( Array a )
{
  uint32 crc;
  Type type;
  Category category;
...
static uint32 computeRecipe( Program *p, int ind )
{
  ProgramVariable *pv;
  Object obj;
  uint32 tcrc;
...
/*---------------------------------------------------------------*/
int _dxf_ExManageCacheTable( ModPath *mod_path, uint32 reccrc, int outnbr )
{
  int i, limit;
  pathtag *pt;
  pathtag npt;
  ...
  DXDebug( "1", "cache table entry: cpath = %s, reccrc = 0x%08x, outnbr = %d",
           ...
           pt->outnbr == outnbr )
      {
        ...
        if ( pt->reccrc != reccrc )
        {
          DXDebug( "1",
                   ...
                   pt->reccrc, reccrc, _dxf_ExPathToString( &pt->mod_path ) );
          if ( pt->reccrc != 0 )
...
check_async:
  ...
  for ( i = 0; i < SIZE_LIST( n->inputs ); i++ )
  {
    ...
    ExDebug( "1", "Cache value for input %s.%d = 0x%08x ", n->name, i,
             inTags[i] );
  }
  ...
  if ( n->flags & ( MODULE_CONTAINS_STATE | MODULE_CHANGES_STATE ) )
  {
    ...
    else
    {
      ...
      if ( oldtag == 0 )
      {
        DXSetCacheEntry( new_tag_obj, CACHE_PERMANENT, mod_cache_str, ntags,
                         0 );
        DXDelete( old_tag_obj );
      }
  ...
  for ( i = 0; i < SIZE_LIST( n->outputs ); i++ )
  {
         ...
         ( n->required != -1 && ( !IsSwitch( n ) || !IsFastSwitch( n ) ) ) )
    {
      ...
      if ( n->procgroupid )
      {
        ...
        out->reccrc = EXTAG( _dxf_ExCRCByteV( out->reccrc & 0x7fffffff,
                                              (unsigned char *)n->procgroupid,
                                              1, size ) );
      }
      ExDebug( "1", "Cache value for %s.%d = 0x%08x ", n->name, i,
               out->reccrc );
    }
    ...
    else if ( fcache == CACHE_LAST && !pv->cacheattr )
      tag_changed = _dxf_ExManageCacheTable( &n->mod_path, out->reccrc, i );
