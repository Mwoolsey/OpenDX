...
The feedback terms table consists of 256, 32-bit entries.  Notes:
    ...
    It might not be obvious, but the feedback 
...
/*
 * Updates the crc 'c' with the contents of the byte 'b'
 */

#define UPDCB(b, c)\
    (cr3tab [((EXCRC) (c) ^ (EXCRC) (b)) & 0xff] ^ (((c) >> 8) & 0x00FFFFFF))
...
EXCRC _dxf_ExCRCFloat (EXCRC crc, double v)
{
    ...
    return (_dxf_ExCRCByteV (crc, (unsigned char *) &f, sizeof (float), 1));
}

EXCRC _dxf_ExCRCDouble (EXCRC crc, double v)
{
    return (_dxf_ExCRCByteV (crc, (unsigned char *) &v, sizeof (double), 1));
}

EXCRC _dxf_ExCRCString (EXCRC crc, char *v)
{
    return (_dxf_ExCRCByte0 (crc, (unsigned char*) v));
}
...
{
    return (_dxf_ExCRCByteV (crc, (unsigned char *) v, sizeof (int), n));
}

EXCRC _dxf_ExCRCFloatV (EXCRC crc, float *v, int n)
{
    return (_dxf_ExCRCByteV (crc, (unsigned char *) v, sizeof (float), n));
}

EXCRC _dxf_ExCRCDoubleV (EXCRC crc, double *v, int n)
{
    return (_dxf_ExCRCByteV (crc, (unsigned char *) v, sizeof (double), n));
}
