...
int DXEnvironment::Setup( const int argc, char **argv )
{
  ...
  for ( i = 0; i < numparams; i++ )
  {
    ...
    else if ( s == "-image" || s == "-edit" || s == "-java" || s == "-kiosk" ||
              s == "-menubar" )
    {
      uimode = s;
      if ( s == "-menubar" )
    ...
    else if ( s == "-verbose" || s == "-echo" )
    {
      seeflags = true;
    }
    else if ( s == "-help" || s == "-shorthelp" || s == "-h" )
      ShortHelp();
    else if ( s == "-morehelp" || s == "-longhelp" )
      LongHelp();
    else if ( s == "-version" )
    {
      showversion = true;
  ...
  addExFlag(
      exhighlight );  // Since highlight can only be turned off with the flag.
